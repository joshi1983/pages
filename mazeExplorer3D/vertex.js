...
class Vertex {
	...
	*/
	constructor(position, colour, textureCoordinates) {
		this._validateArrayOfNumber(position, 'position', 3);
		this._validateArrayOfNumber(colour, 'colour', 3);
		this._validateArrayOfNumber(textureCoordinates, 'textureCoordinates', 2);
		this.position = position;
		this.colour = colour;
		this.textureCoordinates = textureCoordinates;
	}

	_validateArrayOfNumber(a, name, len) {
		if (!(a instanceof Array))
			throw new Error(name + ' must be an Array.');
		if (a.length !== len)
			throw new Error(name + ' must have length ' + len + '.  Not: ' + a.length);
		for (var i = 0; i < len; i++) {
