...
import { GLTFTransformer } from './GLTFTransformer.js';
...
export class GLTFNode {
	...
	getTrianglePositions(binDataView) {
		...
		var result = [];
		if (this.mesh)
			result = this.mesh.getTrianglePositions(binDataView);
		for (var i = 0; i < this.children.length; i++) {
			GLTFUtils.addToArray(result, this.children[i].getTrianglePositions(binDataView));
		}
