...
export class TestParseLogger extends ParseLogger {
	constructor(logger, code, ignoreWarnings, isLoggingMessage) {
		...
		if (typeof isLoggingMessage === 'function')
			...
			this.isLoggingMessage = function(msg) {
				return (msg.type === MessageTypes.TypeWarning) || (outer.isLoggingErrors && msg.type === MessageTypes.TypeError);
			};
		else {
			this.isLoggingMessage = function(msg) {
				return (outer.isLoggingErrors && msg.type === MessageTypes.TypeError) || msg.type !== MessageTypes.TypeError;
			};
		}
		this.logger = logger;
		this.code = code;
