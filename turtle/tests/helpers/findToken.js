...
export function findToken(filterInfo, tokens, logger) {
	...
	const matches = tokens.filter(function(token) {
		...
		if (filterInfo.type !== undefined && filterInfo.type !== token.type)
			return false;
		if (filterInfo.hasParentVal !== undefined &&
		(token.parentNode === null || token.parentNode.val !== filterInfo.hasParentVal))
			return false;
		if (filterInfo.hasParentType !== undefined &&
		(token.parentNode === null ||
		token.parentNode.type !== filterInfo.hasParentType))
			return false;
		if (filterInfo.hasGrandParentVal !== undefined &&
		(token.parentNode === null || token.parentNode.parentNode === null ||
		token.parentNode.parentNode.val !== filterInfo.hasGrandParentVal))
			return false;
