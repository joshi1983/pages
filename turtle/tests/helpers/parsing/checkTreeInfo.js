...
export function checkTreeInfo(token, treeInfo, logger, ParseTreeTokenType) {
	if ((typeof ParseTreeTokenType !== 'object') &&
	(typeof ParseTreeTokenType !== 'function'))
		throw new Error(`ParseTreeTokenType must be specified as an object or function but found type ${typeof ParseTreeTokenType} ` +
		`which is ${ParseTreeTokenType}`);
	if (typeof (ParseTreeTokenType.getNameFor) !== 'function')
		throw new Error(`ParseTreeTokenType.getNameFor must be specified as a function but found type ${typeof (ParseTreeTokenType.getNameFor)} which is ${ParseTreeTokenType.getNameFor}`);
