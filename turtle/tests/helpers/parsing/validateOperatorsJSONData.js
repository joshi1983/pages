...
function getSortedSymbolsString(operators) {
	const symbols = operators.map(op => op.symbol).filter(symbol => symbol !== undefined);
...
export function validateOperatorsJSONData(operators, logger, assignmentsMustContainEqualCharacter) {
	if (assignmentsMustContainEqualCharacter === undefined)
		assignmentsMustContainEqualCharacter = true;
	if (typeof assignmentsMustContainEqualCharacter !== 'boolean')
		throw new Error(`assignmentsMustContainEqualCharacter must be undefined or boolean but found ${assignmentsMustContainEqualCharacter}`);
		...
		operators.forEach(function(operatorInfo, index) {
			...
			if (typeof operatorInfo !== 'object')
				...
				if (operatorInfo.isAssignment !== undefined) {
					...
					if (assignmentsMustContainEqualCharacter &&
					operatorInfo.isAssignment === true && operatorInfo.symbol.indexOf('=') === -1)
				...
				if (index > 0 && typeof operators[index - 1] === 'object' && operators[index - 1] !== null && typeof operators[index - 1].symbol === 'string') {
					...
					if (typeof prevSymbol === 'string' && typeof operatorInfo.symbol === 'string' && prevSymbol.localeCompare(operatorInfo.symbol) >= 0)
						plogger(`operators should be sorted by symbol but found pair out of order with symbols ${prevSymbol} and ${operatorInfo.symbol}.  All symbols sorted properly are: ${getSortedSymbolsString(operators)}`);
				}
