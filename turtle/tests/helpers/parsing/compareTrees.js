...
import { ParseTreeToken as PythonParseTreeToken } from '../../../modules/parsing/python-parsing/ParseTreeToken.js';
...
function toReadable(key, val) {
	if (key === 'type')
		return ParseTreeTokenType.getNameFor(val);
	else
		return val;
}
	...
	if (node1 !== null && node2 !== null) {
		...
		propertyKeys_.forEach(function(key) {
			if (node1[key] !== node2[key]) {
				let msg = `node ${key} values expected to be the same but they are not.  ${node1Description}.${key}=${toReadable(key, node1[key])}, ${node2Description}.${key}=${toReadable(key, node2[key])}`;
				if (key !== 'val')
...
export function compareTrees(node1, node2, logger, settings) {
	...
	if (node1.children.length !== node2.children.length)
		logger(`children length expected to be the same but they are not. node1.children.length=${node1.children.length}, `+
			...
			`node2.val=${node2.val}, node1.type is ${ParseTreeTokenType.getNameFor(node1.type)}, node2.type=${ParseTreeTokenType.getNameFor(node2.type)}`);
	else {
