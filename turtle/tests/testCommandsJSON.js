...
function validateArgCount(commandInfo, logger) {
	if (commandInfo.argCount !== undefined) {
		if (typeof commandInfo.argCount === 'string') {
			...
			if (typeof commandInfo.argCount.min !== 'number' && commandInfo.argCount.min !== undefined)
				logger('If argCount.min is specified, it should be a number.  Not: ' + commandInfo.argCount.min);
			if (typeof commandInfo.argCount.max !== 'number' && commandInfo.argCount.max !== undefined)
				logger('If argCount.max is specified, it should be a number.  Not: ' + commandInfo.argCount.max);
		}
		else
	...
	commands.forEach(function(commandInfo, index) {
		...
		if (typeof commandInfo.isIndependentlyUseful !== 'boolean')
			...
			if (commandInfo.args.filter(a => a.refTypes !== undefined).length === 0) {
				plogger('isIndependentlyUseful tends to be true only for turtle or compiled commandGroup commands or they have refTypes set on an argument.  ' +
