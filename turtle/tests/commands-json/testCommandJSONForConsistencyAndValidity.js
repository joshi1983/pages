...
export function testCommandJSONForConsistencyAndValidity(logger) {
	commands.forEach(function(commandInfo, index) {
		...
		const arrayProperties = ['names', 'args', 'hiddenKeywords', 'hintNames', 'searchKeywords'];
		...
		if (commandInfo.hiddenSearchKeywords !== undefined)
			plogger(`The hiddenSearchKeywords is not to be set.  Consider renaming to hiddenKeywords.`);
		...
		if (commandInfo.isUnstable !== undefined && typeof commandInfo.isUnstable !== 'boolean')
			plogger('Command isUnstable should be undefined or boolean but is not at index ' + index + ', primaryName = ' + commandInfo.primaryName);
