export function testCommandArg(commandInfo, arg, argIndex, logger) {
	...
	const optionalStrings = ['format', 'sameAsOutputIfTypesEqual'];
	for (const key of optionalStrings) {
		if (arg[key] !== undefined && typeof arg[key] !== 'string')
			logger(`${key} must be either undefined or a string.  Not: ${arg[key]}`);
	}
