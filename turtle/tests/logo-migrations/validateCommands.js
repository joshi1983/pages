...
import { validateApplicableForQBasicFixer } from './validateApplicableForQBasicFixer.js';
...
function isOutOfOrder(prev, next) {
	for (const info of [prev, next]) {
		if (typeof info !== 'object' ||
		typeof info.primaryName !== 'string')
			return false;
	}
	const prevName = prev.primaryName.toLowerCase();
	const nextName = next.primaryName.toLowerCase();
	...
	if (prevName === nextName) {
		if (prev.className !== next.className)
			return false;
		if (prev.isStaticMethod !== next.isStaticMethod)
			return false;
...
export function validateCommands(fullInfoObject, logger) {
	if (!(fullInfoObject.commands instanceof Array))
		...
		fullInfoObject.commands.forEach(function(commandInfo, index) {
			...
			if (index > 0 && isOutOfOrder(fullInfoObject.commands[index - 1], commandInfo)) {
			...
			fullInfoObject.primaryNameIsNotUnique !== true) {
			...
			const optionalBooleanValues = ['applicableForQBasicFixer', 'isIndependentFunction',
			'isReadingRegister', 'isStaticMethod', 'isWritingToRegister',
				...
				commandInfo.names.forEach(function(name) {
					if (typeof name !== 'string')
						...
						if (lowerCaseNames.has(name.toLowerCase())) {
							if (fullInfoObject.caseSensitiveCommandNames !== true)
			...
			validateApplicableForQBasicFixer(commandInfo, plogger);
