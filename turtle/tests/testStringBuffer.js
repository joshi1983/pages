...
function testInsertAtStart(logger) {
	const cases = [
		{'strings': [], 'in': 'hi', 'result': 'hi'},
		{'strings': ['yo'], 'in': 'hi', 'result': 'hiyo'},
		{'strings': ['yo','ya'], 'in': 'hi', 'result': 'hiyoya'},
	];
	cases.forEach(function(caseInfo, index) {
		const plogger = prefixWrapper(`Case ${index}, strings: ${JSON.stringify(caseInfo.strings)}`, logger);
		const stringBuffer = new StringBuffer();
		stringBuffer.strings = caseInfo.strings;
		stringBuffer.insertAtStart(caseInfo.in);
		if (stringBuffer.toString() !== caseInfo.result)
			plogger(`Expected ${caseInfo.result} but got ${stringBuffer.toString()}`);
	});
}
