...
function processTestCase(caseInfo, logger) {
	let testParseLogger;
	if (caseInfo.ignoreParseErrors)
		testParseLogger = new TestParseLogger(noop, caseInfo.code);
	else
		testParseLogger = new TestParseLogger(logger, caseInfo.code);
	...
	if (typeof tree !== 'object')
		...
		if (caseInfo.numTopTokens !== undefined &&
		countTopTokens(tokens) !== caseInfo.numTopTokens) {
...
function testAsyncCases(logger) {
	var cases = [
		...
		{'codeUrl': 'xkcd.lgo', 'numTopTokens': 4},
	...
	cases.forEach(function(caseInfo) {
		if (caseInfo.codeUrl.indexOf('/') === -1)
			caseInfo.codeUrl = 'tests/data/logo-scripts/' + caseInfo.codeUrl;
...
export function testParseTree(logger) {
	var cases = [
		...
		{'code': '[-:x 2]', 'numTopTokens': 1, 'toString': '[ - :x 2 ]'},
		{'code': 'make "x 4\nprint [-:x 2]', 'numTopTokens': 5, 'toString': 'make "x 4 print [ - :x 2 ]'},
		{
			'code': 'to donothing\nend',
	...
	cases.forEach(function(caseInfo, index) {
		const prefixedLogger = prefixWrapper(`Case ${index}. Failure while processing code ${caseInfo.code}`, logger);
		try {
