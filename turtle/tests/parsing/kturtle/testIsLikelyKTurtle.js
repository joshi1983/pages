...
for (let i = 0; i < pythonTurtleExampleFiles.length; i++) {
	const url = pythonTurtleExampleFiles[i];
	pythonExamples.push(await fetchText('tests/data/python/' + url));
}
...
export function testIsLikelyKTurtle(logger) {
	...
	pythonExamples.forEach(function(code) {
		cases.push({'in': code, 'out': false});
	});
