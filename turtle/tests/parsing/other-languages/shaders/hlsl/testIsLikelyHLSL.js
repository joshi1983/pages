...
const nonExamples = ArrayUtils.combine(
glslExamples, holyCExamples, kojoExamples,
kotlinExamples, processingExamples, rustTurtleExamples, wgslExamples);

export function testIsLikelyHLSL(logger) {
	const cases = [];
	hlslExamples.forEach(function(code) {
		cases.push({
			'in': code,
			'out': true
		});
	...
	testInOutPairs(cases, isLikelyHLSL, logger);
};