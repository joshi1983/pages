...
export function testIsLikelyQBasic(logger) {
	const cases = basicFusionExamples.concat(bazzBasicExamples).concat(pBasicExamples).concat(microAExamples).
	concat(webLogoExamplesContent).concat(smallVisualBasicExamples).map(function(content) {
		return {
			'in': content,
			'out': false
		};
