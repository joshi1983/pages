...
'../../../../modules/parsing/other-languages/pitrified-go-turtle/MigrationInfo.js';
			...
			if (typeof info !== 'object' || info === null) {
				...
				if (info.package !== null) {
					if (typeof info.package !== 'string') {
						plogger(`package must be a string or null but found ${info.package}`);
					}
...
export function testMigrationJSON(logger) {
	if (typeof data !== 'object')
		...
		validateConstantOrFunctionBasics(data.properties, packageAliases, prefixWrapper('properties', logger));
		if (data.functions instanceof Array) {
			data.functions.forEach(function(info, index) {
				if (typeof info !== 'object')
					return;

				const plogger = prefixWrapper(`function index ${index}, name=${info.name}`, logger);
