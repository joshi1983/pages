...
function testHighLevelCases(logger) {
	...
	cases.forEach(function(caseInfo) {
		...
		const prefixLogger = prefixWrapper('Failure while processing code ' + caseInfo.code, logger);
		try {
...
export function testLogoScannerTokenSplitter(logger) {
	testHighLevelCases(logger);
	testSpecialCases(logger);
	testSplitSingleTokenAroundQuotes(logger);
};