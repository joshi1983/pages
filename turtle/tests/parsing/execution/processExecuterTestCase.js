	...
	if (testLogger.hasLoggedErrors())
		...
		if (testLogger.hasLoggedErrors())
			...
			compileOptionsArray.forEach(function(compileOptions) {
				...
				const turtleInfo = createPrintCountTestTurtle();
				...
				if (turtleInfo.settings.messages.length !== caseInfo.messages.length)
					prefixLogger('Expected to print ' + caseInfo.messages.length + ' but printed ' + turtleInfo.settings.messages.length + ' times.  The messages actually printed were: ' + JSON.stringify(turtleInfo.settings.messages));
					...
					for (let i = 0; i < caseInfo.messages.length; i++) {
						...
						if (actual !== expected) {
							prefixLogger('Expected message ' + expected + ' but got ' + actual + '.');
						}
					}
				}
			});
