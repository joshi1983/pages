...
function testRunningProcedure(logger) {
	...
	const print = turtle.settings.print;
	let msg;
	turtle.settings.print = function(_msg) {
		msg = _msg;
	};
	instruction.execute(context);
	...
	turtle.settings.print = print; // restore so no side effects continue.
}

export function testCallHighOrderInstruction(logger) {
	testRunningCommand(prefixWrapper('testRunningCommand', logger));
	testRunningProcedure(prefixWrapper('testRunningProcedure', logger));
};