...
function compileCode(code, harmonizeChecked, exampleInfo, logger) {
	...
	if (parseLogger.hasLoggedErrors())
		...
		if (parseLogger.hasLoggedErrors())
			logger(`No analysis errors expected but some found.  Messages include ${parseLogger.messages.map(m => m.msg).join(',')}`);
...
function checkZipConsistency(fetchedContent, url, logger) {
	...
	if (copyrightenExamples.some(cr => cr.filename === url))
		return; // We don't expect copyrighten examples to be in the zip.
