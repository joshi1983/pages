...
function testRemoveAllChildren(logger) {
	const token1 = new ParseTreeToken('tok1', null, 0, 0, ParseTreeTokenType.LEAF);
	token1.removeAllChildren();
	const token = new ParseTreeToken("hi", null, 0, 0, ParseTreeTokenType.LONG_STRING_LITERAL, "hi");
	token1.appendChild(token);
	assertEquals(1, token1.children.length, prefixWrapper('After appendChild(...', logger));
	token1.removeAllChildren();
	assertEquals(0, token1.children.length, prefixWrapper('After removeAllChildren()', logger));
}
...
function testRemoveSingleToken(logger) {
	const root = new ParseTreeToken(null, null, 0, 0, ParseTreeTokenType.TREE_ROOT);
	const child1 = new ParseTreeToken(3, null, 0, 0, ParseTreeTokenType.NUMBER_LITERAL, '3');
	root.appendChild(child1);
	child1.removeSingleToken();
	assertEquals(0, root.children.length, prefixWrapper(`After calling removeSingleToken() on token 3`, logger));
	if (root.children.length === 0) {
		const plus = new ParseTreeToken('+', null, 0, 0, ParseTreeTokenType.BINARY_OPERATOR);
		const child2 = new ParseTreeToken('y', null, 0, 0, ParseTreeTokenType.LEAF);
		root.appendChild(plus);
		plus.appendChild(child1);
		plus.appendChild(child2);
		plus.removeSingleToken();
		assertEquals(2, root.children.length, prefixWrapper(`After calling removeSingleToken() on token +`, logger));
	}
}
