...
function testCreateFromScannedToken(logger) {
	...
	cases.forEach(function(caseInfo) {
		...
		if (result.type !== caseInfo.type)
			logger('Expected type ' + ParseTreeTokenType.getNameFor(caseInfo.type) + ' for input ' + caseInfo.in + ' but got ' + ParseTreeTokenType.getNameFor(result.type));
		if (result.toString() !== caseInfo.str)
			logger('Expected ' + caseInfo.str + ' but got ' + result.toString());
	});
}
