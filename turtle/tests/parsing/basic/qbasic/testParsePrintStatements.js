...
export function testParsePrintStatements(logger) {
	const cases = [{
		...
		'treeInfo': {
			...
			'children': [
				{'val': null, 'type': ParseTreeTokenType.FUNCTION_CALL, 'children': [
					{'val': '?', 'type': ParseTreeTokenType.IDENTIFIER},
					{'val': null, 'type': ParseTreeTokenType.ARG_LIST, 'children': [
						{'val': 'x', 'type': ParseTreeTokenType.IDENTIFIER, 'children': []},
					]}
				]}
		...
		'code': 'print 1 2', // missing a comma which is weird.  
		// We still want to parse it in a way that doesn't have number literals as children of number literals.
		// We want to parse it almost like the correct comma or semicolon was there.
		'numTopChildren': 1,
		'treeInfo': {
			'children': [
				{'val': null, 'type': ParseTreeTokenType.FUNCTION_CALL, 'children': [
