	...
	cases.forEach(function(caseInfo, index) {
		...
		if (tokens.length !== numTokens) {
			...
			caseInfo.tokens.forEach(function(tok, index) {
				...
				if (actualTok.s !== s)
					plogger('Expected ' + tok.s + ' but got ' + actualTok.s + ' at index ' + index);
				if (colIndex !== undefined && actualTok.colIndex !== colIndex)
					plogger('Expected colIndex of ' + colIndex + ' but got ' + actualTok.colIndex + ' for token with s of ' + s + ', actual tokens are ' + JSON.stringify(tokens));
			});
		}
	});
}