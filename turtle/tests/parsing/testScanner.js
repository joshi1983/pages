...
export function testScanner(logger) {
	const cases = [
		...
		// {} are used by MSWLogo, FMSLogo and some others to represent arrays.
		// WebLogo doesn't support arrays but we want to scan the brackets properly
		// to help autofix code from them into valid WebLogo code
		{'code': '{}', 'tokens': ['{', '}']},
		{'code': '{-2}', 'tokens': ['{', '-2', '}']},
		{'code': '{2}', 'tokens': ['{', '2', '}']},
		{'code': '{1 2 3}', 'tokens': ['{', '1', '2', '3', '}']},
		{'code': '{a b}', 'tokens': ['{', 'a', 'b', '}']},
		...
		},
		{
			'code': 'repeat :n~\n[]',
			'tokens': [
				{'s': 'repeat', 'colIndex': 5}, {'s': ':n', 'colIndex': 8},
				{'s': '~', 'colIndex': 9},
				{'s': '\n', 'colIndex': 10}, {'s': '[', 'colIndex': 0},
				{'s': ']', 'colIndex': 1}
			]
		},
		{
	...
	cases.forEach(function(caseInfo, index) {
		...
		try {
			...
			if (tokens.length !== numTokens) {
				plogger('Expected ' + numTokens + ' but got ' + tokens.length +
					' tokens from code ' + caseInfo.code + ', the actual scanned token strings are ' +
					tokens.map(t => t.s).join(', '));
			...
			console.error(e);
			plogger(`An error or exception was thrown while scanning.  The message was: ${exceptionToString(e)}`);
		}
	});
}