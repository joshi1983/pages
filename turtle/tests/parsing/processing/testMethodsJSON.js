		...
		methods.forEach(function(methodData, index) {
			...
			if (typeof methodData !== 'object')
				...
				if (methodData.argCount !== undefined) {
					const argCount = methodData.argCount;
					if (typeof argCount === 'object') {
						if (!Number.isInteger(argCount.min) || argCount.min < 0)
							plogger(`When argCount is specified as an object, min should be a positive integer but got ${argCount.min}.`);						
						if (!Number.isInteger(argCount.max) || argCount.max < 1)
							plogger(`When argCount is specified as an object, max should be an integer at least 1 but got ${argCount.max}.`);						
					}
					else if (!Number.isInteger(argCount) || argCount < 0)
						plogger(`When argCount is specified and is not an object, it should be a positive integer but got ${methodData.argCount}.`);
					else if (typeof methodData.to === 'string') {
						const info = Command.getCommandInfo(methodData.to);
						if (info !== undefined) {
							let expectedArgCount = argCount;
