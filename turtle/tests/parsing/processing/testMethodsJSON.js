...
export function testMethodsJSON(logger) {
	if (!(methods instanceof Array))
		...
		for (const procName of procNames) {
			const url = `logo-scripts/processing-content/${procName}.lgo`;
			fetchText(url).catch(function(e) {
				logger(`Failed to download procedure code from URL: ${url}`);
			}).then(function(content) {
				testProcedure(procName, content, logger);
			});
		}
	}
};