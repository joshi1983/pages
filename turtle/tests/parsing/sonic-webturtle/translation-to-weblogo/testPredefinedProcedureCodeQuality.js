...
export function testPredefinedProcedureCodeQuality(logger) {
	for (const [name, content] of procedureMap) {
		...
		if (content.indexOf('$$$') === -1 && name === 'webTurtleTransparent') {
			...
			analyzeCodeQuality(cachedParseTree.root, parseLogger, proceduresMap, initialVariablesMap, isCompleteProgram);
			...
			analyzeCodeQuality(formattedTree.root, formattedParseLogger, proceduresMap, initialVariablesMap, isCompleteProgram);
		}
	}
};