...
function validateClassName(className, logger) {
	const validClassNames = ['random', 'screen', 'time', 'turtle'];
...
function validatePythonFunctionInfo(funcInfo, logger) {
	if (typeof funcInfo !== 'object')
		...
		validateClassName(funcInfo.class, prefixWrapper('validateClassName', logger));
		validateNames(funcInfo.names, prefixWrapper('validateNames', logger));
		validateArgCount(funcInfo, prefixWrapper('validateArgCount', logger));
		validateArgsArray(funcInfo, prefixWrapper('validateArgsArray', logger));
		validateTranslateToCommandInfo(funcInfo, prefixWrapper('validateTranslateToCommandInfo', logger));
...
export function testPythonTurtleJSON(logger) {
	if (!(pythonTurtleFunctions instanceof Array))
		...
		pythonTurtleFunctions.forEach(function(funcInfo, index) {
			validatePythonFunctionInfo(funcInfo, prefixWrapper(`Function ${index}`, logger));
		});
