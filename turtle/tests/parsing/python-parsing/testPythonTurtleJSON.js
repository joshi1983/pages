...
function validateArgsArray(funcInfo, logger) {
	...
	if (!(funcInfo.args instanceof Array))
		...
		funcInfo.args.forEach(function(argInfo, index) {
			if (typeof argInfo !== 'object')
				logger(`Every element in args must be an object but found ${argInfo} at index ${index}`);
			else {
				const plogger = prefixWrapper(`Argument ${index}, name=${argInfo.name}`, logger);
				if (typeof argInfo.name !== 'string')
					plogger(`Argument name must be a string.`);
			}
		});
	}
}
