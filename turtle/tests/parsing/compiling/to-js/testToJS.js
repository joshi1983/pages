import { testAddDeclarationsToProcessOptions } from
'./testAddDeclarationsToProcessOptions.js';
import { testCommandsToJS } from
'./testCommandsToJS.js';
import { testExecuteLists } from
'./testExecuteLists.js';
import { testExecuteProcedures } from
'./testExecuteProcedures.js';
import { testExecuteTests } from
'./testExecuteTests.js';
import { testGetImportStatements } from
'./testGetImportStatements.js';
import { testJSSimplifiers } from
'./js-simplifiers/testJSSimplifiers.js';
import { testTranslateBadCode } from
'./testTranslateBadCode.js';
import { testTranslateWebLogoToJS } from
'./testTranslateWebLogoToJS.js';
import { testWebLogoToJavaScriptJSON } from
'./testWebLogoToJavaScriptJSON.js';
import { wrapAndCall } from
'../../../helpers/wrapAndCall.js';

export function testToJS(logger) {
	wrapAndCall([
		testAddDeclarationsToProcessOptions,
		testCommandsToJS,
		testExecuteLists,
		testExecuteProcedures,
		testExecuteTests,
		testGetImportStatements,
		testJSSimplifiers,
		testTranslateBadCode,
		testTranslateWebLogoToJS,
		testWebLogoToJavaScriptJSON
	], logger);
};