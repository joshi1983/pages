...
function validateArgs(commandInfo, logger) {
	if (commandInfo.args === undefined)
		return;
	if (!(commandInfo.args instanceof Array))
		logger(`Either args should be undefined or an Array but found ${commandInfo.args}`);
	else {
		for (let i = 0; i < commandInfo.args.length; i++) {
			const argInfo = commandInfo.args[i];
			if (typeof argInfo !== 'object') {
				logger(`Every element of args should be an object but found something else at index ${i}`);
			}
			else {
				const aLogger = prefixWrapper(`Argument ${i}, name=${argInfo.name}`, logger);
				if (typeof argInfo.name !== 'string')
					aLogger(`name should be a string but found ${argInfo.name}`);
				if (argInfo.prefix !== undefined && typeof argInfo.prefix !== 'string')
					aLogger(`prefix should either be undefined or a string but found ${argInfo.prefix}`);
			}
		}
	}
}
