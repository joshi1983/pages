...
const expectedResultDTO = [
	{"name":"javascript","code": "context.turtle.setPenSize(this.validateNumber(this.errorCaseCheck23(0)))"},
...
export function testConvertInstructionsToJavaScriptForMandelbrot(logger) {
	...
	for (let i = 0; i < expectedResult.length; i++) {
		...
		const codeIgnoreCase = 'context.turtle.setPenSize(this.validateNumber(this.errorCaseCheck';
		if (expected.code.indexOf(codeIgnoreCase) !== -1) {
			if (actual.code.indexOf(codeIgnoreCase) === -1)
				logger(`Expected to find ${codeIgnoreCase} in code for instruction ${i} but got code to be ${actual.code}`);

			// don't compare this as deeply because the function number is unrelated to the requirement being tested here.
			// If this case wasn't ignored, it would lead to too many noisy test failures.
		}
		else if (!DeepEquality.equals(actual, expected))
