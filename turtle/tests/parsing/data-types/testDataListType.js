...
function testGetIntersectionWith(logger) {
	const cases = [
		...
		{'subtypes': 'int(min=0)', 'other': 'colorlist', 'out': 'colorlist'},
		{'subtypes': 'int(min=-900)', 'other': 'colorlist', 'out': 'colorlist'},
		{'subtypes': 'int(min=256)', 'other': 'colorlist', 'out': null},
			// colorlist elements are supposed to be in the range 0..255.
			// a minimum of 256 puts all values out of range.
			// Therefore no intersection between list<int(min=256)> and colorlist.

		{'subtypes': 'num', 'other': 'alphacolorlist', 'out': 'alphacolorlist'},
		{'subtypes': 'list|num|string', 'other': 'colorlist', 'out': 'colorlist'},
		...
		{'subtypes': 'num(unfinite)', 'other': 'colorlist', 'out': null},
		{'subtypes': 'num(min=256)', 'other': 'colorlist', 'out': null},
		{'subtypes': 'num(min=infinity)', 'other': 'colorlist', 'out': null},
	...
	cases.forEach(function(caseInfo, index) {
		...
		if (caseInfo.out === null && result !== null)
			...
			if (result === null || result.toString() !== caseInfo.out)
...
function testToString(logger) {
	const cases = [
		...
		{'minLen': 1, 'subtypes': 'num(finite)', 'out': 'list<num(finite)>(minlen=1)'},
		{'minLen': 1, 'subtypes': 'num(finite,min=0)', 'out': 'list<num(finite,min=0)>(minlen=1)'},
