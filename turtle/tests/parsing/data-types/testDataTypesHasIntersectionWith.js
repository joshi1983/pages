...
export function testDataTypesHasIntersectionWith(logger) {
	const cases = [
		...
		{
			'inArgs': ['int(min=0)', 'int(min=10)'], 'out': true
		},
		{
			'inArgs': ['int(min=10)', 'int(min=0)'], 'out': true
		},
		{
			'inArgs': ['int', 'int(min=0)'], 'out': true
		},
		{
			'inArgs': ['int', 'num'], 'out': true
		},
		{
			'inArgs': ['int', 'num(min=0)'], 'out': true
		},
		{
			'inArgs': ['int', 'num(finite,max=10,min=0)'], 'out': true
		},
		{
			'inArgs': ['int(min=20)', 'num(finite,max=10,min=0)'], 'out': false
		},
		{
			'inArgs': ['int', 'num(finite)'], 'out': true
		},
		{
