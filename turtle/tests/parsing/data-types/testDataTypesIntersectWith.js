...
export function testDataTypesIntersectWith(logger) {
	const cases = [
		{'in': ['', 'num'], 'out': ''},
		{'in': ['num', ''], 'out': ''},
		{'in': ['num', 'num(finite)'], 'out': 'num(finite)'},
		{'in': ['int', 'num(finite)'], 'out': 'int'},
		...
		{'in': ['color', 'int(max=2,min=2)'], 'out': 'int(max=2,min=2)'},
		{'in': ['color', 'bool'], 'out': ''},
		...
		{'in': ['cproc', 'int'], 'out': ''},
		{'in': ['cproc:1', 'cproc'], 'out': 'cproc:1'},
		{'in': ['cproc:1', 'cproc:2'], 'out': ''},
		{'in': ['cproc', 'colorstring'], 'out': 'colorstring'},
		// the set of procedure names that match color names.
		// For example, red, green, blue could be both procedure names and color names.
		// Ideally, the result would exclude hex color codes but we're not quite there yet.
		// We didn't add a colorname type yet because the need hasn't become apparent through a lot of
		// practical use of WebLogo.
		...
		{'in': ['alphacolor', 'int(max=2,min=2)'], 'out': 'int(max=2,min=2)'},
	...
	cases.forEach(function(caseInfo, index) {
		...
		if (actualResultString !== caseInfo.out)
			logger(escapeHTML(`Case ${index}, in=${caseInfo.in.join(',')}, Expected ${caseInfo.out} but got ${actualResultString} `+
