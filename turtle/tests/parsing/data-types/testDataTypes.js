...
function testGetTypesCompatibleWithValue(logger) {
	const cases = [
		...
		{'in': [1, 2, 3, 4, 5], 'result': 'list<int(max=5,min=1)>(minlen=5)'},
		{'in': [1.2], 'result': 'list<num(finite,max=1.2,min=1.2)>(minlen=1)'},
		{'in': [1, 1.2], 'result': 'list<num(finite,max=1.2,min=1)>(minlen=2)'},
		{'in': [1, "hi"], 'result': 'list<int(max=1,min=1)|string>(minlen=2)'},
		{'in': [1.2, "hi"], 'result': 'list<num(finite,max=1.2,min=1.2)|string>(minlen=2)'},
		{'in': [1, "red"], 'result': 'list<colorstring|int(max=1,min=1)>(minlen=2)'},
	];
	cases.forEach(function(caseInfo, index) {
...
function testEquals(logger) {
	...
	const cases = [
		...
		['list<>', 'list', true],
		['list<>', 'list<>', true],
		['list<int>', 'list<num>', false],
		['list<int>', 'list<int>', true],
		['list<int(min=0)>', 'list<int(min=0)>', true],
		['list<int(min=0)>', 'list<int(min=10)>', false],
		['list<num>', 'list', false],
