...
function testGetTypesCompatibleWithValue(logger) {
	const cases = [
		...
		{'in': [1, 2, 3, 4, 5], 'result': 'list<int>'},
		{'in': [1.2], 'result': 'list<num>'},
		{'in': [1, 1.2], 'result': 'list<num>'},
		{'in': [1, "hi"], 'result': 'list<int|string>'},
		{'in': [1.2, "hi"], 'result': 'list<num|string>'},
		{'in': [1, "red"], 'result': 'list<colorstring|int>'},
	];
	cases.forEach(function(caseInfo, index) {
...
function testEquals(logger) {
	...
	cases.forEach(function(caseInfo) {
		...
		if (val1.equals(val2) !== caseInfo[2])
			logger(escapeHTML(`Expected ${caseInfo[2]} but got ${val1.equals(val2)} for new DataTypes(${val1.toString()}).equals(${val2.toString()})`));
	});
}
