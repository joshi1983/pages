...
function testGetLastToken(logger) {
	const cases = [
		{'code': 'fd 100', 'outToken': {
			'val': 100
		}},
		{
			'code': 'fd 100\npenDown\nprint :x',
	...
	cases.forEach(function(caseInfo, index) {
		...
		const writeOptimizedCachedParseTree = new WriteOptimizedCachedParseTree(cachedParseTree.root, proceduresMap);
		const allTokens = writeOptimizedCachedParseTree.getAllTokens();
		const expectedResult = findToken(caseInfo.outToken, allTokens, plogger);
		const result = writeOptimizedCachedParseTree.getLastToken();
		if (result.lineIndex !== expectedResult.lineIndex || result.val!== expectedResult.val || result.colIndex !== expectedResult.colIndex)
...
function testRemoveTokenForProcedure(logger) {
	const code = 'to p\nend';
	const cachedParseTree = getCachedParseTreeFromCode(code, logger);
	const wTree = new WriteOptimizedCachedParseTree(cachedParseTree.root, cachedParseTree.getProceduresMap());
	const toToken = wTree.getTokensByType(ParseTreeTokenType.PROCEDURE_START_KEYWORD)[0];
	if (typeof toToken !== 'object')
		logger(`toToken expected to be an object but found ${toToken}`);
	else {
		toToken.remove();
		wTree.tokenRemoved(toToken);
		const procedures = wTree.getProceduresMap();
		if (procedures.size !== 0)
			logger(`After removing toToken, 0 procedures expected but found ${procedures.size}`);
	}
}

function testRemoveTokensForProcedure(logger) {
	const code = 'to p\nend';
	const cachedParseTree = getCachedParseTreeFromCode(code, logger);
	const wTree = new WriteOptimizedCachedParseTree(cachedParseTree.root, cachedParseTree.getProceduresMap());
	const toToken = wTree.getTokensByType(ParseTreeTokenType.PROCEDURE_START_KEYWORD)[0];
...
export function testWriteOptimizedCachedParseTree(logger) {
	wrapAndCall([
		...
		testGetLastToken,
		testTokenRemoved,
		testRemoveTokenForProcedure,
		testRemoveTokensForProcedure,
		testTokenReplaced
	], logger);
};