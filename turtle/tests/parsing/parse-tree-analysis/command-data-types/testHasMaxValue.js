import { DataTypes } from
'../../../../modules/parsing/data-types/DataTypes.js';
import { hasMaxValue } from
'../../../../modules/parsing/parse-tree-analysis/command-data-types/hasMaxValue.js';
import { testInOutPairs } from '../../../helpers/testInOutPairs.js';
await DataTypes.asyncInit();

export function testHasMaxValue(logger) {
	const cases = [
		{'in': 'int', 'out': false},
		{'in': 'int(min=0)', 'out': false},
		{'in': 'int(max=10,min=0)', 'out': true},
		{'in': 'int(max=10)', 'out': true},
		{'in': 'num', 'out': false},
		{'in': 'num(finite)', 'out': false},
		{'in': 'num(finite,min=0)', 'out': false},
		{'in': 'num(finite,max=0)', 'out': true},
		{'in': 'num(finite,max=10,min=0)', 'out': true},
		{'in': 'num(finite,min=-10)', 'out': false},
		{'in': 'num(unfinite)', 'out': false},
		{'in': 'num(unfinite,min=-infinity)', 'out': false},
		{'in': 'num(unfinite,min=infinity)', 'out': false},
		{'in': 'num(unfinite,min=infinity)|string', 'out': false},
		{'in': 'num(finite)|string', 'out': false},
		{'in': 'bool', 'out': false},
		{'in': 'bool|num', 'out': false},
		{'in': 'bool|num(finite)', 'out': false},
		{'in': 'string', 'out': false},
		{'in': 'transparent', 'out': false},
		{'in': 'string|transparent', 'out': false},
		{'in': 'list|num', 'out': false},
	];
	testInOutPairs(cases, hasMaxValue, logger);
};