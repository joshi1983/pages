...
export function testValidateUnusedVariables(logger) {
	const cases = [
		...
		// No warning since x is global and the last assignment to the variable before the program halts.
		// The assigned value might still be used by running commands using the Commander.
...
make "gradient createLinearGradient [0 0] [0 100] :colorStops "pad`, 'warn': false},
		...
		{'code': 'to p\nlocalmake "p []\nqueue "p 1\nprint :p\nend', 'warn': false},
		{'code': 'to p :s\nfd :s\nend\nto q :s\nfd :s\nEnd', 'warn': false},
		{'code': 'to p :s\nend\nto q :s\nfd :s\nEnd', 'warn': true}, // parameter s in procedure p is never used.
...
print :x`, 'warn': true},
		...
		{'code': 'for [] []', 'warn': false},
		{'code': 'make "x 0 make "y 1 swap "x "y print :x print :y', 'warn': false},
		// Both x and y are used.  They're used by the swap command and then again by printing the results of the swap.

		{'code': 'to p\nlocalmake "x 0 localmake "y 1 swap "x "y print :x\nend', 'warn': true},
		// The value assigned to y by the swap is not used.

		{'code': 'to p\nlocalmake "x 0 localmake "y 1 swap "x "y print :x\nend', 'warn': true},
		// The value assigned to x by the swap is not used.
		{'code': `make "x 1
print :x
...
`, 'warn': false},
	...
	processValidationTestCases(cases, logger, validateUnusedVariables);
};