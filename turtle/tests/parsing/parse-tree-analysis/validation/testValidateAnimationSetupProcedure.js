...
function addThumbnailTimeCases(cases) {
	const tCases = [
		...
		{'t': 'penUp', 'error': true},
		{'t': 'animation.time', 'error': true},
		{'t': 'animation.timeRatio', 'error': true},
		{'t': 'animation.clampedTimeRatio', 'error': true},
		{'t': 'animation.duration', 'error': true}
	];
...
export function testValidateAnimationSetupProcedure(logger) {
	const cases = [
		...
		{'code': 'to animation.setup\noutput animation.timeRatio\nend\n', 'error': true, 'warn': false},
		{'code': 'to animation.setup\noutput animation.clampedTimeRatio\nend\n', 'error': true, 'warn': false},
		{'code': 'to animation.setup\noutput penUp\nend\n', 'error': true, 'warn': false},
		{'code': `to animation.setup
	localmake "result createPList
	setProperty "result "duration 10
	setProperty "result "unrecognizedProperty 10
	output :result
end`, 'error': false, 'warn': true}, // unrecognized property warning expected
		{'code': `to animation.setup
	output createPList
end`, 'error': true, 'warn': false}, // missing duration warning expected
	];
