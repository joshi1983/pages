...
export function testEvaluateTokenDataTypes(logger) {
	const cases = [
		...
		{'code': 'to p :x\nfd :x\nend', 'tokenTypeChecks': [
			{'val': 'x', 'hasParentVal': 'fd', 'type': ParseTreeTokenType.VARIABLE_READ, 'types': 'num'}
		]},
		{'code': 'to p :x\nprint -:x\nend\np 3\np9', 'tokenTypeChecks': [
			{'val': '-', 'types': 'int'}
		]},
...
p`, // not valid code but it still should not throw a JavaScript error.
	...
	cases.forEach(function(caseInfo, index) {
		...
		caseInfo.tokenTypeChecks.forEach(function(tokenTypeInfo) {
			const match = findToken(tokenTypeInfo, cachedParseTree, plogger);
			if (match !== undefined) {
				let result = tokenTypes.get(match);
				if (typeof result === 'string')
					plogger('Expected an instance of DataTypes but got a string: ' + result);
				...
				if (result !== tokenTypeInfo.types) {
					...
					plogger(escapeHTML(`Expected ${tokenTypeInfo.types} but got ${result} for check ${JSON.stringify(tokenTypeInfo)}`));
				}
			}
		});
	});
