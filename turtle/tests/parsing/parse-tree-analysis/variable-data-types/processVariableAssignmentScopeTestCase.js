...
export function processVariableAssignmentScopeTestCase(caseInfo, index, logger) {
	...
	caseInfo.scopeChecks.forEach(function(scopeCheck) {
		...
		if (variable === undefined)
			...
			if (scopeCheck.numScopes !== undefined && scopeCheck.numScopes !== variable.scopes.length)
				plogger(`Expected ${scopeCheck.numScopes} but found ${variable.scopes.length}`);
			...
			scopeCheck.scopes.forEach(function(scopeInfo, index) {
				...
				else if (scopeInfo.singleValueUndefined === true) {
					if (varScope.singleValue !== undefined)
						pplogger(`Expected singleValue to be undefined but got "${varScope.singleValue}"`);
				}
