...
p 4.4`, 'result': true},
	...
	cases.forEach(function(caseInfo) {
		...
		if (varReadToken === undefined)
			logger('Unexpectedly unable to find variable read token corresponding with "print :x" in code: ' + caseInfo.code);
		else {
...
function testParameters(logger) {
	const cases = [
		...
		{'code': 'to print\nend', 'params': []}, // not valid WebLogo code but we want to check that no JavaScript errors get thrown while looking at parameters.
		{'code': 'to p :x\nend', 'params': [['p', 'x']]},
		{'code': 'to p :X\nend\nto p2 :Y\nend', 'params': [['p', 'x'], ['p2', 'y']]},
	];
	cases.forEach(function(caseInfo) {
