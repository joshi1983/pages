...
export function testParseTreeToCodeWithComments(logger) {
	testGetTokenIndexes(prefixWrapper('testGetTokenIndexes', logger));
	testSimulateFixingWhileCondition(prefixWrapper('testSimulateFixingWhileCondition', logger));
	testUnchangedCode(prefixWrapper('testUnchangedCode', logger));
	testWithChangedToken(prefixWrapper('testWithChangedToken', logger));
};