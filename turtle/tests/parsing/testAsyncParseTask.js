...
function testCancel(logger) {
	...
	const task = new AsyncParseTask('fd 50', proceduresMap, resolve, reject);
	task.cancel();
}
...
function testFailedParse(logger) {
	...
	cases.forEach(function(caseInfo, index) {
		const code = caseInfo;
		new AsyncParseTask(code, new Map(), noop, noop);
	});
}
...
export function testAsyncParseTask(logger) {
	if (status.isFailing)
		...
		testCancel(prefixWrapper('testCancel', logger));
		testFailedParse(prefixWrapper('testFailedParse', logger));
		testTreeCompareWithSync(prefixWrapper('testTreeCompareWithSync', logger));
	}
};