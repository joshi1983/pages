...
function testMergeLinesWithVariousCases(logger) {
	const points = [new Vector3D(1, 2, 3), new Vector3D(4, 5, 6), new Vector3D(-1, -4, 3),
	new Vector3D(18, -23, -34)];
	const cases = [
		...
		{'indexes': [0, 1, 0, 2], 'isResultDefined': true},
		{'indexes': [1, 0, 1, 2], 'isResultDefined': true}
	];
...
// Look at the white line in the Proposed Montana Flags example is affected if this test fails.
function testMergeLineWiderThanLength(logger) {
	const points = [new Vector3D(0, 0, 0), new Vector3D(1, 0, 0), new Vector3D(5, 5, 0)];
	const cases = [
		{'indexes': [0, 1, 1, 2], 'isResultDefined': true}
	];
	cases.forEach(function(caseInfo) {
		const line1 = new LineSegmentShape(points[caseInfo.indexes[0]], points[caseInfo.indexes[1]]);
		const line2 = new LineSegmentShape(points[caseInfo.indexes[2]], points[caseInfo.indexes[3]]);
		const penWidth = 5;
		line1.style.setPenWidth(penWidth);
		line2.style.setPenWidth(penWidth);
		const result = mergeLines(line1, line2);
		if (caseInfo.isResultDefined !== (result !== undefined))
			logger(`Expected is result defined of ${caseInfo.isResultDefined} but got ${result !== undefined}`);
	});
...
export function testMergeLines(logger) {
	wrapAndCall([
		testMergeLinesWithVariousCases,
		testMergeLineWiderThanLength
	], logger);
};