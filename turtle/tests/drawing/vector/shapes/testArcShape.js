...
function testBasics(logger) {
	...
	if (a.rotationRadians !== 0)
		logger('Expected rotationRadians of 0 but got ' + a.rotationRadians);
	if (a.radius !== 10)
		logger('Expected radius of 10 but got ' + a.radius);
	if (a.angle !== Math.PI)
...
function testBoundingBox(logger) {
	...
	*/
	const cases = [
		...
		{'radius': 100, 'angle': Math.PI, 'rotation': 0, 'penWidth': 2, 'threshold': 1, 'lineCap': LineCap.Butt, 'result': {
			'max': {
				'x': 1,
				'y': 101,
				'z': 0.00001
		...
		{'radius': 100, 'angle': Math.PI, 'rotation': Math.PI / 2, 'penWidth': 100, 'lineCap': LineCap.Round, 'threshold': 0.0001, 'result': {
			'max': {
				'x': 150,
				'y': 150,
				'z': 0.000001
		...
		{'radius': 100, 'angle': Math.PI, 'rotation': Math.PI / 2, 'penWidth': 100, 'lineCap': LineCap.Butt, 'threshold': 0.0001, 'result': {
			'max': {
				'x': 150,
				'y': 150,
				'z': 0.000001
		...
		{'radius': 100, 'angle': Math.PI, 'rotation': Math.PI / 2, 'penWidth': 100, 'lineCap': LineCap.Square, 'threshold': 0.0001, 'result': {
			'max': {
				'x': 150,
				'y': 150,
				'z': 0.000001
	...
	cases.forEach(function(caseInfo, index) {
		...
		if (caseInfo.lineCap !== undefined) {
			if (!Number.isInteger(caseInfo.lineCap))
				throw new Error(`lineCap must either be undefined or be an integer.  Not ${caseInfo.lineCap}`);
