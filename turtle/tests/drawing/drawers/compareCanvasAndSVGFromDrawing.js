...
export async function compareCanvasAndSVGFromDrawing(drawing, threshold, pixelGap, logger) {
	...
	if (result > threshold) {
		logger(`Expected SVG and canvas drawers to create the same or very similar image but difference ratio found to be ${result}.  The tolerance threshold was ${threshold}.  Run the code with prototypes/svgToCanvasPrototype to visually compare the results and start troubleshooting.`);
	}
};