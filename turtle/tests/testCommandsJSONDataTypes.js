...
function testMixItemsDataTypesMatchMix(logger) {
	...
	if (mixInfo.returnTypes !== mixItemsInfo.returnTypes)
		...
		if (argInfo.listElementTypes !== mixInfo.args[1].types)
			logger(escapeHTML(`Expected mixItems first argument elementTypes to match types from args[1] of mix command.  ${argInfo.listElementTypes} != ${mixInfo.args[1].types}`));
	}
}
