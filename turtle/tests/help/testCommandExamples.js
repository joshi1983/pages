...
export async function testCommandExamples(logger) {
	const progressIndicator = new ProgressIndicator('testCommandExamples');
			...
			codeElements.forEach(function(codeElement) {
				...
				if (parseLogger.hasLoggedErrorsOrWarnings())
					...
					if (parseLogger.hasLoggedErrors())
						...
						const javaScriptCode = translateWebLogoToJS(s);
						if (typeof javaScriptCode !== 'string')
							plogger(`Expected translateWebLogoToJS to return a String(the JavaScript code) but found ${javaScriptCode}`);
					}
				}
			});
		}
		progressIndicator.setProgressRatio(i / commands.length);
		await sleep(100);
	}
