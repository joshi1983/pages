...
function processJSONTestCases(logger) {
	commands.filter(function(commandInfo) {
		...
		commandInfo.testCases.forEach(function(caseInfo) {
			...
			const keysToLeaveUndefined = ['inArgs', 'returnValues'];
			for (const key of keysToLeaveUndefined) {
				if (caseInfo[key] !== undefined)
					plogger(`Expected ${key} to be undefined.  Did you mean to use a slightly different key?`);
			}
			try {
				...
				if (commandGroups.has(commandInfo.commandGroup)) {
					...
					const methodName = getMethodNameForCommand(commandInfo.primaryName);
