...
export async function testSimulateAllSupportedExports(logger) {
	...
	for (let i = 0; i < webLogoExamplesContent.length; i++) {
		...
		const plogger = prefixWrapper(`Case ${i}`, logger);
		try {
			...
			console.error(e);
			plogger(`Exception thrown while simulating exports for WebLogo example ${i}.  The error message is ${exceptionToString(e)}.  The WebLogo code is ${code}`);
