...
]).concat(processingExamples);
// Until a processing translator is made, processingExamples should be classified as WebLogo.
...
function testWebLogoCases(logger) {
	webLogoExampleCases.forEach(function(content, index) {
		...
		if (nonWebLogoTranslators.has(translator)) {
			...
			logger(`WebLogo code should not get translated but codeToTranslator returned ${translator}, which has name ${translatorName} for WebLogo example ${index} which has the code: ${content}`);
		}
	});
};
