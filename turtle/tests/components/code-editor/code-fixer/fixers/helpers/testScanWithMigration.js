...
function compareTokens(tokenInfo, resToken, logger) {
	let s;
	...
	if (s !== undefined) {
		if (resToken.s !== s)
			logger(`s expected to be "${s}" but found "${resToken.s}"`);
	}
}
	...
	cases.forEach(function(caseInfo, index) {
		...
		if (comments.length !== caseInfo.tokens.length)
			...
			for (let i = 0; i < comments.length; i++) {
				...
				compareTokens(tokenInfo, resToken, prefixWrapper(`Token ${i}`, plogger));
			}
		}
	});
}
	...
	cases.forEach(function(caseInfo, index) {
		...
		if (!(tokens instanceof Array))
			...
			for (let i = 0; i < tokens.length; i++) {
				...
				compareTokens(expectedInfo, token, prefixWrapper(`Token ${i}, s=${token.s}`, plogger));
			}
		}
	});
}
