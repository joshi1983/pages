...
export function testFixDynamicScopes(logger) {
	const cases = [
		...
		{'code': 'to p\nprint :x\nend\nto q\nlocalmake "x 3\np\nend',
		'to': 'to p\n:x print :x\nend\nto q\nlocalmake "x 3\np\n:x end', 'logged': true},
		{'code': 'to p\nprint :some_long_name\nend\nto q\nlocalmake "some_long_name 3\np\nend',
		'to': 'to p\n :some_long_name print :some_long_name\nend\nto q\nlocalmake "some_long_name 3\np\n:some_long_name end', 'logged': true},
		{'code': `to draw_all_flags :ratio


repeat count :flags_dict [
localmake "i item repcount :flags_dict
...
to main
draw_all_flags false output "Ready
...
]`, 'to': `to draw_all_flags :ratio


 :flags_dict repeat count :flags_dict [
localmake "i item repcount :flags_dict
...
to main
:flags_dict draw_all_flags false  :flags_dict

output
 "Ready end if :__name__ = "__main__ [
