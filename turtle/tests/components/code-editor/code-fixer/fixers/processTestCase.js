...
function checkCacheIsNotStale(cachedTree, logger) {
	...
	if (allTokensFromCache.length !== actualTokens.length)
		logger(`Expected getAllTokens() to return ${actualTokens.length} but got ${allTokensFromCache.length}`);
	else {
...
export function processTestCase(caseInfo, fixerFunction, logger) {
	...
	if (fixedCode !== caseInfo.to) {
		...
		expectedDiv.innerText = caseInfo.to;
		let span = document.createElement('span');
		span.innerText = 'Fixed code expected to be';
		div.appendChild(span);
		div.appendChild(expectedDiv);
		span = document.createElement('span');
		span.innerText = ' but got ';
		div.appendChild(span);
		const fixedCodeDiv = document.createElement('div');
		fixedCodeDiv.innerText = fixedCode;
		div.appendChild(fixedCodeDiv);
		const detailsDiv = document.createElement('div');
		span = document.createElement('span');
		span.innerText = 'Comparison details:';
		div.appendChild(span);
		detailsDiv.innerText = getStringComparisonDetails(caseInfo.to, fixedCode);
	...
	validateWriteOptimizedCachedParseTree(treeArg, prefixWrapper('validateWriteOptimizedCachedParseTree', logger));
	if (caseInfo.compareTreeSettings !== undefined) {
