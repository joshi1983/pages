...
export function testFixCode(logger) {
	const cases = [
		...
		{'in': 'Define "box [ [size] [ repeat 4 [forward :size right 90] ] ]', 'out': 'to box  :size \n repeat 4 [forward :size right 90]  end'},
	...
	cases.forEach(function(caseInfo) {
		const parseLogger = new TestParseLogger(logger, caseInfo.in);
		...
		if (result !== caseInfo.out) {
			...
			logger(`Expected "${caseInfo.out}" but got "${result}".  String comparison details: ${comparisonDetails}`);
		}
	});
};