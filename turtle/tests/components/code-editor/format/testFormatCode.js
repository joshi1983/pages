...
export function testFormatCode(logger) {
	const cases = [
		{'in': '', 'out': ''},
		{'in': 'undefinedProcedure', 'out': 'undefinedProcedure'},
		// When there is only 1 use of a procedure name, just keep it in the original case.
		...
		{'in': 'print \'This program draws shapes based on the number you enter in a uniform pattern.\'',
			'out': 'print \'This program draws shapes based on the number you enter in a uniform pattern.\''},
		{'in': `to p
	...
	parallelogram :churchWidth - :roofWidth :roofSlantedHeight -:roofAngle
		...
		{'in': 'clamp :percentage -100 100', 'out': 'clamp :percentage -100 100'},
		{'in': 'make "percentage clamp :percentage -100 100', 'out': 'make "percentage clamp :percentage -100 100'},
	];
	testInOutPairs(cases, formatCode, logger);
};