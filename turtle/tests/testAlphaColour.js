...
function testGetRGBAExpression(logger) {
	const cases = [
		{'in': '#1234', 'result': 'rgba(34,51,68, 0.06666666666666667)'}
	];
	cases.forEach(function(caseInfo, index) {
		const plogger = prefixWrapper(`Case ${index}, in=${caseInfo.in}`, logger);
		const c = new AlphaColour(caseInfo.in);
		const result = AlphaColour.getRGBAExpression(c);
...
function testIsValidAlphaColourString(logger) {
	...
	cases.forEach(function(caseInfo, index) {
		const result = AlphaColour.isValidAlphaColourString(caseInfo.in);
		const plogger = prefixWrapper(`Case ${index}, in = "${caseInfo.in}"`, logger);
		if (result !== caseInfo.out)
			plogger(`Expected "${caseInfo.out}" but got "${result}"`);
	});
...
function testIsValidRGBArray(logger) {
	...
	cases.forEach(function(caseInfo, index) {
		const result = AlphaColour.isValidRGBArray(caseInfo.in);
...
function testTo8DigitHTMLCode(logger) {
	...
	cases.forEach(function(caseInfo, index) {
		const c = new AlphaColour(caseInfo.in);
		const result = c.to8DigitHTMLCode();
		const plogger = prefixWrapper(`Case ${index}`, logger);
		if (result !== caseInfo.out)
			plogger(`Expected "${caseInfo.out}" but got "${result}"`);
	});
...
export function testAlphaColour(logger) {
	testAlphaColourConstructor(prefixWrapper('testAlphaColourConstructor', logger));
	testCanBeInterprettedAsAlphaColour(prefixWrapper('testCanBeInterprettedAsAlphaColour', logger));
	testEquals(prefixWrapper('testEquals', logger));
	testGetRGBAExpression(prefixWrapper('testGetRGBAExpression', logger));
	testIsValidAlphaColourString(prefixWrapper('testIsValidAlphaColourString', logger));
	testIsValidRGBArray(prefixWrapper('testIsValidRGBArray', logger));
	testTo8DigitHTMLCode(prefixWrapper('testTo8DigitHTMLCode', logger));
	testToARGBArray(prefixWrapper('testToARGBArray', logger));
	testVariousMethods(prefixWrapper('testVariousMethods', logger));
}