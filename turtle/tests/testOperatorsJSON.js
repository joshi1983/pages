	...
	if (!(operators instanceof Array))
		...
		operators.forEach(function(info, index) {
			...
			if (typeof info !== 'object')
				...
				if (!(info.seeAlsoCommands instanceof Array))
					plogger('Expected seeAlsoCommands to be an Array but got ' + info.seeAlsoCommands);
				else {
					info.seeAlsoCommands.forEach(function(commandPrimaryName) {
						if (typeof commandPrimaryName !== 'string')
							plogger(`seeAlsoCommands must contain only strings but instead found: ${commandPrimaryName}`);
						else {
							const commandInfo = Command.getCommandInfo(commandPrimaryName);
							if (commandInfo === undefined)
								plogger(`Unable to find seeAlsoCommands command named ${commandPrimaryName}`);
							else if (commandInfo.primaryName !== commandPrimaryName)
								plogger(`Command info found from ${commandPrimaryName} but primaryName is ${commandInfo.primaryName}. Don't use an alternative command name or a different case.`);
						}
					});
				}
