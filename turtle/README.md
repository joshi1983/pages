...
- File -> New removes all code from the code editor.  
This could be very annoying for someone who spent lots of time writing code and clicked "New" by accident.
If there is more than 2 or 3 non-empty lines of code in the code editor, show a dialog to confirm that removing the # lines of code from the code editor is what he/she wants to do.
...
Debugger features:
	- add a way to maximize the Debugger.
	- make it possible to collapse the global variables section.
	- make it possible to copy a variable's value as JSON.
...
QBasic:
	- DONE: add validator for SELECT tokens checking that they all end with END_SELECT.
	- DONE: fix parameter unused warnings for qbDraw procedure.
	- DONE: add support for varptr.
	- adjust autofix to run some extra functions such as fixDynamic scope when a recent paste happened.
	- test that fixDynamicScopes works with madlibs FMSLogo example at: https://fmslogo.sourceforge.io/workshop/madlibs.shtml
