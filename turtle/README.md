...
- make a prototype for drawing arbitrary gif animation frames to canvas using https://github.com/buzzfeed/libgif-js
...
DONE: - look up Edinborough Logo.
	- Logo Programming for the IBM PC says on page 6 that it is easier to learn than the MIT verasion of Logo.
...
- optimize SVG exporting if a rectular path is found.
	- use a rect tag name.
- optimize SVG exporting if a rounded rectangle path is found.  That is a rect with circular arcs on all 4 corners of equal radii.
...
- Add part to Reset & Test button for live redrawing every time there are no code quality errors.
...
- '#' is used to start comments in several languages.  If an error message is for a LEAF token starting with '#', mention that ';' is how to do one line comments in WebLogo.
