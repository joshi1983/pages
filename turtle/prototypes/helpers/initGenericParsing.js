...
function supportTabs(textarea, latestRefreshOutputs) {
...
export function initGenericParsing(ParseTreeTokenType, parse, code, validateTree, validateCode, translate) {
	...
	function init() {
		...
		latestRefreshOutputs = function() {
			refreshOutputsForCode(textarea.value);
		};
		supportTabs(textarea, latestRefreshOutputs);
		textarea.addEventListener('input', latestRefreshOutputs);
		textarea.value = code;
		latestRefreshOutputs();
	}
	...
	return {
		'refreshOutputs': latestRefreshOutputs
	};
}