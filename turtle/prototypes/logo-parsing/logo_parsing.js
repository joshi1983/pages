...
let dataTypesContainer, valueContainer;
...
function updateDataTypes(types) {
	...
	if (types === undefined || types === null) {
		...
		s = types;
	else if (types instanceof DataTypes)
		s = DataTypes.stringify(types.types);
	else
		s = DataTypes.stringify(types);
	dataTypesContainer.innerText = s;
}

function updateEvaluatedValue(value) {
	valueContainer.innerText = valueToLiteralCode(value);
}
...
function init() {
	...
	valueContainer = document.getElementById('evaluated-value');
	function refreshCodeOutputs() {
		...
		if (tree !== undefined) {
			...
			const tokenToValue = cachedParseTree.getTokenValues();
			for (const node of parseTreeContainer.querySelectorAll('.parse-tree-token-heading')) {
				if (node instanceof Element) {
					...
					if (token !== undefined) {
						...
						const evaluatedValue = tokenToValue.get(token);
						node.addEventListener('mouseover', function() {
							updateDataTypes(assignedTypes);
							updateEvaluatedValue(evaluatedValue);
						});
					}
				}
			}
