import { LogoParser } from
'../../modules/parsing/LogoParser.js';

// Some of the following imports must wait
// to prevent an error like: "isInstructionList is not a function"
await LogoParser.asyncInit();

import { codeToTranslator } from
'../../modules/components/code-editor/code-fixer/codeToTranslator.js';
import { asyncInit as asyncInitCreateParameterizedGroups } from '../../modules/parsing/createParameterizedGroups.js';
import { isLikely0L } from
'../../modules/parsing/other-languages/l-systems/0L/isLikely0L.js';
import { isLikelyAda } from
'../../modules/parsing/other-languages/ada/isLikelyAda.js';
import { isLikelyAmosBasic } from
'../../modules/parsing/other-languages/basic/amos-basic/isLikelyAmosBasic.js';
import { isLikelyAnsiBasic } from
'../../modules/parsing/other-languages/basic/ansi-basic/isLikelyAnsiBasic.js';
import { isLikelyApex } from
'../../modules/parsing/other-languages/apex/isLikelyApex.js';
import { isLikelyAppleSoftBasic } from
'../../modules/parsing/other-languages/basic/applesoft-basic/isLikelyAppleSoftBasic.js';
import { isLikelyArc } from
'../../modules/parsing/other-languages/lisp/arc/isLikelyArc.js';
import { isLikelyASMTurtle } from
'../../modules/parsing/other-languages/asm-turtle/isLikelyASMTurtle.js';
import { isLikelyAtariTurboBasicXL } from
'../../modules/parsing/other-languages/basic/atari-turbo-basic-xl/isLikelyAtariTurboBasicXL.js';
import { isLikelyBasic } from
'../../modules/parsing/other-languages/basic/isLikelyBasic.js';
import { isLikelyBasic256 } from
'../../modules/parsing/other-languages/basic/basic-256/isLikelyBasic256.js';
import { isLikelyBasicFusion } from
'../../modules/parsing/other-languages/basic/basic-fusion/isLikelyBasicFusion.js';
import { isLikelyBasilBasic } from
'../../modules/parsing/other-languages/basic/basil/isLikelyBasilBasic.js';
import { isLikelyBatch } from
'../../modules/parsing/other-languages/batch/isLikelyBatch.js';
import { isLikelyBazzBasic } from
'../../modules/parsing/other-languages/basic/bazz-basic/isLikelyBazzBasic.js';
import { isLikelyBBCBasic } from
'../../modules/parsing/other-languages/basic/bbc-basic/isLikelyBBCBasic.js';
import { isLikelyBCPL } from
'../../modules/parsing/other-languages/bcpl/isLikelyBCPL.js';
import { isLikelyCanvas2D } from
'../../modules/components/code-editor/code-fixer/fixers/canvas-2d/isLikelyCanvas2D.js';
import { isLikelyCGJennings } from
'../../modules/parsing/other-languages/l-systems/cgjennings/isLikelyCGJennings.js';
import { isLikelyCheerfulNetherlandsLogo } from
'../../modules/components/code-editor/code-fixer/fixers/cheerful-netherlands-logo/isLikelyCheerfulNetherlandsLogo.js';
import { isLikelyCobol } from
'../../modules/parsing/other-languages/cobol/isLikelyCobol.js';
import { isLikelyCodeHeartTurtleScript } from
'../../modules/components/code-editor/code-fixer/fixers/codeheart-turtlescript/isLikelyCodeHeartTurtleScript.js';
import { isLikelyCommodoreBasic } from
'../../modules/parsing/other-languages/basic/commodore-basic/isLikelyCommodoreBasic.js';
import { isLikelyCSS } from
'../../modules/parsing/other-languages/css/isLikelyCSS.js';
import { isLikelyFMSLogo } from
'../../modules/parsing/other-languages/fms-logo/isLikelyFMSLogo.js';
import { isLikelyForth } from
'../../modules/parsing/other-languages/forth/isLikelyForth.js';
import { isLikelyFractInt } from
'../../modules/parsing/other-languages/l-systems/fractint/isLikelyFractInt.js';
import { isLikelyGermanLogo } from
'../../modules/components/code-editor/code-fixer/fixers/german-logo/isLikelyGermanLogo.js';
import { isLikelyGleam } from
'../../modules/parsing/other-languages/gleam/isLikelyGleam.js';
import { isLikelyGLSL } from
'../../modules/parsing/other-languages/shaders/glsl/isLikelyGLSL.js';
import { isLikelyGroovy } from
'../../modules/parsing/other-languages/groovy/isLikelyGroovy.js';
import { isLikelyHaskell } from
'../../modules/parsing/other-languages/haskell/isLikelyHaskell.js';
import { isLikelyHLSL } from
'../../modules/parsing/other-languages/shaders/hlsl/isLikelyHLSL.js';
import { isLikelyHPGL } from
'../../modules/parsing/other-languages/hp-gl/isLikelyHPGL.js';
import { isLikelyHolyC } from
'../../modules/parsing/other-languages/holy-c/isLikelyHolyC.js';
import { isLikelyJavaScript } from
'../../modules/parsing/other-languages/js-parsing/isLikelyJavaScript.js';
import { isLikelyJavaScriptProcessing } from
'../../modules/parsing/other-languages/processing/js-processing/isLikelyJavaScriptProcessing.js';
import { isLikelyKojo } from
'../../modules/parsing/other-languages/kojo/isLikelyKojo.js';
import { isLikelyKotlin } from
'../../modules/parsing/other-languages/kotlin/isLikelyKotlin.js';
import { isLikelyKTurtle } from
'../../modules/parsing/other-languages/kturtle/isLikelyKTurtle.js';
import { isLikelyLogo3D } from
'../../modules/components/code-editor/code-fixer/fixers/logo-3d/isLikelyLogo3D.js';
import { isLikelyLogoInterpreter } from
'../../modules/components/code-editor/code-fixer/fixers/logo-interpreter/isLikelyLogoInterpreter.js';
import { isLikelyLua } from
'../../modules/parsing/other-languages/lua/isLikelyLua.js';
import { isLikelyMatLab } from
'../../modules/parsing/other-languages/math/matlab/isLikelyMatLab.js';
import { isLikelyMetaLanguage } from
'../../modules/parsing/other-languages/ml/isLikelyMetaLanguage.js';
import { isLikelyMicroABasic } from
'../../modules/parsing/other-languages/basic/micro-a/isLikelyMicroABasic.js';
import { isLikelyModula2 } from
'../../modules/parsing/other-languages/modula-2/isLikelyModula2.js';
import { isLikelyOcaml } from
'../../modules/parsing/other-languages/ml/ocaml/isLikelyOcaml.js';
import { isLikelyPapert } from
'../../modules/components/code-editor/code-fixer/fixers/papert/isLikelyPapert.js';
import { isLikelyPBasic } from
'../../modules/parsing/other-languages/basic/pbasic/isLikelyPBasic.js';
import { isLikelyPHP } from
'../../modules/parsing/other-languages/php/isLikelyPHP.js';
import { isLikelyPitrifiedGoTurtle } from
'../../modules/parsing/other-languages/pitrified-go-turtle/isLikelyPitrifiedGoTurtle.js';
import { isLikelyPlayBasic } from
'../../modules/parsing/other-languages/basic/playbasic/isLikelyPlayBasic.js';
import { isLikelyPovRay } from
'../../modules/parsing/other-languages/pov-ray/isLikelyPovRay.js';
import { isLikelyProcessing } from
'../../modules/parsing/other-languages/processing/isLikelyProcessing.js';
import { isLikelyProlog } from
'../../modules/parsing/other-languages/prolog/isLikelyProlog.js';
import { isLikelyPythonCode } from
'../../modules/parsing/other-languages/python/isLikelyPythonCode.js';
import { isLikelyQBasic } from
'../../modules/parsing/other-languages/basic/qbasic/isLikelyQBasic.js';
import { isLikelyRuby } from
'../../modules/parsing/other-languages/ruby/isLikelyRuby.js';
import { isLikelyRustTurtle } from
'../../modules/parsing/other-languages/rust-turtle/isLikelyRustTurtle.js';
import { isLikelySeaTurtle } from
'../../modules/components/code-editor/code-fixer/fixers/sea-turtle/isLikelySeaTurtle.js';
import { isLikelySinclairBasic } from
'../../modules/parsing/other-languages/basic/sinclair-basic/isLikelySinclairBasic.js';
import { isLikelySmallVisualBasic } from
'../../modules/parsing/other-languages/basic/small-visual-basic/isLikelySmallVisualBasic.js';
import { isLikelySonicWebTurtle } from
'../../modules/parsing/other-languages/sonic-webturtle/isLikelySonicWebTurtle.js';
import { isLikelySugarLabsTurtleBlocks } from
'../../modules/parsing/other-languages/sugarlabs-turtle-blocks/isLikelySugarLabsTurtleBlocks.js';
import { isLikelySuperLogo } from
'../../modules/components/code-editor/code-fixer/fixers/super-logo/isLikelySuperLogo.js';
import { isLikelySwift } from
'../../modules/parsing/other-languages/swift/isLikelySwift.js';
import { isLikelyTektronix405XBasic } from
'../../modules/parsing/other-languages/basic/tektronix-405x-basic/isLikelyTektronix405XBasic.js';
import { isLikelyTerrapin } from
'../../modules/components/code-editor/code-fixer/fixers/terrapin/isLikelyTerrapin.js';
import { isLikelyTexasInstruments99_4a } from
'../../modules/parsing/other-languages/basic/texas-instruments-99-4a/isLikelyTexasInstruments99_4a.js';
import { isLikelyTRS80Basic } from
'../../modules/parsing/other-languages/basic/trs-80-basic/isLikelyTRS80Basic.js';
import { isLikelyTrueBasic } from
'../../modules/parsing/other-languages/basic/true-basic/isLikelyTrueBasic.js';
import { isLikelyTuring } from
'../../modules/parsing/other-languages/pascal/turing/isLikelyTuring.js';
import { isLikelyTurtleGraphicsFun } from
'../../modules/parsing/other-languages/turtle-graphics-fun/isLikelyTurtleGraphicsFun.js';
import { isLikelyTurtleToyNet } from
'../../modules/parsing/other-languages/turtletoy-net/isLikelyTurtleToyNet.js';
import { isLikelyWGSL } from
'../../modules/parsing/other-languages/shaders/wgsl/isLikelyWGSL.js';
import { isLikelyZig } from
'../../modules/parsing/other-languages/zig/isLikelyZig.js';
import { logo3DToWebLogo } from
'../../modules/components/code-editor/code-fixer/fixers/logo-3d/logo3DToWebLogo.js';
import { logoInterpreterToWebLogo } from
'../../modules/components/code-editor/code-fixer/fixers/logo-interpreter/logoInterpreterToWebLogo.js';
import { papertToWebLogo } from
'../../modules/components/code-editor/code-fixer/fixers/papert/papertToWebLogo.js';
import { ready } from '../../modules/ready.js';
import { terrapinToWebLogo } from
'../../modules/components/code-editor/code-fixer/fixers/terrapin/terrapinToWebLogo.js';
import { translateAppleSoftBasicToWebLogo } from
'../../modules/parsing/other-languages/basic/applesoft-basic/translation-to-weblogo/translateAppleSoftBasicToWebLogo.js';
import { translate as translateASMTurtle } from
'../../modules/parsing/other-languages/asm-turtle/translation-to-weblogo/translate.js';
import { translateCheerfulToWebLogo } from
'../../modules/components/code-editor/code-fixer/fixers/cheerful-netherlands-logo/translateCheerfulToWebLogo.js';
import { translate as translateFMSLogo } from
'../../modules/parsing/other-languages/fms-logo/translation-to-weblogo/translate.js';
import { translate as translateKTurtle } from
'../../modules/parsing/other-languages/kturtle/translation-to-weblogo/translate.js';
import { translate as translatePovRay } from
'../../modules/parsing/other-languages/pov-ray/translation-to-weblogo/translate.js';
import { translateProcessingToWebLogo } from
'../../modules/parsing/other-languages/processing/translation-to-weblogo/translateProcessingToWebLogo.js';
import { translateQBASICToWebLogo } from
'../../modules/parsing/other-languages/basic/qbasic/translation-to-weblogo/translateQBASICToWebLogo.js';
import { translate as translateSonicWebTurtle } from
'../../modules/parsing/other-languages/sonic-webturtle/translation-to-weblogo/translate.js';
import { translateToWebLogo as translateCanvas2DToWebLogo } from
'../../modules/components/code-editor/code-fixer/fixers/canvas-2d/translateToWebLogo.js';
import { translateToWebLogo as translateCodeHeartTurtleScriptToWebLogo } from
'../../modules/components/code-editor/code-fixer/fixers/codeheart-turtlescript/translateToWebLogo.js';
import { translateBBCBasicToWebLogo } from
'../../modules/parsing/other-languages/basic/bbc-basic/translation-to-weblogo/translateBBCBasicToWebLogo.js';
import { translateTurtleBlocksToWebLogo } from
'../../modules/parsing/other-languages/sugarlabs-turtle-blocks/translation-to-weblogo/translateTurtleBlocksToWebLogo.js';
import { newTranslatePythonCodeToWebLogo } from
'../../modules/parsing/other-languages/python/newTranslatePythonCodeToWebLogo.js';

const initialCode = `100 PAGE
110 SET DEGREES
120 PRINT "MULTIPLIER ";
130 INPUT M
140 MOVE 65,50
150 FOR A=3 TO 7200 STEP 3
160 R0=2+A/160
170 R=SIN(M*A)
180 X=65+R0*R*COS(A)
190 Y=50+R0*R*SIN(A)
200 DRAW X,Y
210 NEXT A
220 HOME
230 PRINT
240 PRINT`;
let codeElement, outputElement;
const translatorNameMap = new Map([
	[logo3DToWebLogo, 'Logo3D'],
	[logoInterpreterToWebLogo, 'Logo Interpreter'],
	[papertToWebLogo, 'Papert'],
	[terrapinToWebLogo, 'Terrapin'],
	[translateAppleSoftBasicToWebLogo, 'AppleSoft BASIC'],
	[translateASMTurtle, 'ASM Turtle(Josh Greig\'s educational assembly language with turtle graphics)'],
	[translateCheerfulToWebLogo, 'Cheerful'],
	[translateFMSLogo, 'FMSLogo'],
	[translateKTurtle, 'KTurtle'],
	[translatePovRay, 'POV Ray Script'],
	[translateProcessingToWebLogo, 'Processing'],
	[translateQBASICToWebLogo, 'QBASIC'],
	[translateSonicWebTurtle, 'Sonic WebTurtle'],
	[translateCanvas2DToWebLogo, 'Canvas2D'],
	[translateCodeHeartTurtleScriptToWebLogo, 'CodeHeart TurtleScript'],
	[translateBBCBasicToWebLogo, 'BBC BASIC'],
	[translateTurtleBlocksToWebLogo, 'TurtleBlocks'],
	[newTranslatePythonCodeToWebLogo, 'Python']
]);
const isLikelyNamePairs = [
	[isLikely0L, '0L L-Systems'],
	[isLikelyAda, 'Ada'],
	[isLikelyAmosBasic, 'AMOS BASIC'],
	[isLikelyAnsiBasic, 'ANSI BASIC'],
	[isLikelyApex, 'Apex (from Salesforce)'],
	[isLikelyAppleSoftBasic, 'AppleSoft BASIC'],
	[isLikelyArc, 'Arc (a dialect of Lisp by Paul Graham and Robert Morris)'],
	[isLikelyASMTurtle, 'ASM Turtle'],
	[isLikelyAtariTurboBasicXL, 'Atari Turbo Basic XL'],
	[isLikelyBasic, 'Basic in general(no specific dialect)'],
	[isLikelyBasic256, 'Basic 256'],
	[isLikelyBasicFusion, 'Basic Fusion ( https://basicfusion.org )'],
	[isLikelyBasilBasic, 'Basil Basic'],
	[isLikelyBazzBasic, 'Bazz Basic'],
	[isLikelyBatch, 'Batch'],
	[isLikelyBBCBasic, 'BBC BASIC'],
	[isLikelyBCPL, 'BCPL (Basic Combined Programming Language)'],
	[isLikelyCanvas2D, 'Canvas2D'],
	[isLikelyCGJennings, 'CGJennings L-Systems'],
	[isLikelyCheerfulNetherlandsLogo, 'Cheerful Netherlands'],
	[isLikelyCobol, 'Cobol'],
	[isLikelyCodeHeartTurtleScript, 'CodeHeart TurtleScript'],
	[isLikelyCommodoreBasic, 'Commodore Basic'],
	[isLikelyCSS, 'CSS'],
	[isLikelyFMSLogo, 'FMS Logo'],
	[isLikelyForth, 'Forth ( a stack-oriented programming language )'],
	[isLikelyFractInt, 'Fractint'],
	[isLikelyGermanLogo, 'German Logo(Commodore 64 or Apple II)'],
	[isLikelyGleam, 'Gleam'],
	[isLikelyGLSL, 'GLSL(OpenGL Shader Language)'],
	[isLikelyGroovy, 'Groovy'],
	[isLikelyHaskell, 'Haskell'],
	[isLikelyHLSL, 'HLSL(High level shading language from Microsoft DirectX)'],
	[isLikelyHPGL, 'HP-GL'],
	[isLikelyHolyC, 'Holy C'],
	[isLikelyJavaScript, 'JavaScript in general(no specific graphics library)'],
	[isLikelyJavaScriptProcessing, 'JS Processing(ie. P5.js)'],
	[isLikelyKojo, 'Kojo(basically Scala + turtle graphics)'],
	[isLikelyKotlin, 'Kotlin'],
	[isLikelyKTurtle, 'KTurtle'],
	[isLikelyLogo3D, 'Logo3D'],
	[isLikelyLogoInterpreter, 'Logo Interpreter'],
	[isLikelyLua, 'Lua'],
	[isLikelyMatLab, 'MatLab'],
	[isLikelyMetaLanguage, 'ML (Meta Language)'],
	[isLikelyMicroABasic, 'Micro(A) BASIC'],
	[isLikelyModula2, 'Modula-2'],
	[isLikelyOcaml, 'Ocaml'],
	[isLikelyPapert, 'Papert'],
	[isLikelyPBasic, 'pBasic (a dialect of Basic by John Roland Penner)'],
	[isLikelyPHP, 'PHP (language mostly for programming web servers)'],
	[isLikelyPitrifiedGoTurtle, 'Pitrified Go Turtle'],
	[isLikelyPlayBasic, 'PlayBasic'],
	[isLikelyPovRay, 'POV Ray'],
	[isLikelyProcessing, 'Processing'],
	[isLikelyProlog, 'Prolog'],
	[isLikelyPythonCode, 'Python'],
	[isLikelyQBasic, 'QBASIC'],
	[isLikelyRuby, 'Ruby'],
	[isLikelyRustTurtle, 'Rust Turtle'],
	[isLikelySeaTurtle, 'SeaTurtle( https://oaksnow.com/seaturtle/ )'],
	[isLikelySinclairBasic, 'Sinclair BASIC'],
	[isLikelySmallVisualBasic, 'Small Visual Basic'],
	[isLikelySonicWebTurtle, 'Sonic Web Turtle'],
	[isLikelySugarLabsTurtleBlocks, 'Sugar Labs Turtle Blocks'],
	[isLikelySuperLogo, 'Super Logo'],
	[isLikelySwift, 'Swift'],
	[isLikelyTektronix405XBasic, 'Tektronix 405X BASIC'],
	[isLikelyTerrapin, 'Terrapin'],
	[isLikelyTexasInstruments99_4a, 'Texas Instruments 99/4A BASIC'],
	[isLikelyTRS80Basic, 'TRS-80(Tandy\'s) BASIC'],
	[isLikelyTrueBasic, 'TRUE Basic'],
	[isLikelyTuring, 'Turing'],
	[isLikelyTurtleGraphicsFun, 'Turtle Graphics Fun ( https://www.turtlegraphics.fun )'],
	[isLikelyTurtleToyNet, 'TurtleToy.net'],
	[isLikelyWGSL, 'WGSL (Shader language used with WebGPU)'],
	[isLikelyZig, 'Zig']
];

function translatorToLanguageName(translator) {
	if (translator instanceof Array)
		translator = translator[0];
	const result = translatorNameMap.get(translator);
	if (result === undefined)
		return `Unknown Language(${translator.constructor.name})`;
	else
		return result;
}

function addPairElements(container, pairs, cssClasses) {
	for (const pair of pairs) {
		const div = document.createElement('div');
		if (cssClasses !== undefined && cssClasses.length !== 0) {
			const iconSpan = document.createElement('span');
			for (const cssClass of cssClasses) {
				iconSpan.classList.add(cssClass);
			}
			div.appendChild(iconSpan);
		}
		const nameSpan = document.createElement('span');
		nameSpan.classList.add('name');
		nameSpan.innerText = pair[1];
		div.appendChild(nameSpan);
		container.appendChild(div);
	}
}

function updateClassificationReport() {
	const code = codeElement.value;
	const translator = codeToTranslator(code);
	outputElement.innerText = '';
	const nameDiv = document.createElement('div');
	const nameSpan = document.createElement('span');
	outputElement.innerText = '';
	nameSpan.innerText = translatorToLanguageName(translator);
	nameDiv.appendChild(nameSpan);
	outputElement.appendChild(nameDiv);
	const likelies = isLikelyNamePairs.filter((pair) => pair[0](code));
	const unlikelies = isLikelyNamePairs.filter((pair) => !pair[0](code));
	const likelyContainer = document.createElement('div');
	const unlikelyContainer = document.createElement('div');
	likelyContainer.classList.add('likely');
	unlikelyContainer.classList.add('unlikely');
	addPairElements(likelyContainer, likelies, ['fa', 'fa-solid', 'fa-check']);
	addPairElements(unlikelyContainer, unlikelies, ['fa', 'fa-solid', 'fa-not-equal']);
	const dialectSpecificLiklies = likelies.filter(L => L[0] !== isLikelyBasic &&
		L[0] !== isLikelyJavaScript && L[0] !== isLikelyMetaLanguage);
	if (dialectSpecificLiklies.length > 1) {
		const tooManyLikelyDiv = document.createElement('div');
		tooManyLikelyDiv.classList.add('warning');
		tooManyLikelyDiv.innerText = `Too many likely functions are returning true.  At most only 1 dialect-specific should return true for given code but ${dialectSpecificLiklies.length} are returning true.`;
		likelyContainer.appendChild(tooManyLikelyDiv);
	}
	outputElement.appendChild(likelyContainer);
	outputElement.appendChild(unlikelyContainer);
}

function init() {
	codeElement = document.getElementById('code');
	outputElement = document.getElementById('classification-report');
	codeElement.addEventListener('change', updateClassificationReport);
	codeElement.addEventListener('input', updateClassificationReport);
	codeElement.value = initialCode;
	updateClassificationReport();
}

await asyncInitCreateParameterizedGroups();
ready(init);