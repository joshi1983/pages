...
class PrivateLiveRedrawer {
		...
		*/
		...
		this.executer.isReadyToGetDrawing()) {
			...
			isNumber(this._t) && isNumber(this._duration))
				getSnapshotStyleFromProgram(this.executer.program, this._t, this._duration, drawing).then(function(snapshotStyle) {
	...
	async refreshProgram() {
		try {
			this.executer = new FastExecuter(200);
			await this.executer.compile(Code.sourceCode);
		}
		catch (e) {
			if (typeof e === 'string')
		...
		this._duration = animationSetup.get('duration');
		this.startDrawing();
	}
