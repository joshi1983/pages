...
export class TurtleDrawState extends EventDispatcher {
	...
	circle(radius) {
		if (this.orientation instanceof Orientation3D) {
			return processPenUpStyle(this, new OrientedCircleShape(new Vector3D(this.position), this.orientation.clone(),
				radius, this.style.deepClone()));
		}
		return processPenUpStyle(this, getSimplestShape(new CircleShape(new Vector3D(this.position), radius, this.style.deepClone())));
	}
	...
	circleLeft(radius) {
		...
		return processPenUpStyle(this, getSimplestShape(new CircleShape(p, radius, this.style.deepClone())));
	}
