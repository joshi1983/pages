...
export class TurtleDrawState extends EventDispatcher {
	...
	arc2(radius, angleRadians) {
		const returnCircle = (angleRadians >= Math.PI * 2 || angleRadians <= -Math.PI * 2);
		...
		if (this.isInPolygon || returnCircle === false) {
			const sanitized = TurtleDrawState.sanitizeArcProperties(startAngle, angleRadians);
	...
	arcLeft(radius, angleRadians) {
		...
		this.position = arcCentre.plus(displacement);
		const returnCircle = (angleRadians >= Math.PI * 2);
		if (returnCircle === true)
			angleRadians = angleRadians % (Math.PI * 2);
	...
	static convertGradientPoint(p) {
		if (p.length > 2)
			p = p.slice(0, 2);
		return new Vector2D(p);
	}

	static createLinearGradient(p1, p2, colorStops, spreadMethod) {
		return new LinearGradient(
			 Gradient.sanitizeColorStops(colorStops),
			 TurtleDrawState.convertGradientPoint(p1),
			 TurtleDrawState.convertGradientPoint(p2),
			 SpreadMethod.parse(spreadMethod.toLowerCase())
		);
	}

	static createRadialGradient(outerCentre, focus, radius, colorStops, spreadMethod) {
		return new RadialGradient(
			 Gradient.sanitizeColorStops(colorStops),
			 TurtleDrawState.convertGradientPoint(outerCentre),
			 TurtleDrawState.convertGradientPoint(focus),
			 radius,
			 SpreadMethod.parse(spreadMethod.toLowerCase())
		);
	}
	...
	ellipseArc2(angle, radius1, radius2, startAngle) {
		...
		if (this.isInPolygon || returnEllipse === false) {
			const sanitized = TurtleDrawState.sanitizeArcProperties(startAngle, angle);
	...
	forward(distance) {
		if (distance === 0 && this.style.getLineCap() !== "round")
	...
	getPolyShape() {
		...
		return new PathShape(this.polyPoints, true, this.style.deepClone());
	}
	...
	image(width, height, url) {
		return new RasterRectangleShape(new Vector3D(this.position), 
			width, height, this.orientation.headingRadians, url, this.style.deepClone());
	}
	...
	static sanitizeArcProperties(startAngle, angle) {
		if (angle < 0) {
			startAngle += angle;
			angle = -angle;
		}
	...
	setFillLinearGradient(p1, p2, colorStops, spreadMethod) {
		this.setFillGradient(TurtleDrawState.createLinearGradient(p1, p2, colorStops, spreadMethod));
	}

	setFillRadialGradient(outerCentre, focus, radius, colorStops, spreadMethod) {
		this.setFillGradient(TurtleDrawState.createRadialGradient(outerCentre, focus, radius, colorStops, spreadMethod));
	}
	...
	setHeading(newHeadingRadians) {
		if (typeof newHeadingRadians !== 'number' || isNaN(newHeadingRadians))
