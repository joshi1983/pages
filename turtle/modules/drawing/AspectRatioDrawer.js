...
function refreshSnapshotStyle() {
	if (Code.latestProgram !== undefined) {
		...
		const drawing = Settings.executer.turtle.drawing;
		if (drawing === undefined)
			throw new Error(`refreshSnapshotStyle called when drawing is undefined.`);
		const getSnapshotPromise = getSnapshotStyleFromProgram(Code.latestProgram, Settings.animationTime, Settings.animationDurationSeconds, drawing);
...
Settings.executer.addEventListener('execution-stopped', refreshSnapshotStyle);
