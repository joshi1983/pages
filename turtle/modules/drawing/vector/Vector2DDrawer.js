...
export class Vector2DDrawer {
	constructor(screenColour) {
		...
		this.setScreenColour(screenColour);
	}
	...
	drawShape(shape) {
		if (shape instanceof ArcShape)
			this.drawArc(...arguments);
		else if (shape instanceof CircleShape)
			this.drawCircle(...arguments);
		else if (shape instanceof EllipseShape)
			this.drawEllipse(...arguments);
		else if (shape instanceof EllipseArcShape)
			this.drawEllipseArc(...arguments);
		else if (shape instanceof LineSegmentShape)
			this.drawLine(...arguments);
		else if (shape instanceof PathShape)
			this.drawPath(...arguments);
		else if (shape instanceof RasterRectangleShape)
			this.drawRasterRectangle(...arguments);
		else if (shape instanceof SphereShape)
			this.drawSphere(...arguments);
		else if (shape instanceof TextShape)
			this.drawText(...arguments);
		else
			throw new Error('Unable to draw shape: ' + shape);
	}
	...
	setScreenColour(c) {
		if (!(c instanceof Colour) && c !== Transparent)
			throw new Error('setScreenColour requires c to be a Colour or Transparent');
		this.screenColour = c;
	}
