...
export class ShapeStyle extends EventDispatcher {
	...
	assignFromObject(objectProperties) {
		if (objectProperties instanceof ShapeStyle) {
			...
			if (typeof objectProperties.pen === 'object') {
				...
				if (pen.color instanceof Colour)
			...
			if (typeof objectProperties.material === 'object') {
				...
				if (typeof material.fill === 'object') {
					...
					if (fill.color === Transparent || fill.color instanceof Colour)
	...
	deepClone() {
		...
		if (this.getFillColor() !== Transparent)
			result.material.fill.color = new Colour(this.getFillColor());
		if (this.getFillGradient() !== undefined)
			result.material.fill.gradient = this.getFillGradient().deepClone();

		result.pen.color = new Colour(this.getPenColor());
	...
	getLineCap() {
		if (this.pen.lineJoinStyle === LineJoinStyle.Round)
			return 'round';
		else
			return 'square';
	}
	...
	setFillColor(c) {
		...
		else if (c !== Transparent && !(c instanceof Colour))
			throw new Error('c must be either transparent or a Colour.  c = ' + c);
	...
	setPenColor(c) {
		if (!(c instanceof Colour))
			throw new Error('c must be a Colour');
		else if (c === null)
	...
	usesFill() {
		return this.material.fill.color !== Transparent || this.material.fill.gradient !== undefined;
	}
};