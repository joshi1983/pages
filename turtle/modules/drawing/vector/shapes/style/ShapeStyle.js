...
import { MixBlendMode } from '../mix-blend-modes/MixBlendMode.js';
...
export class ShapeStyle extends EventDispatcher {
	...
	assignFromObject(objectProperties) {
		if (objectProperties instanceof ShapeStyle) {
			...
			if (typeof objectProperties.pen === 'object') {
				...
				if (Number.isInteger(pen.blendMode))
					this.setPenBlendMode(pen.blendMode);
			...
			if (typeof objectProperties.material === 'object') {
				...
				if (typeof material.fill === 'object') {
					...
					if (Number.isInteger(fill.blendMode))
						this.setFillBlendMode(fill.blendMode);
	...
	assignFromShapeStyle(shapeStyle) {
		...
		this.setPenBlendMode(shapeStyle.getPenBlendMode());
		...
		this.setFillBlendMode(shapeStyle.getFillBlendMode());
	...
	getFillBlendMode() {
		return this.material.fill.blendMode;
	}
	...
	setFillBlendMode(blendModeInt) {
		if (!Number.isInteger(blendModeInt))
			throw new Error(`Expected blendModeInt to be an integer but found ${blendModeInt}`);

		this.material.fill.blendMode = blendModeInt;
	}
	...
	setPenBlendMode(blendModeInt) {
		if (!Number.isInteger(blendModeInt))
			throw new Error(`Expected blendModeInt to be an integer but found ${blendModeInt}`);

		this.pen.blendMode = blendModeInt;
	}
