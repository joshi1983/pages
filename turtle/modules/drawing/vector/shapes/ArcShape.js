...
export class ArcShape extends Shape {
	...
	*/
	...
	getBoundingBox() {
		...
		const anglePair = [rotationRadians, rotationRadians + clampedAngle];
		const angles = anglePair.slice();
		...
		const lineCap = this.style.getLineCap();
		for (let i = 0; i < angles.length; i++) {
			const angle = Math.PI * 1.5 - angles[i];
			result.include(this.position.plus(Vector3D.createFromPolar(angle, this.radius + pw)));
			result.include(this.position.plus(Vector3D.createFromPolar(angle, this.radius - pw)));
		}
		if (lineCap === LineCap.Round || lineCap === LineCap.Square) {
			for (let index = 0; index < anglePair.length; index++) {
				const angle = Math.PI * 1.5 - anglePair[index];
				const v1 = this.position.plus(Vector3D.createFromPolar(angle, this.radius));
				if (lineCap === LineCap.Round) {
					result.include(v1.plus(new Vector3D(pw, pw, 0)));
					result.include(v1.plus(new Vector3D(-pw, -pw, 0)));
				}
				else {
					const sign = Math.sign(0.5 - index); // -1 for end and 1 for start.
					const middle = v1.plus(Vector3D.createFromPolar(angle + sign * Math.PI / 2, pw));
					result.include(middle.plus(Vector3D.createFromPolar(angle, pw)));
					result.include(middle.plus(Vector3D.createFromPolar(angle, -pw)));
				}
			}
		}
