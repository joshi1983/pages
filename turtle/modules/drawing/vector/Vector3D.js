...
export class Vector3D extends Vector {
	...
	static createFromPolar(angleRadians, distance) {
		if (!isNumber(angleRadians))
			throw new Error(`angleRadians must be a number.  Specified was ${angleRadians}`);
		if (!isNumber(distance))
			throw new Error(`distance must be a number.  Specified was ${distance}`);
