...
export class PerspectiveTransformer {
	constructor(width, height, isYOutputFlipped) {
		if (!isNumber(width))
			throw new Error('width must be a number but got ' + width);
		if (!isNumber(height))
			throw new Error('height must be a number but got ' + height);
		if (isYOutputFlipped === undefined)
			isYOutputFlipped = false;

		this.isYOutputFlipped = isYOutputFlipped;
		this.width = width;
		this.height = height;
