import { processBlendModesForCanvasContextFill } from
'./processBlendModesForCanvasContextFill.js';
import { processBlendModesForCanvasContextStroke } from
'./processBlendModesForCanvasContextStroke.js';
...
export function drawPath(canvasDrawer, path, ctx) {
	...
	if (path.style.isFillVisible()) {
		processBlendModesForCanvasContextFill(path.style, ctx);
		ctx.fill();
	}
	if (path.style.isPenVisible()) {
		processBlendModesForCanvasContextStroke(path.style, ctx);
		ctx.stroke();
	}
	if (!path.isClosed)
		ctx.closePath();
