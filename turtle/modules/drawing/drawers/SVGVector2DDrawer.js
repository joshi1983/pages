...
export class SVGVector2DDrawer extends Vector2DDrawer {
	...
	_getStartTag() {
		const result = new StringBuffer();
		result.append(`<svg xmlns="${SVGVector2DDrawer.xmlns}" width="${this.width}" height="${this.height}"`);
		if (this.screenColour !== Transparent)
			result.append(` style="background-color:${getShortestRGBHexCode(this.screenColour)}"`);
		return result.toString() + '>';
	}
	...
	toString(options) {
		...
		const startTags = [this._getDeclaration(), webLogoComment, this._getStartTag(), this._getDefs(), gTag].join('\n');
