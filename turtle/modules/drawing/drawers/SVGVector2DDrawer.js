...
export class SVGVector2DDrawer extends Vector2DDrawer {
	...
	drawPath(path) {
		...
		if (!path.containsNonPoints()) {
			...
			if (path.isClosed)
				...
				tagName = 'polyline';
			this.pushTag('<' + tagName + ' points="' + points.join(", ") + '" ' + this.getStyleAttributes(path.style) + ' />');
		}
		else {
			...
			for (let i = 0; i < path.elements.length; i++) {
				...
				if (element instanceof Vector) {
					...
					tagString.append(` A ${rounder.formatNumber(arc.radius)} ${rounder.formatNumber(arc.radius)} 0 ${largeArcFlag} ${counterClockwiseFlag} ${
						this.rounder.formatNumber(end.getX())} ${rounder.formatNumber(end.getY())}`);
				}
			}
			if (path.isClosed)
				tagString.append(' z');
			tagString.append('" ' + this.getStyleAttributes(path.style) + '/>');
			this.pushTag(tagString.toString());
		}
	}
	...
	drawRasterRectangle(rasterShape) {
		...
		this.pushTag(`<image width="${rasterShape.width}" height="${rasterShape.height}" xlink:href="${encodeUrl(rasterShape.dataUrl)}" ${transformAttributes}  preserveAspectRatio="none"/>`);
	}
	...
	getStrokeStyle(style) {
		...
		if (pw !== 0) {
			...
			if (gradient === undefined) {
				result += ' stroke="' + style.getPenColor().to6DigitHTMLCode() + '"';
			}
			else {
	...
	getStyleAttributes(style) {
		...
		else if (style.getFillColor() === Transparent)
			result += ' fill="none"';
		else
			result += ' fill="' + style.getFillColor().to6DigitHTMLCode() + '"';
	...
	toString(options) {
		...
		const startTags = [this._getDeclaration(), this._getStartTag(), this._getMeta(), this._getDefs(), gTag].join('\n');
