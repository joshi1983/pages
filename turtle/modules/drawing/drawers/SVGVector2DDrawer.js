...
import { MixBlendMode } from '../vector/shapes/mix-blend-modes/MixBlendMode.js';
import { mixBlendModeToSVGName } from './svg/mixBlendModeToSVGName.js';
import { pathTo2D } from './shape-to-2d/pathTo2D.js';
...
export class SVGVector2DDrawer extends Vector2DDrawer {
	...
	drawArc(arc) {
		if (arc.angle < 0) {
			arc = arc.deepClone(); // avoid mutating the passed shape.
			arc.swapArcDirection(); // get a positive angle that should look the same.
		}
		...
		this.pushTag('<path d="M ' + this.rounder.formatNumber(end.getX()) + ' ' + this.rounder.formatNumber(end.getY()) +
		   ...
		   this.rounder.formatNumber(start.getY()) + '" ' + this.getStyleAttributes(arc.style, false) + '/>');
	}
	...
	drawEllipseArc(eArc) {
		...
		this.pushTag('<path d="M ' + this.rounder.formatNumber(end.getX()) + ' ' + this.rounder.formatNumber(end.getY()) +
			...
			this.rounder.formatNumber(start.getX()) + ' ' +
			this.rounder.formatNumber(start.getY()) + '" ' + this.getStyleAttributes(eArc.style, false) + '/>');
	}
	...
	getStrokeStyle(style, mightNeedMiterLimit) {
		...
		if (pw !== 0 && style.isPenVisible()) {
			...
			else if (mightNeedMiterLimit) {
				result += ` stroke-miterlimit="${this.rounder.formatNumber(style.getMiterLimit())}"`;
			}
	...
	getStyleAttributes(style, mightNeedMiterLimit) {
		let result = this.getStrokeStyle(style, mightNeedMiterLimit);
		const gradient = style.getFillGradient();
		const fillColor = style.getFillColor();
		let styleValue = '';
		if (style.getFillBlendMode() !== MixBlendMode.Normal) {
			styleValue += `mix-blend-mode: ${mixBlendModeToSVGName(style.getFillBlendMode())}`;
		}
		// FIXME: appropriately handle cases where the pen blend mode is different from the fill blend mode.
		if (styleValue !== '') {
