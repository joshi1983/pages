...
export function drawUTurnRect(svgDrawer, path) {
	...
	svgDrawer.pushTag(`<rect${xFormatted}${yFormatted} width="${rounder.formatNumber(width)}" height="${rounder.formatNumber(height)}"${svgDrawer.getStyleAttributes(path.style)}${rotation}${rxFormatted} />`);
};