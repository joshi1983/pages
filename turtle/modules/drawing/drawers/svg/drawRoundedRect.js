...
export function drawRoundedRect(svgDrawer, path) {
	...
	if (yStr === '0')
		...
		yStr = ` y="${yStr}"`;
	svgDrawer.pushTag(`<rect${xStr}${yStr} width="${rounder.formatNumber(width)}" height="${rounder.formatNumber(height)}"${svgDrawer.getStyleAttributes(path.style, false)}${rotation} rx="${cornerRadiusRounded}" />`);
};