...
export function drawPath(svgDrawer, path) {
	...
	if (isSimpleRect(path)) {
	...
	else if (isUTurnRect(path)) {
		drawUTurnRect(svgDrawer, path);
	}
		...
		if (path.isClosed)
			...
			tagName = 'polyline';
		svgDrawer.pushTag('<' + tagName + ' points="' + points.join(", ") + '" ' + svgDrawer.getStyleAttributes(path.style, true) + ' />');
	}
	else {
		...
		if (path.isClosed)
			tagString.append(' z');
		tagString.append('" ' + svgDrawer.getStyleAttributes(path.style, true) + '/>');
