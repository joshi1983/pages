...
export class IndexSearchTopic {
	...
	static TopicTypeGlossaryTerm = 3;
	static TopicTypeTutorial = 4;
	static TopicTypeDataType = 5;
	static TopicTypeBreakpoint = 6;
	...
	constructor(type, info, click) {
		...
		if (info.readCommand !== undefined)
			this.hiddenKeywords.push(info.readCommand);
		if (info.hiddenKeywords !== undefined)
			ArrayUtils.pushAll(this.hiddenKeywords, info.hiddenKeywords);
	}
...
typesInfo[IndexSearchTopic.TopicTypeGlossaryTerm] = {
	'iconClass': 'glossary-icon',
	'title': 'glossary'
};
