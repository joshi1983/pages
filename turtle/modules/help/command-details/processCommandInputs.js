	...
	if (commandInfo.args === undefined || commandInfo.args.length === 0)
		...
		commandInfo.args.forEach(function(arg) {
			...
			if (arg.uselessCases instanceof Array) {
				const p = document.createElement('p');
				p.innerText = `${arg.name} should not be ${valuesToStringExpression(arg.uselessCases)}.`;
				li.appendChild(p);
			}
			...
			if (typeof arg.listElementTypes === 'string') {
				const p = document.createElement('p');
				p.innerText = `Every list element must be ${argumentToEnglish(arg.listElementTypes)}`;
				li.appendChild(p);
			}
