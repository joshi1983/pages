...
function numToString(numType, typeStr) {
	if (numType.min > -Infinity && numType.max < Infinity) {
		if (numType.min === numType.max)
			return '' + numType.min;

		return `number in ${numType.min}..${numType.max}`;
	}
	else if (numType.min > -Infinity)
		return `number at least ${numType.min}`;
	else if (numType.max < Infinity)
		return `number at most ${numType.max}`;

	if (numType.isFiniteOnly)
...
export function dataTypesToEnglish(types, escapeForHTML) {
	...
	types = types.map(function(type) {
		...
		if (type.name === 'num' &&
		(type.max < Infinity || type.min > -Infinity))
			return numToString(type, typeStr);
