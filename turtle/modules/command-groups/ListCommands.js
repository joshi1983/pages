...
import { indexOfSorted } from './helpers/indexOfSorted.js';
...
export class ListCommands {
	...
	sort(list1) {
		return list1.slice().sort(compare);
	}

	sortedp(list1) {
		for (let i = 1; i < list1.length; i++) {
			if (compare(list1[i - 1], list1[i]) > 0)
				return false;
		}
		return true;
	}
...
indexOf, indexOfSorted, lput, mixItems, mixItems2, remove, sumOfFirsts].forEach(function(func) {
