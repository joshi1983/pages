...
export class Turtle extends EventDispatcher {
	...
	forward(distance) {
		...
		if (this.drawState.isPenDown && shape.style.isPenVisible())
			this.drawing.addForegroundShape(shape);
	}
	...
	penColor() {
		return this.drawState.getPenColor().toArray();
	}
	...
	setFillLinearGradient(p1, p2, colorStops, spreadMethod) {
		const msg = gradientSpreadMethod(spreadMethod);
		if (msg === undefined)
			this.drawState.setFillLinearGradient(p1, p2, colorStops, spreadMethod);
		else
			throw new Error(msg);
	}
	...
	setFillRadialGradient(outerCentre, focus, radius, colorStops, spreadMethod) {
		const msg = gradientSpreadMethod(spreadMethod);
		if (msg === undefined)
			this.drawState.setFillRadialGradient(outerCentre, focus, radius, colorStops, spreadMethod);
		else
			throw new Error(msg);
	}
