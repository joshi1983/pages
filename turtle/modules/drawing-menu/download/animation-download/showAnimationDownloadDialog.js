...
import { bindFrameSequenceIndexInputs } from './modes/bindFrameSequenceIndexInputs.js';
...
import { getTotalFrameDownloadCount } from './modes/getTotalFrameDownloadCount.js';
...
import { updateFrameIndexRange } from './modes/updateFrameIndexRange.js';
...
function updateFrameCount(totalFrames) {
	...
	if (typeof totalFrames !== 'number') {
		totalFrames = getTotalFrameDownloadCount(startFrameIndexInput, endFrameIndexInput,
	...
	let maxIndex = durationSeconds * parseInt(frameRatesDropdown.value);
	if (!Number.isInteger(maxIndex))
		maxIndex = durationSeconds * 24;

	maxIndex = Math.floor(maxIndex);
	updateFrameIndexRange(startFrameIndexInput, endFrameIndexInput, maxIndex, mode);
...
function getSettings() {
	...
	const settings = {
		...
		'notifyFrameCanvas': Thumbnail.notifyFrameCanvas,
		'notifyMessage': notifyMessage,
		'startFrameIndex': parseInt(startFrameIndexInput.value),
		'endFrameIndex': parseInt(endFrameIndexInput.value)
	};
...
export function showAnimationDownloadDialog() {
	...
	endFrameIndexInput = document.getElementById('animation-download-end-frame-index');
	bindFrameSequenceIndexInputs(startFrameIndexInput, endFrameIndexInput);
	Thumbnail.refreshElements();
