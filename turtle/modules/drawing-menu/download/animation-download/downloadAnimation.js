...
export async function downloadAnimation(program, width, height, settings) {
	if (!Number.isInteger(width) || width <= 0)
		throw new Error(`width must be a positive integer but found ${width}`);
	if (!Number.isInteger(height) || height <= 0)
		throw new Error(`height must be a positive integer but found ${height}`);
	if (typeof settings !== 'object')
		throw new Error(`settings must be an object but found ${settings}`);
	if (!Number.isInteger(settings.fps) || settings.fps <= 0)
		throw new Error(`settings.fps must be a positive integer but found ${settings.fps}`);
	...
	for (let t = 0; t < duration; t+= timeInterval) {
		if (settings.endFrameIndex >= 0 && frameIndex > settings.endFrameIndex)
			break;
		else if (frameIndex >= settings.startFrameIndex) {
