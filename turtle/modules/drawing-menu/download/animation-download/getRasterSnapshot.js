...
*/
export function getRasterSnapshot(program, width, height, animationTimeSeconds, animationDurationSeconds) {
	...
	return getSnapshotStyleFromProgram(program, animationTimeSeconds, animationDurationSeconds).then(function(snapshotStyle) {
		const drawingUnitDimensions = snapshotStyle.getDimensionsForAspectRatio(width / height);
		return getDrawingSnapshot(program, animationTimeSeconds, animationDurationSeconds).then(function(drawing) {
