...
export function circle(circle, degreesPerArcDivision) {
	...
	for (let i = 0; i < numSides; i++) {
		...
		const x = circle.position.getX() + r * Math.cos(a);
		const y = circle.position.getY() + r * Math.sin(a);
		const newPoint = new Vector3D(x, y, circle.position.getZ());
		if (prev !== undefined) {
