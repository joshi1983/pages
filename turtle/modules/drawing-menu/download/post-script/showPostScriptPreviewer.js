...
function initializeViewer(postScriptDrawing) {
	...
	viewer = new SVGDrawingViewer(container, postScriptDrawing);
	const pageSize = PageSize.getDefaultPageSize();
	viewer.setAspectRatio(pageSize.getWidthPostScriptUnits() / pageSize.getHeightPostScriptUnits());
	const zoomIn = document.getElementById("post-script-zoom-in");
	const zoomOut = document.getElementById("post-script-zoom-out");
