...
function createHeadingLine(parseTreeToken) {
...
export function parseTreeTokenToElement(parseTreeToken) {
	if (!(parseTreeToken instanceof ParseTreeToken))
		throw new Error('parseTreeToken must be a ParseTreeToken');
	const result = document.createElement('div');
	const headingLineInfo = createHeadingLine(parseTreeToken);
	...
	if (parseTreeToken.children.length !== 0) {
		...
		parseTreeToken.children.forEach(function(childToken) {
			childList.appendChild(parseTreeTokenToElement(childToken));
		});
