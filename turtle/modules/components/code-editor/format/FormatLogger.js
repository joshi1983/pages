...
export class FormatLogger {
	...
	log(s, parseToken) {
		...
		for (let i = this.scanIndex; i < this.scannedTokens.length; i++) {
			...
			this.newLine();
			this.loggedSections.push(new LoggedSection(token.s.trim(),
				ParseTreeToken.createFromScannedToken(token, new Set()), false, this.indentation));
			this.newLine();
			this.scanIndex = i + 1;
		}
		this.loggedSections.push(new LoggedSection(s, parseToken, !this._removeSpacePrefixForNextLog, this.indentation));
		this._removeSpacePrefixForNextLog = false;
	}
