...
function canBeBroken(loggedSections) {
	...
	if (loggedSections.filter((s, index) => index !== 0 && s.isSpacePrefixed === false).length === loggedSections.length)
		return false;
	if (loggedSections[0].token.type === ParseTreeTokenType.UNARY_OPERATOR &&
	loggedSections[0].token.children.length !== 0 &&
	loggedSections.length === 2)
		return false;
...
function getSingleLineString(indentation, sections) {
	...
	if (sections.length !== 0)
		indentation = sections[0].indentation;
	let indentationS = '\t'.repeat(indentation);
