...
export class LoggedSection {
	constructor(s, token, isSpacePrefixed, indentation) {
		if (typeof s !== 'string')
		...
		if (!Number.isInteger(indentation) || indentation < 0)
			throw new Error(`indentation must be an integer at least 0 but found ${indentation}`);
	...
	breakAt(index) {
		...
		return lines.map(s => new LoggedSection(s, this.token, false, 0));
	}

	isExplodable() {
		if ([ParseTreeTokenType.COMMENT,
		ParseTreeTokenType.LEAF,
		ParseTreeTokenType.LONG_STRING_LITERAL,
		ParseTreeTokenType.PROCEDURE_START_KEYWORD,
		ParseTreeTokenType.VARIABLE_READ].indexOf(this.token.type) !== -1)
			return false;
	...
	explode() {
		if (this.isExplodable()) {
			return this.s.split(' ').map(
				(s, index) =>
				new LoggedSection(s, this.token, index === 0 && this.isSpacePrefixed, this.indentation));
		}
		else
			return [this];
	}
	...
	static createByJoining(sections) {
		...
		const isSpacePrefixed = firstSection.isSpacePrefixed;
		const token = firstSection.token;
		return new LoggedSection(s, token, isSpacePrefixed, firstSection.indentation);
	}
};