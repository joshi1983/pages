...
export function shouldNotShowAutofixPromptForCode(originalCode) {
	const tokens = LogoScanner.scan(originalCode);
	let code = tokensToCode(tokens.filter(shouldBeChecked));
	let result = false;
	if (code.indexOf('http://') !== -1 || code.indexOf('https://') !== -1)
		result = true;
	if (result) {
		const [translator, adjustAesthetics] = codeToTranslator(originalCode);
		if (translator.name !== '')
			result = false;
	}
	return result;
