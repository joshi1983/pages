...
function getCallingLineNumberFromExecutingProcedure(context) {
	return function(executingProcedure) {
		...
		const token = instruction.parseTreeToken;
		if (token.module !== undefined && token.module.name === 'internal')
			return;
...
function getCurrentLinesFromCallStack() {
	...
	if (context.logoProgram.code !== Code.getSourceCode())
		return;
	return context.procedureStack.map(getCallingLineNumberFromExecutingProcedure(context)).filter(Number.isInteger);
}
