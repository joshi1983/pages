...
export function animationSetupFixer(cachedParseTree, fixLogger) {
	...
	if (proc !== undefined) {
		...
		const animationTimeTokens = CommandCalls.filterCommandCalls(procTokens, [
			'animation.clampedTimeRatio', 'animation.time', 'animation.timeRatio'
		]);
