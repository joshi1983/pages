...
const commandsRequiringString = Command.getAllCommandsInfo().
	filter(info => info.primaryName !== 'to' &&
		...
		info.args.some(argInfo => matchingTypes.has(argInfo.types) && argInfo.format === undefined)
	);
...
function convertNumberToString(token, cachedParseTree, fixLogger) {
	token.type = ParseTreeTokenType.STRING_LITERAL;
	token.val = token.originalString;
	token.originalString = undefined;
	cachedParseTree.tokenTypeChanged(token, ParseTreeTokenType.NUMBER_LITERAL);
	fixLogger.log(`Added quote to number because a string is required.`, token);
}
		...
		for (let i = 0; i < numChildren; i++) {
			...
			if (matchingTypes.has(argInfo.types)) {
				...
				if (child.type === ParseTreeTokenType.NUMBER_LITERAL) {
					convertNumberToString(child, cachedParseTree, fixLogger);
					continue;
				}
