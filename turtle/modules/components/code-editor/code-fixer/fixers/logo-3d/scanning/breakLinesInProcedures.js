...
function isCommandCallOfInterest(tokens, index) {
	const token = tokens[index];
	const prev = tokens[index - 1];
	if (prev === undefined || prev.lineIndex !== token.lineIndex ||
	prev.s.toLowerCase() === 'to')
		return false;
	
	const info = Logo3DCommands.getCommandInfo(token.s);
	if (info === undefined || (info.returnTypes !== undefined && info.returnTypes !== null))
		return false;
...
function isKeywordOfInterest(tokens, index) {
	const prev = tokens[index - 1];
	if (prev === undefined)
		return false;
	const token = tokens[index];
...
export function breakLinesInProcedures(code) {
	...
	for (let i = 2; i < scanTokens.length; i++) {
		...
		isCommandCallOfInterest(scanTokens, i) ||
		isKeywordOfInterest(scanTokens, i)) {
