...
function fh(code) {
	return formatCode(harmonizeCase(code));
}
...
export function logo3DToWebLogo(code, parseLogger) {
	...
	code = scanInfo.code;
	let tree = LogoParser.getParseTree(tokens, tempParseLogger, proceduresMap, options);
	if (tree === undefined)
		return fh(code); // the code is unfixable if it can't be parsed.
	...
	code = processArcUpDown(code, fixLogger);
	return fh(code);
};