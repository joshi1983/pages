...
function convertTokenToInstructionListToken(token, cachedParseTree) {
	const oldType = token.type;
	const childToken = token.shallowClone();
	for (const child of token.children) {
		child.remove();
		childToken.appendChild(child);
	}
	token.appendChild(childToken);
	token.val = null;
	token.type = ParseTreeTokenType.LIST;
	cachedParseTree.tokenTypeChanged(token, oldType);
	cachedParseTree.tokenAdded(childToken);
}

function getExtraInstructionTokens(listToken) {
	const result = [];
			...
			if (nameToken.nextSibling !== null && nameToken.nextSibling.type === ParseTreeTokenType.LIST) {
				...
				if (listToken.children.length > 3) {
					...
					if (paramListToken.children.length >= 2) {
						...
						const extraInstructionTokens = getExtraInstructionTokens(listToken);
						...
						if (instructionListToken.type !== ParseTreeTokenType.LIST)
							convertTokenToInstructionListToken(instructionListToken, cachedParseTree);
						...
						for (const extraInstructionToken of extraInstructionTokens) {
							if (extraInstructionToken.type === ParseTreeTokenType.LIST) {
								removeListBrackets(extraInstructionToken, logger, cachedParseTree);
								const children = extraInstructionToken.children.slice().reverse();
								for (const child of children) {
									child.remove();
									instructionListToken.prependChild(child);
								}
								extraInstructionToken.remove();
								cachedParseTree.tokenRemoved(extraInstructionToken);
							}
							else {
								extraInstructionToken.remove();
								instructionListToken.prependChild(extraInstructionToken);
							}
						}
