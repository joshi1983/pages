...
export function instructionListSquareBracketsRemoveFixer(cachedParseTree, fixLogger) {
	...
	lists.forEach(function(listToken) {
		const parent = listToken.parentNode;
		const squareLeftBracket = listToken.children[0];
		const squareRightBracket = listToken.children[listToken.children.length - 1];
		...
		listToken.remove();
		tokensToRemove.push(squareLeftBracket, squareRightBracket, listToken);
