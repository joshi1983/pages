import { Command } from
'../../../../../parsing/Command.js';
import { isInstructionList } from
'../../../../../parsing/parse-tree-analysis/isInstructionList.js';
import { Operators } from
'../../../../../parsing/Operators.js';
import { ParseTreeTokenType } from
'../../../../../parsing/ParseTreeTokenType.js';

function isBracketToRemove(token) {
	return token.val === '(' ||
		token.val === ')';
}

function isUnneeded(expr) {
	if (expr.children.length === 3) {
		const parent = expr.parentNode;
		const middle = expr.children[1];
		if (middle.children.length === 0)
			return true;

		if (middle.type === ParseTreeTokenType.BINARY_OPERATOR &&
		parent.type === ParseTreeTokenType.BINARY_OPERATOR) {
			// if parent operator has lower precedence, return true?
			const parentInfo = Operators.getOperatorInfo(parent.val);
			const info = Operators.getOperatorInfo(middle.val);
			if (parentInfo.precedence < info.precedence)
				return true;
		}
		else if (middle.type === ParseTreeTokenType.PARAMETERIZED_GROUP) {
			const info = Command.getCommandInfo(middle.val);
			if (info !== undefined) {
				if (info.primaryName === 'invoke')
					return false; // invoke always must be wrapped in curved brackets.

				const argCount = Command.getArgCount(info);
				if (argCount !== undefined &&
				middle.children.length !== argCount.defaultCount) {
					return false;
				}
			}
			if (parent.type === ParseTreeTokenType.PARAMETERIZED_GROUP)
				return true;
		}
		else if (middle.type === ParseTreeTokenType.CURVED_BRACKET_EXPRESSION ||
		middle.type === ParseTreeTokenType.LIST)
			return true;

		if (parent.type === ParseTreeTokenType.PARAMETERIZED_GROUP) {
			const next = expr.nextSibling;
			if (next !== null && next.type === ParseTreeTokenType.LIST)
				return true;
			if (parent.children.length === 1) {
				return true;
			}
		}
		if (isInstructionList(parent))
			return true;

	}
	return false;
}

export function removeUnneededCurvedBrackets(cachedParseTree, fixLogger) {
	const curvedBracketExpressions = cachedParseTree.getTokensByType(ParseTreeTokenType.CURVED_BRACKET_EXPRESSION).
		filter(isUnneeded);
	const toRemove = [];
	for (const expr of curvedBracketExpressions) {
		const childrenToKeep = expr.children.filter(t => !isBracketToRemove(t));
		if (childrenToKeep.length === 1) {
			for (const child of expr.children) {
				if (isBracketToRemove(child)) {
					child.remove();
					toRemove.push(child);
				}
			}
			const middle = expr.children[0];
			const parent = expr.parentNode;
			middle.remove();
			parent.replaceChild(expr, middle);
			toRemove.push(expr);

			if (middle.type === ParseTreeTokenType.NUMBER_LITERAL &&
			parent.val === '-' &&
			parent.type === ParseTreeTokenType.UNARY_OPERATOR) {
				// For example, we're changing -(12.3) to -12.3.
				middle.val = -middle.val;
				middle.originalString = '' + middle.val;
				parent.removeSingleToken();
				cachedParseTree.tokenRemoved(parent);
			}
			fixLogger.log(`Removed unneeded curved brackets`, middle);
		}
	}
	cachedParseTree.tokensRemoved(toRemove);
};