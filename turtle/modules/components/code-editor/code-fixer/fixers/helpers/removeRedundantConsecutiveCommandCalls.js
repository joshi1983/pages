...
function mightBeInPoly(token) {
	let tok = token.previousSibling;
	while (tok !== null) {
		if (tok.type === ParseTreeTokenType.PARAMETERIZED_GROUP) {
			const info = Command.getCommandInfo(tok.val);
			if (info === undefined)
				return true; // calls a procedure.  The procedure might call polyStart.
			else if (info.primaryName === 'polyEnd')
				return false;
			else if (info.primaryName === 'polyStart')
				return true;
		}
