...
function getLastDescendentToken(token) {
	let result = token;
	const descendentTokens = token.getAllDescendentsAsArray();
...
export function filledFixer(cachedParseTree, fixLogger) {
	...
	tokens.forEach(function(filledToken) {
		...
		filledToken.parentNode.removeChild(filledToken);
		colorToken.parentNode.removeChild(colorToken);
		wrappedInstructionListToken.parentNode.removeChild(wrappedInstructionListToken);
		...
		cachedParseTree.tokenAdded(setFillColor);
		cachedParseTree.tokenAdded(polyEnd);
