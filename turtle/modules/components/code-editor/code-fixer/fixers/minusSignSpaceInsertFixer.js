...
function previousLeafMightBeUndefinedProcedureCall(token) {
	let tok = token.previousSibling;
	while (tok !== null) {
		if (tok.type === ParseTreeTokenType.LEAF && validateIdentifier(tok.val) === undefined)
			return true;
...
function isOfInterest(token) {
	if (token.type === ParseTreeTokenType.NUMBER_LITERAL && token.val >= 0)
		return false;
	if (previousLeafMightBeUndefinedProcedureCall(token))
		return false;
...
export function minusSignSpaceInsertFixer(cachedParseTree, fixLogger) {
	...
	tokens.forEach(function(token) {
		...
		if (newChildNeeded && parent.nextSibling !== null) {
			const newChild = parent.nextSibling;
			newChild.remove();
