import { germanLogoToWebLogo } from
'./fixers/german-logo/germanLogoToWebLogo.js';
import { isLikely0L } from
'../../../parsing/other-languages/l-systems/0L/isLikely0L.js';
import { isLikelyAmosBasic } from
'../../../parsing/other-languages/basic/amos-basic/isLikelyAmosBasic.js';
import { isLikelyAppleSoftBasic } from
'../../../parsing/other-languages/basic/applesoft-basic/isLikelyAppleSoftBasic.js';
import { isLikelyASMTurtle } from
'../../../parsing/other-languages/asm-turtle/isLikelyASMTurtle.js';
import { isLikelyBasic } from
'../../../parsing/other-languages/basic/isLikelyBasic.js';
import { isLikelyBasic256 } from
'../../../parsing/other-languages/basic/basic-256/isLikelyBasic256.js';
import { isLikelyBasicFusion } from
'../../../parsing/other-languages/basic/basic-fusion/isLikelyBasicFusion.js';
import { isLikelyBasilBasic } from
'../../../parsing/other-languages/basic/basil/isLikelyBasilBasic.js';
import { isLikelyBazzBasic } from
'../../../parsing/other-languages/basic/bazz-basic/isLikelyBazzBasic.js';
import { isLikelyBBCBasic } from
'../../../parsing/other-languages/basic/bbc-basic/isLikelyBBCBasic.js';
import { isLikelyCanvas2D } from
'./fixers/canvas-2d/isLikelyCanvas2D.js';
import { isLikelyCGJennings } from
'../../../parsing/other-languages/l-systems/cgjennings/isLikelyCGJennings.js';
import { isLikelyCheerfulNetherlandsLogo } from
'./fixers/cheerful-netherlands-logo/isLikelyCheerfulNetherlandsLogo.js';
import { isLikelyCodeHeartTurtleScript } from
'./fixers/codeheart-turtlescript/isLikelyCodeHeartTurtleScript.js';
import { isLikelyCommodoreBasic } from
'../../../parsing/other-languages/basic/commodore-basic/isLikelyCommodoreBasic.js';
import { isLikelyFMSLogo } from
'../../../parsing/other-languages/fms-logo/isLikelyFMSLogo.js';
import { isLikelyFractInt } from
'../../../parsing/other-languages/l-systems/fractint/isLikelyFractInt.js';
import { isLikelyGermanLogo } from
'./fixers/german-logo/isLikelyGermanLogo.js';
import { isLikelyJavaScript } from
'../../../parsing/other-languages/js-parsing/isLikelyJavaScript.js';
import { isLikelyJavaScriptProcessing } from
'../../../parsing/other-languages/processing/js-processing/isLikelyJavaScriptProcessing.js';
import { isLikelyKojo } from
'../../../parsing/other-languages/kojo/isLikelyKojo.js';
import { isLikelyKTurtle } from
'../../../parsing/other-languages/kturtle/isLikelyKTurtle.js';
import { isLikelyLogo3D } from
'./fixers/logo-3d/isLikelyLogo3D.js';
import { isLikelyLogoInterpreter } from
'./fixers/logo-interpreter/isLikelyLogoInterpreter.js';
import { isLikelyMicroABasic } from
'../../../parsing/other-languages/basic/micro-a/isLikelyMicroABasic.js';
import { isLikelyPapert } from
'./fixers/papert/isLikelyPapert.js';
import { isLikelyPBasic } from
'../../../parsing/other-languages/basic/pbasic/isLikelyPBasic.js';
import { isLikelyPitrifiedGoTurtle } from
'../../../parsing/other-languages/pitrified-go-turtle/isLikelyPitrifiedGoTurtle.js';
import { isLikelyPovRay } from
'../../../parsing/other-languages/pov-ray/isLikelyPovRay.js';
import { isLikelyProcessing } from
'../../../parsing/other-languages/processing/isLikelyProcessing.js';
import { isLikelyPythonCode } from
'../../../parsing/other-languages/python/isLikelyPythonCode.js';
import { isLikelyQBasic } from
'../../../parsing/other-languages/basic/qbasic/isLikelyQBasic.js';
import { isLikelySeaTurtle } from
'./fixers/sea-turtle/isLikelySeaTurtle.js';
import { isLikelySinclairBasic } from
'../../../parsing/other-languages/basic/sinclair-basic/isLikelySinclairBasic.js';
import { isLikelySonicWebTurtle } from
'../../../parsing/other-languages/sonic-webturtle/isLikelySonicWebTurtle.js';
import { isLikelySugarLabsTurtleBlocks } from
'../../../parsing/other-languages/sugarlabs-turtle-blocks/isLikelySugarLabsTurtleBlocks.js';
import { isLikelySuperLogo } from
'./fixers/super-logo/isLikelySuperLogo.js';
import { isLikelyTektronix405XBasic } from
'../../../parsing/other-languages/basic/tektronix-405x-basic/isLikelyTektronix405XBasic.js';
import { isLikelyTexasInstruments99_4a } from
'../../../parsing/other-languages/basic/texas-instruments-99-4a/isLikelyTexasInstruments99_4a.js';
import { isLikelyTerrapin } from
'./fixers/terrapin/isLikelyTerrapin.js';
import { isLikelyTuring } from
'../../../parsing/other-languages/pascal/turing/isLikelyTuring.js';
import { isLikelyTurtleGraphicsFun } from
'../../../parsing/other-languages/turtle-graphics-fun/isLikelyTurtleGraphicsFun.js';
import { isLikelyTurtleToyNet } from
'../../../parsing/other-languages/turtletoy-net/isLikelyTurtleToyNet.js';
import { logo3DToWebLogo } from './fixers/logo-3d/logo3DToWebLogo.js';
import { logoInterpreterToWebLogo } from './fixers/logo-interpreter/logoInterpreterToWebLogo.js';
import { papertToWebLogo } from './fixers/papert/papertToWebLogo.js';
import { StringUtils } from '../../../StringUtils.js';
import { superLogoToWebLogo } from './fixers/super-logo/superLogoToWebLogo.js';
import { terrapinToWebLogo } from './fixers/terrapin/terrapinToWebLogo.js';
import { translate0LToWebLogo } from
'../../../parsing/other-languages/l-systems/0L/translation-to-weblogo/translate0LToWebLogo.js';
import { translateAppleSoftBasicToWebLogo } from
'../../../parsing/other-languages/basic/applesoft-basic/translation-to-weblogo/translateAppleSoftBasicToWebLogo.js';
import { translate as translateASMTurtle } from
'../../../parsing/other-languages/asm-turtle/translation-to-weblogo/translate.js';
import { translateBasic256ToWebLogo } from
'../../../parsing/other-languages/basic/basic-256/translation-to-weblogo/translateBasic256ToWebLogo.js';
import { translateBasicFusionToWebLogo } from
'../../../parsing/other-languages/basic/basic-fusion/translation-to-weblogo/translateBasicFusionToWebLogo.js';
import { translateBasilBasicToWebLogo } from
'../../../parsing/other-languages/basic/basil/translation-to-weblogo/translateBasilBasicToWebLogo.js';
import { translateBazzBasicToWebLogo } from
'../../../parsing/other-languages/basic/bazz-basic/translation-to-weblogo/translateBazzBasicToWebLogo.js';
import { translateCGJenningsToWebLogo } from
'../../../parsing/other-languages/l-systems/cgjennings/translation-to-weblogo/translateCGJenningsToWebLogo.js';
import { translateCheerfulToWebLogo } from
'./fixers/cheerful-netherlands-logo/translateCheerfulToWebLogo.js';
import { translateCommodoreBasicToWebLogo } from
'../../../parsing/other-languages/basic/commodore-basic/translation-to-weblogo/translateCommodoreBasicToWebLogo.js';
import { translate as translateFMSLogo } from
'../../../parsing/other-languages/fms-logo/translation-to-weblogo/translate.js';
import { translateFractIntToWebLogo } from
'../../../parsing/other-languages/l-systems/fractint/translation-to-weblogo/translateFractIntToWebLogo.js';
import { translateKojoToWebLogo } from
'../../../parsing/other-languages/kojo/translation-to-weblogo/translateKojoToWebLogo.js';
import { translate as translateKTurtle } from
'../../../parsing/other-languages/kturtle/translation-to-weblogo/translate.js';
import { translateMicroABasicToWebLogo } from
'../../../parsing/other-languages/basic/micro-a/translation-to-weblogo/translateMicroABasicToWebLogo.js';
import { translatePBasicToWebLogo } from
'../../../parsing/other-languages/basic/pbasic/translation-to-weblogo/translatePBasicToWebLogo.js';
import { translatePitrifiedGoTurtleToWebLogo } from
'../../../parsing/other-languages/pitrified-go-turtle/translation-to-weblogo/translatePitrifiedGoTurtleToWebLogo.js';
import { translate as translatePovRay } from
'../../../parsing/other-languages/pov-ray/translation-to-weblogo/translate.js';
import { translateQBASICToWebLogo } from
'../../../parsing/other-languages/basic/qbasic/translation-to-weblogo/translateQBASICToWebLogo.js';
import { translate as translateSonicWebTurtle } from
'../../../parsing/other-languages/sonic-webturtle/translation-to-weblogo/translate.js';
import { translateToWebLogo as translateCanvas2DToWebLogo } from
'./fixers/canvas-2d/translateToWebLogo.js';
import { translateToWebLogo as translateCodeHeartTurtleScriptToWebLogo } from
'./fixers/codeheart-turtlescript/translateToWebLogo.js';
import { translateAmosBasicToWebLogo } from
'../../../parsing/other-languages/basic/amos-basic/translation-to-weblogo/translateAmosBasicToWebLogo.js';
import { translateBBCBasicToWebLogo } from
'../../../parsing/other-languages/basic/bbc-basic/translation-to-weblogo/translateBBCBasicToWebLogo.js';
import { translateJSProcessingToWebLogo } from
'../../../parsing/other-languages/processing/js-processing/translation-to-weblogo/translateJSProcessingToWebLogo.js';
import { translateProcessingToWebLogo } from
'../../../parsing/other-languages/processing/translation-to-weblogo/translateProcessingToWebLogo.js';
import { translateSeaTurtleToWebLogo } from
'./fixers/sea-turtle/translation-to-weblogo/translateSeaTurtleToWebLogo.js';
import { translateSinclairBasicToWebLogo } from
'../../../parsing/other-languages/basic/sinclair-basic/translation-to-weblogo/translateSinclairBasicToWebLogo.js';
import { translateTektronix405XBasicToWebLogo } from
'../../../parsing/other-languages/basic/tektronix-405x-basic/translation-to-weblogo/translateTektronix405XBasicToWebLogo.js';
import { translateTI99BasicToWebLogo } from
'../../../parsing/other-languages/basic/texas-instruments-99-4a/translation-to-weblogo/translateTI99BasicToWebLogo.js';
import { translateToWebLogo as translateJavaScriptToWebLogo } from
'../../../parsing/other-languages/js-parsing/translation-to-weblogo/translateToWebLogo.js';
import { translateTuringToWebLogo } from
'../../../parsing/other-languages/pascal/turing/translation-to-weblogo/translateTuringToWebLogo.js';
import { translateTurtleBlocksToWebLogo } from
'../../../parsing/other-languages/sugarlabs-turtle-blocks/translation-to-weblogo/translateTurtleBlocksToWebLogo.js';
import { translateTurtleGraphicsFunToWebLogo } from
'../../../parsing/other-languages/turtle-graphics-fun/translation-to-weblogo/translateTurtleGraphicsFunToWebLogo.js';
import { translateTurtleToyNetToWebLogo } from
'../../../parsing/other-languages/turtletoy-net/translation-to-weblogo/translateTurtleToyNetToWebLogo.js';
import { newTranslatePythonCodeToWebLogo } from
'../../../parsing/other-languages/python/newTranslatePythonCodeToWebLogo.js';

const translatorPairs = new Map([
	[isLikely0L, [translate0LToWebLogo, false]],
	[isLikelyAmosBasic, [translateAmosBasicToWebLogo, false]],
	[isLikelyAppleSoftBasic, [translateAppleSoftBasicToWebLogo, false]],
	[isLikelyASMTurtle, [translateASMTurtle, false]],
	[isLikelyBasic256, [translateBasic256ToWebLogo, false]],
	[isLikelyBasicFusion, [translateBasicFusionToWebLogo, false]],
	[isLikelyBasilBasic, [translateBasilBasicToWebLogo, false]],
	[isLikelyBazzBasic, [translateBazzBasicToWebLogo, false]],
	[isLikelyBBCBasic, [translateBBCBasicToWebLogo, false]],
	[isLikelyCanvas2D, [translateCanvas2DToWebLogo, false]],
	[isLikelyCGJennings, [translateCGJenningsToWebLogo, false]],
	[isLikelyCheerfulNetherlandsLogo, [translateCheerfulToWebLogo, false]],
	[isLikelyCodeHeartTurtleScript, [translateCodeHeartTurtleScriptToWebLogo, false]],
	[isLikelyCommodoreBasic, [translateCommodoreBasicToWebLogo, false]],
	[isLikelyFMSLogo, [translateFMSLogo, false]],
	[isLikelyFractInt, [translateFractIntToWebLogo, false]],
	[isLikelyGermanLogo, [germanLogoToWebLogo, false]],
	[isLikelyJavaScriptProcessing, [translateJSProcessingToWebLogo, false]],
	[isLikelyKojo, [translateKojoToWebLogo, false]],
	[isLikelyKTurtle, [translateKTurtle, false]],
	[isLikelyLogo3D, [logo3DToWebLogo, true]],
	[isLikelyLogoInterpreter, [logoInterpreterToWebLogo, false]],
	[isLikelyPapert, [papertToWebLogo, true]],
	[isLikelyPitrifiedGoTurtle, [translatePitrifiedGoTurtleToWebLogo, false]],
	[isLikelyMicroABasic, [translateMicroABasicToWebLogo, false]],
	[isLikelyPBasic, [translatePBasicToWebLogo, false]],
	[isLikelyPovRay, [translatePovRay, false]],
	[isLikelyProcessing, [translateProcessingToWebLogo, false]],
	[isLikelyQBasic, [translateQBASICToWebLogo, false]],
	[isLikelySeaTurtle, [translateSeaTurtleToWebLogo, false]],
	[isLikelySinclairBasic, [translateSinclairBasicToWebLogo, false]],
	[isLikelySonicWebTurtle, [translateSonicWebTurtle, false]],
	[isLikelySugarLabsTurtleBlocks, [translateTurtleBlocksToWebLogo, false]],
	[isLikelySuperLogo, [superLogoToWebLogo, false]],
	[isLikelyTektronix405XBasic, [translateTektronix405XBasicToWebLogo, false]],
	[isLikelyTerrapin, [terrapinToWebLogo, false]],
	[isLikelyTexasInstruments99_4a, [translateTI99BasicToWebLogo, false]],
	[isLikelyTuring, [translateTuringToWebLogo, false]],
	[isLikelyTurtleGraphicsFun, [translateTurtleGraphicsFunToWebLogo, false]],
	[isLikelyTurtleToyNet, [translateTurtleToyNetToWebLogo, false]]
]);
const defaultResult = [(code) => code, false];

export function codeToTranslator(code) {
	code = StringUtils.sanitizeLineBreaks(code);
	let result = defaultResult;
	if (isLikelyPythonCode(code)) {
		result = [newTranslatePythonCodeToWebLogo, false];
	}
	for (const [isFunc, resultPair] of translatorPairs) {
		if (isFunc(code)) {
			/*
			These translators can make a lot of changes to the code.
			It is better to make no changes than make the wrong changes.
			For this reason, if more than 1 translator is matched, do nothing.

			Ideally, only 1 category or classification will be matched so we
			can be pretty confident that the changes are appropriate.
			*/
			if (result !== defaultResult)
				return defaultResult;
			result = resultPair;
		}
	}
	if (result === defaultResult) {
		if (isLikelyBasic(code))
			return [translateQBASICToWebLogo, false];
		if (isLikelyJavaScript(code))
			return [translateJavaScriptToWebLogo, false];
	}
	return result;
};