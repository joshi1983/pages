...
class PrivateCode extends EventDispatcher {
	constructor() {
		super(['run-started']);
		this.sourceCode = '';
	...
	eraseAllGlobalVariablesAndProcedures() {
		if (this.executer !== undefined) {
			const context = this.executer.executionContext;
			if (context !== undefined) {
				if (this.executer._isPaused !== true)
					this.executer.pauseContinuousExecution(true);

				this.executer.halt(); 
				// halt() to make it so the program can't be unpaused.
				// All global variables will be removed which will cause problems if a previously run program is unpaused.

				context.globalVariables.clear();
				// FIXME: how do we remove all procedures?
			}
		}
	}
	...
	run() {
		if (this.refreshProgram(CommandBoxParseLogger)) {
			...
			if (!this.executer.isRunning()) {
				...
				super._dispatchEvent('run-started', {});
			}
		}
	}
