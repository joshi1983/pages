...
class PrivateCode {
			...
			if (!parseLogger.hasLoggedErrors()) {
				...
				if (program.instructions.length === 0) {
					if (program.procedures.size !== 0) {
						...
						parseLogger.warn('At least one procedure was defined but it is not called so there is nothing to run.  You can call the procedure(s) from Commander, though.', tree);
					}
					else
						parseLogger.warn('There are no instructions to run.', tree);
					return false;
				}
				else {
					this.latestProgram = program;
	...
	setSourceCode(sourceCode) {
		if (typeof sourceCode !== 'string')
			throw new Error('setSourceCode requires a string');
