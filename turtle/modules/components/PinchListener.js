...
function touchMove(event, pincher) {
	...
	if (pincher.passiveOptions.passive === false)
...
class Pincher {
	constructor(e, listener, passiveOptions) {
		if (!((e instanceof Element) || (e instanceof Window)))
			throw new Error('e must be an Element or Window.  Not: ' + e);
		if (typeof listener !== 'function')
			throw new Error('listener must be a function.  Not: ' + listener);
		this.e = e;
		this.listener = listener;
		this.touchPoints = [];
		this.passiveOptions = passiveOptions;
		pinchers.push(this);
		this._bind();
	}
	...
	_bind() {
		...
		this.listeners.forEach(function(listener, index) {
			outer.e.addEventListener(eventKeys[index], listener, outer.passiveOptions);
		});
	}
...
export class PinchListener {
	static bind(e, listener, passive) {
		...
		if (typeof passive === 'boolean')
			...
			throw new Error(`passive must be undefined, true, or false but passive = ${passive}`);

		new Pincher(e, listener, passive);
	}

	static unbind(e, listener) {
		for (let i = 0; i < pinchers.length; i++) {
			const pincher = pinchers[i];
