...
import { processAsSwap } from
'./assignment-operators/processAsSwap.js';
...
function processWithPotentialBrackets(processToken, token, result, settings) {
...
export function processAssignmentOperator(processToken) {
	...
	return function(token, result, settings) {
		...
		if (processAsSwap(token, result, settings))
			return;
		...
		if (token.val === '=') {
			if (secondToken !== undefined)
				processToken(secondToken, result, settings);
		}
			...
			result.append(':' + varNameToken.val + ` ${token.val[0]} `);
			if (secondToken !== undefined) {
				processWithPotentialBrackets(processToken, secondToken, result, settings);
			}
		}
		else if (token.val === '**=') {
