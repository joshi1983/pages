...
function getExpectedChildrenLengthForExport(token) {
	if (token.children.length !== 0) {
		const firstChild = token.children[0];
		if (firstChild.type === ParseTreeTokenType.WILDCARD ||
		(firstChild.val === 'as' &&
		firstChild.type === ParseTreeTokenType.BINARY_OPERATOR))
			return 2;
	}
	return 1;
}
...
const typeToFunction = new Map([
	...
	[ParseTreeTokenType.EXPORT, getExpectedChildrenLengthForExport],
