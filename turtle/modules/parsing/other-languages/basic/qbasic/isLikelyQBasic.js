...
// indicators of Atari Turbo Basic XL
...
// indicators of Bazz Basic
/(^|[\r\n])\s*\[[ ]*(inits|main|start|title)[ ]*\]/,
...
// indicators of Basic Fusion
/(^|[\r\n])\s*END[ \t]+OBJECT\s*([\r\n]|$)/i,
/(^|[\r\n])\s*fastgraphics[ \t]*([\r\n]|$)/i, // used in both Basic Fusion and Play Basic
/(^|[\r\n])\s*(FONTDATA|playmusic|playsound|printfont)[ \t]+\d/i,
/(^|[\r\n])\s*SETFONT[ \t]+"/i,
/(^|[\r\n])\s*TRI[ \t]*\(\s*\d+\s*,/i,

// indicators of Basil Basic
/([\r\n]|^)[ \t]*println[ \t]["a-z]/i,
/[ \t=,](AUDIO_CLOSE%|AUDIO_MONITOR\%|DAW_ERR\$)[ \t]*\(/i,
/([\r\n]|^)[ \t]*DIM[ \t]+[a-z_][a-z_0-9]*[@%\$]?[ \t]+AS[ \t]+CLASS[ \t]*\(/i,
/([\r\n]|^)[ \t]*end[ \t]+func[ \t]*([\r\n]|$)/i,
/([\r\n]|^)[ \t]*for[ \t]+each[ \t]+[a-z_]/i,
/([\r\n]|^)[ \t]*}\s*NEXT[ \t]*([\r\n]|$)/i,
/([\r\n]|^)[ \t]*DAW_RESET[ \t]*([\r\n]|$)/i,
...
/(^|[\r\n])[ \t]*([bfw]color)[ \t]+(\d{1,3}|[a-z]+)[ \t]*,/i,
/([\r\n]|^)[ \t]*func[ \t]+[a-z]+\s*\(/i,

// indicators of pBasic
/([\r\n]|^)[ \t]*var[ \t]+[a-z_][a-z_\d]*[$]?[ \t]*:[ \t]*(Bool|Float|Int|String)/i,
/(^|[\r\n])\s*var[ \t]+[a-z_][a-z_\d]*[\$]?/i,
	
// indicators of PlayBasic
/([\r\n]|^)[ \t]*endif[ \t]*([\r\n]|$)/i, // QBASIC uses separate END IF keywords instead of endif.

/(^|[\r\n])[ \t]*Sync[ \t]*([\r\n]|$)/i,
	// used in both Play Basic and Basic Fusion
	// Should not be found in QBasic code.

// indicators of Sinclair Basic
/(\s|^)def\s+fn[ ]+[a-z_]+[%$]?\s*\(/i,
/([\r\n:]|^)[ \t]*([1-9]\d*[ \t]+)?(border|ink|paper)[ \t]+\d+[ \t]*([\r\n:]|$)/i,
...
// indicators of Tandy's TRS-80 BASIC
/(^|[\r\n])[ \t]*\d+[ \t]+PAUSE[ \t]+"/,
...
/([\r\n:]|^)[ \t]*(\d+[ \t]+)?line(\s+-?\d|\s*,)/i,
/([\r\n:]|^)[ \t]*(\d+[ \t]+)?GETKEY[ \t]+[a-z]+\$/i,
/([\r\n:]|^)[ \t]*(\d+[ \t]+)?PLOT[ \t]+[a-z_][a-z_\d]*[ \t]*,[ \t]*[a-z_][a-z_\d]*/i,

// indicators of Visual BASIC
/(^|[\r\n])\s*[a-z_][a-z_\d]*[ \t]*=[ \t]*(InputBox|MsgBox)[ \t]*\(/i,
/(^|[\r\n])\s*(Private|Public)[ \t]+(Function|Sub)[ \t]+[a-z][a-z_\d]*[ \t]*\(/i,
/(^|[\r\n])\s*MsgBox[ \t]+"/i,
];
