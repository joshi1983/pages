...
function isCodeBlockExpectedForNextToken(prev, next) {
	...
	else if (prev.type === ParseTreeTokenType.FOR_EACH) {
		if (lastChild === undefined ||
		lastChild.type === ParseTreeTokenType.NEXT)
			return false;
		if (lastChild.type === ParseTreeTokenType.IN)
			return true;
		return false;
	}
...
function isGoodPrevious(prev, next) {
	...
	const parent = prev.parentNode;
	if (parent === undefined)
		throw new Error(`prev.parentNode must be null or a ParseTreeToken but found ${parent}, prev=${prev}`);
