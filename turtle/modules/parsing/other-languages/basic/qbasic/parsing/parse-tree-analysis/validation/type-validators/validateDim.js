...
export function validateDim(token, parseLogger) {
	...
	if (children.length === 0) {
		...
		first.type !== ParseTreeTokenType.AS)
			parseLogger.error(`Expected a DIM to have a first child of type AS, IDENTIFIER, or SHARED but found ${ParseTreeTokenType.getNameFor(first.type)}`, token);
