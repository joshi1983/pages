...
export function isComplete(token, functionsMap) {
	...
	if (token.type === ParseTreeTokenType.CONST) {
		...
		return false; // might be complete but it depends on the next token.
			// Returning false because we want the next token to handle this unknown case.
	}
	...
	token.type === ParseTreeTokenType.SHARED) {
		if (lastChild !== undefined && lastChild.type === ParseTreeTokenType.AS) {
			if (children.length === 1 || children[children.length - 2].type === ParseTreeTokenType.COMMA)
				return false;
			return true;
		}
	}
