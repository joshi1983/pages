...
function isCompatibleWithArgCount(argCount) {
	if (!Number.isInteger(argCount))
		throw new Error(`argCount must be an integer but found ${argCount}`);
...
export function genericProcessTo(migrationData, shouldBeSkipped) {
	const nameInfoMap = new Map();
	const nameToMatchesMap = new Map();
	if (shouldBeSkipped === undefined)
		shouldBeSkipped = function() {return false;};
	if (typeof shouldBeSkipped !== 'function')
		throw new Error(`shouldBeSkipped must either be undefined or a function but found ${shouldBeSkipped}`);
	...
	return function(scanTokens) {
		for (let i = 0; i < scanTokens.length; i++) {
			if (!shouldBeSkipped(scanTokens, i)) {
