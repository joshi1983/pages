...
export function validateCodeBlock(token, parseLogger) {
	...
	if (parent.type === ParseTreeTokenType.CASE) {
		if (first !== undefined && first.type === ParseTreeTokenType.CURLY_LEFT_BRACKET)
			parseLogger.error(`CODE_BLOCK should not start with { when it is a child of ${ParseTreeTokenType.getNameFor(parent.type)}`, token);
	}
	else if (first !== undefined && first.type === ParseTreeTokenType.CURLY_LEFT_BRACKET) {
		if (children.length === 1) {
			parseLogger.error(`Expected a CODE_BLOCK to generally have at least 2 child tokens but found ${children.length}`, token);
		}
		else if (children.length >= 2 && first !== undefined && first.type === ParseTreeTokenType.CURLY_LEFT_BRACKET) {
