...
const unlikelyFMSLogoRegexes = [
	// comments from a lot of popular programming languages including
	// JavaScript, Java, Scala, Kojo, c, c++, c#.
	// the comment you're reading is an example of such a comment.
	/(^|[\r\n])\s*\/\//,

	/(^|[\r\n])\s*--/, // single line comments from Haskell and Elm
	/(^|[\r\n])\s*{-\|\s/, // multiline comments from Elm
	/(^|[\r\n])[ \t]*[a-z_]+[ \t]+:[ \t]+/i,
		// can match signature statements from Elm such as init : () -> (Model, Cmd Msg)
	
	// indicator of Kojo.
	// FMSLogo supports a repeat command but the curly bracket({) from this pattern
	// would be invalid in FMSLogo.  FMSLogo uses '[' instead of Kojo's '{'.
	// For example, repeat(10) {
	/(^|[\r\n])\s*repeat[ \t]*\([ \t]*\d+[ \t]*\)[ \t]*{/,
	...
	// The comma is not used much in FMS Logo.
