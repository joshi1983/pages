...
import { processReadFromArrayElement } from
'./function-calls/processReadFromArrayElement.js';
...
export function processFunctionCall(token, result) {
	if (processReadFromArrayElement(token, result))
		return;
	...
	if (info !== undefined) {
		...
		if (processSpecialFunctionCall(token, result, info)) {
			return;
		}
	}
	if (info !== undefined &&
