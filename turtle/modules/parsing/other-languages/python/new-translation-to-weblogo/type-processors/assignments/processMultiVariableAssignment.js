...
x, y = 1, 2
*/
export function processMultiVariableAssignment(token, result, cachedParseTree, isLocal, settings) {
	if (processAsSwap(token, result))
		return true;
	...
	for (let i = 0; i < leftChildren.length; i++) {
		...
		result.append(`\n${makeCommand} "${varName} `);
		processToken(rightChildren[i], result, cachedParseTree, settings);
	}
	result.append('\n');
	return true;
};