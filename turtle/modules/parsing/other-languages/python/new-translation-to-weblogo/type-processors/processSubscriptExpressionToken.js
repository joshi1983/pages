...
function processListExpression(subscriptExpressionToken, result, prefix, cachedParseTree, settings) {
	...
	if (listToken.type === ParseTreeTokenType.IDENTIFIER)
		...
		processListLiteralToken(listToken, result, cachedParseTree, settings);
	else {
...
export function processSubscriptExpressionToken(token, result, cachedParseTree, settings) {
	...
	if (isBeingRead(token)) {
		...
		if (sublistInfo !== undefined) {
			...
			processListExpression(token, result, ':', cachedParseTree, settings);
		}
	}
	else {
