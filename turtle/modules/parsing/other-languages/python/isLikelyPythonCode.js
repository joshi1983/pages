...
const unlikelyRegexes = [
	/(^|[\n\r])\s*(function|procedure|sub)\s+[a-z_][a-z_\d]*[ \t]*\(/i,
		// matches code from Basic, JavaScript, Pascal

	// indicators of Gleam
	/(^|[\r\n])\s*import[ \t]+(gleam|gleeunit)[ \t]*([\r\n]|\/[ \t]*[a-z_])/,

	// indicators of JavaScript
	/(^|[\n\r])\s*const\s+[a-zA-Z_][a-zA-Z_\d]*\s*=/,
	/(^|[\n\r])\s*(let|var)\s+[a-zA-Z_][a-zA-Z_\d]*\s*/,
	/(^|[\n\r])\s*\/\//, // single line comments in JavaScript, c, HolyC...
	/(^|[\n\r])\s*\/\*/, // multiple line comments in JavaScript, c, HolyC...

	// indicators of Julia
	/(^|[\n\r])\s*using[ \t]+[A-Z_][a-zA-Z_\d]+\s*[\r\n]/,
	...
	// indicators of Scala and Kojo:
	/(^|[\r\n])\s*def\s+[a-zA-Z_][a-zA-Z_\d]*\s*\([\s*a-zA-Z_][a-zA-Z_\d]*\s*:/,
	/(^|[\r\n])\s*def\s+[a-zA-Z_][a-zA-Z_\d]*\s*\(\s*\)\s*[{=]/,
	/(^|[\r\n])\s*def\s+[a-zA-Z_][a-zA-Z_\d]*\s*=/,
	...
	// indicators of Go
	/(^|[\r\n])\s*import\s+\"fmt"/,
	/(^|[\n\r])\s*func\s+[a-zA-Z_][a-zA-Z_\d]*[ \t]*\(/,

	// indicator of Rust and Gleam
	/(^|[\r\n])[ \t]*(pub[ \t]+)?fn\s+[a-zA-Z_][a-zA-Z_\d]*\s*\(/,
];

const methodCallPattern = /\.[ \t]*[a-zA-Z_][a-zA-Z_0-9]*[ \t]*\(/u;
export function likelyContainsMethodCall(line) {
	return methodCallPattern.test(line);
};

const forInRangePattern = /for\s+[a-zA-Z_][a-zA-Z_0-9]*\s+in\s+range\s*\(/u;
