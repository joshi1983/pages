...
export function ifelse(token, result) {
	...
	if (shouldTranslateToTrinaryOperator(token, instructionsToken1, instructionsToken2)) {
		...
		result.append(' ? ');
		processToken(instructionsToken1, result);
		result.append(' : ');
		processToken(instructionsToken2, result);
		result.append(' ) ');
	}
	else {
		...
		processToken(conditionToken, result);
		result.append(') {\n');
		processTokens(filterBrackets(instructionsToken1.children), result);
		result.append('\n}');
