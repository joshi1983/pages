...
export function compileWhile(parseTreeTokens, procedures, result, logger) {
	...
	const whileInstructionsToken = parseTreeTokens[0];
	const whileToken = whileInstructionsToken.parentNode;
	getInstructionsFromToken(whileInstructionsToken, procedures, result, logger);
	...
	setBreakJumpIndexesTo(result.length, result.length - 1, result, whileToken);
};