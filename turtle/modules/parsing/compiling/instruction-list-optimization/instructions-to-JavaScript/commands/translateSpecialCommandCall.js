...
createPList, createPList2, difference, emptyp, evenp, first, item, list, listp, max, min, not, 
...
export function translateSpecialCommandCall(commandInfo, args, compileOptions) {
	...
	if (compileOptions.forProduction !== true && productionOnlySet.has(methodName))
		return; // indicate unable to translate.

	const func = commandsMap.get(methodName);
	if (func === undefined)
		return;
	return func(args);
};