...
const methodNamesOfInterest = new Map([
	['convertToAlphaColour', convertToAlphaColour],
	['convertToAlphaColourOrTransparent', convertToAlphaColourOrTransparent],
	['convertToColour', convertToColour],
	['convertToColourOrTransparent', convertToColourOrTransparent]
]);
...
function convertToProperty(cacheMap, isNameAvailable, instruction) {
	return function(token) {
		const val = toValue(token);
		const colour = methodNamesOfInterest.get(token.val)(val);
