...
const safeExpressionTypes = new Set([
...
function isSafeExpressionTokenToMerge(token) {
	if (safeExpressionTypes.has(token.type))
		return true;
	if (isVariableAssignmentRightSideToken(token))
		return true;

	/*
	If we're not sure that the state of context.valueStack affects the value token evaluates to,
	we can't safely move token to a previous call to stackValue.push.
	We can't do that safely if it could change the calculated value.
	*/
...
function isSafeToMoveValues(pushToken) {
	...
	for (let i = 0; i < children.length; i++) {
		...
		if (!nonValueTypes.has(child.type)) {
			if (!isSafeExpressionTokenToMerge(child))
				return false;
		}
	}
	return true;
...
export function mergeConsecutiveValueStackPushes(jsCode) {
	...
	for (let i = 0; i < pushes.length; i++) {
		...
		if (push.parentNode !== null) {
			...
			const oldColIndex = lastBracket.colIndex;
			let nextPush;
