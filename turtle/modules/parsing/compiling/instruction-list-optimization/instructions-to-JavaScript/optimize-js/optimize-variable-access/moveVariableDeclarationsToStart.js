...
import { isVariableAssignmentRideSideToken } from '../token-classifiers/isVariableAssignmentRideSideToken.js';
...
function assignTokenToReadJSCode(assignToken) {
	...
	const children = funcCall.children[1].children;
...
function isReadTokenOfInterest(readToken) {
	...
	if (isVariableAssignmentRideSideToken(readRoot) &&
...
export function moveVariableDeclarationsToStart(allTokens, variables) {
	...
	for (let i = 0; i < values.length; i++) {
		...
		if (variable !== undefined && needsEndingMake(variable)) {
			endings.append(getEndingMakeStatement(jsName, variable.name, variable.isAlwaysLocal,
				variable.isAlwaysGlobal, localVariablesDeclarations.length !== 0));
		}
	}
