...
export function binaryOperatorInstructionToJavaScriptInstruction(instruction) {
	...
	return new JavaScriptInstruction('context.valueStack[context.valueStack.length - 2] = context.valueStack[context.valueStack.length - 2] ' + jsOperatorSymbol + ' context.valueStack[context.valueStack.length - 1];\ncontext.valueStack.pop();', instruction.parseTreeToken);
};