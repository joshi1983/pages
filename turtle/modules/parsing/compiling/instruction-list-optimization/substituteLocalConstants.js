...
const mutationCommandNames = mutatingCommandsArray.filter(name => ['localmake', 'make', 'swap'].indexOf(name) === -1);
...
export function substituteLocalConstants(instructions, parameters) {
	...
	const swapVariableNames = getVariablesModifiedByCommand(instructions, 'swap');
	...
	forVariableNames !== undefined) {
		const constantNames = localMakeVariableNames.filter(
			...
			swapVariableNames.indexOf(localVarName) === -1 &&
