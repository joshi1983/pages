...
export function validateClass(token, parseLogger) {
	...
	if (children.length < 2) {
		parseLogger.error(`Expected class token to have at least 2 children but found ${children.length}`, token);
	}
	else {
		...
		if (lastChild.type !== ParseTreeTokenType.CLASS_BODY)
			parseLogger.error(`Expected last child of CLASS to be a CLASS_BODY but got type ${ParseTreeTokenType.getNameFor(lastChild.type)}`, token);
	}
};