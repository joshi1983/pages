...
const noChildDataTypes = new Set([
...
function isArgListForFunctionDefinition(token) {
	if (endsWithCompleteArgList(token)) {
		if (token.type === ParseTreeTokenType.METHOD)
			return true;
...
function isArgListForConstructor(token) {
	return token.type === ParseTreeTokenType.CONSTRUCTOR;
}
...
export function isCodeBlockExpected(token) {
	...
	if (isArgListForCatch(token))
		return true;
	if (isArgListForConstructor(token))
		return true;
	...
	(token.type === ParseTreeTokenType.CURVED_BRACKET_EXPRESSION || noChildDataTypes.has(token.type)))
		return true;
	return false;
};