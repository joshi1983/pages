...
function argCountMatched(i, argCount) {
	const iArgCount = i.argCount;
	if (iArgCount === undefined)
		return true;
	if (Number.isInteger(iArgCount))
		return iArgCount === argCount;
	else {
		return iArgCount.min <= argCount &&
			iArgCount.max >= argCount;
	}
}
...
export class ProcessingMethod {
	static getMethodInfo(name, className, argCount, argTypes) {
		...
		if (info === undefined)
			return;
		info = info.filter(i => argCountMatched(i, argCount));
