...
const typesNotNeedingOriginalString = new Set([
...
export function scanTokenToParseTreeToken(token) {
	...
	let s = token.s;
	if (typesNotNeedingOriginalString.has(type))
		s = undefined;
	return new ParseTreeToken(token.s, token.lineIndex, token.colIndex, type, s);
};