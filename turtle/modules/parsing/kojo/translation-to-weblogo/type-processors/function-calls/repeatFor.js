...
export function repeatFor(token, result, settings) {
	...
	if (isRangeOperator(rangeToken) && rangeToken.children.length === 2) {
		...
		if (finalValueToken.val === 'by' && finalValueToken.children.length === 2) {
			...
			if (rangeToken.val === 'until' && finalValueToken.type === ParseTreeTokenType.NUMBER_LITERAL) {
				// might aswell translate to a single number instead of something like '10 - 1'.
				// We want the translation to look fairly simple and understandable and '9' is simpler than '10 - 1'.
				result.append(` ${valueToLiteralCode(evaluateToken(finalValueToken) - 1)} `);
			}
			else {
				processToken(finalValueToken, result, settings);
				if (rangeToken.val === 'until') {
					result.append(' - 1 ');
				}
			}
		}
