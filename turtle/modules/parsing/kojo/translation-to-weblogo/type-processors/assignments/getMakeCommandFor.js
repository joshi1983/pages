	...
	return function(token) {
		if (token.type === ParseTreeTokenType.IDENTIFIER) {
			...
			if (parent.type === ParseTreeTokenType.ARG_LIST) {
				...
				return grandParent !== null && grandParent.type === ParseTreeTokenType.DEF;
			}
			for (const tokenType of [ParseTreeTokenType.VAR]) {
				const closestToken = getClosestOfType(parent, tokenType);
...
function getVariableDeclaration(token, varName) {
	...
	while (tok !== null) {
		if (tok.type === ParseTreeTokenType.DEF) {
			// look for declarations in DEF ARG_LIST's.
...
export function getMakeCommandFor(token, varName) {
	...
	if (declaration === undefined)
		return 'make';
	const func = getClosestOfType(declaration, ParseTreeTokenType.DEF);
	if (func === null)
		return 'make';
	else
		return 'localmake';
