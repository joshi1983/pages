...
import { setPenFont } from
'./function-calls/setPenFont.js';
import { shouldBeWrappedInPoly } from
'./function-calls/shouldBeWrappedInPoly.js';
...
for (const func of [ArrayBuffer, Color, ofDim, repeat, repeatFor, setPenFont, Vector2D, Vector3D]) {
...
export function processFuncCall(token, result, settings) {
	...
	let wrapWithPoly = shouldBeWrappedInPoly(token);
	if (wrapWithPoly)
		result.append('\npolyStart\n');
	if (info !== undefined) {
		...
		if (info.contextRequired) {
			...
			if (firstChild.type === ParseTreeTokenType.EXPRESSION_DOT_PROPERTY) {
				result.append(' ');
				const contextToken = firstChild.children[0];
				processToken(contextToken, result, settings);
			}
		}
	}
	else {
