...
function isIdentifierCompleteWithNext(prev, next) {
	...
	prevParent.type === ParseTreeTokenType.ASSIGNMENT_OPERATOR)) {
...
function isCompleteProtectedWithNext(prev, next) {
	const children = prev.children;
	if (children.length === 0)
		return false;
...
function isVarCompleteWithNext(prev, next) {
	...
	next.type === ParseTreeTokenType.PROTECTED ||
	...
	const children = prev.children;
	const last = children[children.length - 1];
	if (children.length !== 0 && last.type !== ParseTreeTokenType.COMMA) {
		if (next.type === ParseTreeTokenType.IDENTIFIER)
			return true;
	}
...
const completeCheckers = new Map([
	...
	[ParseTreeTokenType.CLASS, isLastChildType(ParseTreeTokenType.CLASS_OBJECT_BODY)],
	[ParseTreeTokenType.CLASS_OBJECT_BODY, isLastChildType(ParseTreeTokenType.CURLY_RIGHT_BRACKET)],
	...
	[ParseTreeTokenType.OBJECT, isLastChildType(ParseTreeTokenType.CLASS_OBJECT_BODY)],
	...
	[ParseTreeTokenType.TRAIT, isLastChildType(ParseTreeTokenType.CLASS_OBJECT_BODY)],
...
export function isCompleteWithNext(prev, next) {
	...
	if (prev.type === ParseTreeTokenType.ANNOTATION)
		...
		if (next.type === ParseTreeTokenType.CURLY_RIGHT_BRACKET ||
		next.type === ParseTreeTokenType.DEF ||
		next.type === ParseTreeTokenType.FAT_ARROW ||
		next.type === ParseTreeTokenType.RETURN ||
		next.type === ParseTreeTokenType.CURLY_LEFT_BRACKET)
			return true;
	}
		...
		if (next.type === ParseTreeTokenType.CURVED_RIGHT_BRACKET)
			return true;
		if (first !== undefined &&
		first.type !== ParseTreeTokenType.CURLY_LEFT_BRACKET &&
		first.type !== ParseTreeTokenType.COLON)
			return true;
	}
	else if (prev.type === ParseTreeTokenType.CURVED_BRACKET_EXPRESSION) {
		if (prevParent.type === ParseTreeTokenType.ASSIGNMENT_OPERATOR) {
	...
	else if (prev.type === ParseTreeTokenType.PROTECTED) {
		return isCompleteProtectedWithNext(prev, next);
	}
	else if (prev.type === ParseTreeTokenType.TUPLE_LITERAL) {
		if (next.type === ParseTreeTokenType.ASSIGNMENT_OPERATOR)
			return false;		
		if (prevParent.type === ParseTreeTokenType.ASSIGNMENT_OPERATOR) {
			if (next.type === ParseTreeTokenType.FAT_ARROW)
				return false;
		}
	}
