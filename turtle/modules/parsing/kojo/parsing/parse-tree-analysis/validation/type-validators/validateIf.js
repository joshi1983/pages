...
export function validateIf(token, parseLogger) {
	...
	if (parent.type === ParseTreeTokenType.FOR_LOOP_SETTINGS) {
		if (token.children.length !== 1)
			parseLogger.error(`Expected IF to have 1 children when parent is FOR_LOOP_SETTINGS but found ${token.children.length}`, token);
	}
	else if (parent.type === ParseTreeTokenType.ELSE_IF) {
