...
export function validateSub(token, parseLogger) {
	...
	if (children.length === 0) {
		if (parent.type !== ParseTreeTokenType.END_SUB) {
			parseLogger.error(`When a SUB has no children, the parent should be an END_SUB but found parent with type ${ParseTreeTokenType.getNameFor(parent.type)}`, token);
		}
	}
	else {
