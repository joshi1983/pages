	...
	if (children.length < 3) {
		...
		if (children.length >= 4) {
			...
			lastToken.type !== ParseTreeTokenType.ELSE)
				parseLogger.error(`Expected IF token with at least 4 children to have a last token of ELSEIF, ELSE, ENDIF but found ${ParseTreeTokenType.getNameFor(endIfToken.type)}`, endIfToken);
		}
	}
};