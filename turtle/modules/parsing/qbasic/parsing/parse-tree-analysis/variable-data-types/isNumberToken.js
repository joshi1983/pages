...
'_bit', '_byte', '_float', '_integer64',
'double', 'integer', 'long', 'single'
]);
	...
	if (token.type === ParseTreeTokenType.BINARY_OPERATOR) {
		...
		if (variable !== undefined) {
			...
			if (declaration !== undefined) {
				const asToken = declaration.children[0];
				if (asToken !== undefined && asToken.type === ParseTreeTokenType.AS) {
					const typeToken = asToken.children[0];
					if (typeToken !== undefined && typeToken.type === ParseTreeTokenType.IDENTIFIER) {
						const val = typeToken.val.toLowerCase();
						if (numberNames.has(val))
							return true;
						else
							return false;
					}
