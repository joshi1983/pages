...
function isGoodPreviousUp(token, next, functionsMap) {
	...
	parent.type === ParseTreeTokenType.FUNCTION_CALL) {
		...
		const functionName = functionCallToFunctionName(parent);
		const info = QBasicInternalFunctions.getFunctionInfo(functionName, functionsMap);
		if ((info === undefined || info.returnTypes !== null) &&
		firstChild !== undefined &&
		firstChild.val === '(' && lastChild.val === ')')
			return false;
	}
