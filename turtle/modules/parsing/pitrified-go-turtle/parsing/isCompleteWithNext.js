...
function isStructComplete(token) {
	const children = token.children;
	const closingBracketFound = children.some(t => t.type === ParseTreeTokenType.CURLY_RIGHT_BRACKET);
	if (!closingBracketFound)
		return false;

	return token.parentNode.type === ParseTreeTokenType.TYPE;
}

function isVarCompleteWithNext(prev, next) {
	if (next.type === ParseTreeTokenType.SWITCH ||
...
const completeCheckers = new Map([
	...
	[ParseTreeTokenType.CONST_DECLARATION_LIST, isLastChildType(ParseTreeTokenType.CURVED_RIGHT_BRACKET)],
	...
	[ParseTreeTokenType.STRUCT, isStructComplete],
	...
	[ParseTreeTokenType.TYPE, isLastChildType(ParseTreeTokenType.STRUCT)],
]);
...
export function isCompleteWithNext(prev, next) {
	...
	if (prev.type === ParseTreeTokenType.ARRAY_LITERAL) {
		if (next.type === ParseTreeTokenType.CONST ||
		next.type === ParseTreeTokenType.FOR ||
		next.type === ParseTreeTokenType.VAR) {
			return true;
		}		
	}
	else if (prev.type === ParseTreeTokenType.ARRAY_SUBSCRIPTS) {
		if (next.type !== ParseTreeTokenType.SQUARE_LEFT_BRACKET)
			return true;
	}
	...
	else if (prev.type === ParseTreeTokenType.CONST) {
		return isVarCompleteWithNext(prev, next); // VAR and CONST have very similar grammar production rules.
	}
		...
		if (prev.children.length === 3) {
			...
			next.type === ParseTreeTokenType.FOR ||
			next.type === ParseTreeTokenType.IF ||			
			next.type === ParseTreeTokenType.SWITCH ||
			next.type === ParseTreeTokenType.VAR) {
				return true;
			}
		}
	}
	else if (prev.type === ParseTreeTokenType.FUNC) {
	...
	//	return isStructCompleteWithNext(prev, next);
	//}
