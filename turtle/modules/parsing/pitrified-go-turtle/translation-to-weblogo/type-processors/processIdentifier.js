...
import { shouldTranslateToSublist } from './identifiers/shouldTranslateToSublist.js';
import { translateToSublist } from './identifiers/translateToSublist.js';
...
export function processIdentifier(token, result, settings) {
	...
	if (children.length === 0 || isFunctionDefinitionArgument(token))
		...
		if (firstChild.type === ParseTreeTokenType.ARRAY_SUBSCRIPT) {
			...
			if (shouldTranslateToSublist(nonBracketTokens)) {
				translateToSublist(nonBracketTokens, result, settings);
				return;
			}
