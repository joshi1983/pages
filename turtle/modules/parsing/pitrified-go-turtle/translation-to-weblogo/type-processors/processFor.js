...
import { shouldTranslateToDoWhile, translateToDoWhile } from
'./for-loops/translateToDoWhile.js';
...
export function processFor(token, result, settings) {
	...
	if (shouldTranslateToDoWhile(token)) {
		translateToDoWhile(token, result, settings);
	}
	else if (shouldTranslateToForever(children)) {
		result.append('\nforever [\n');
