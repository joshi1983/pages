...
function dumpLine(parseTokens, tokenIndex, result) {
	if (tokenIndex > 0) {
		const prev = parseTokens[tokenIndex - 1];
		const lineIndex = prev.lineIndex;
		let mayNeedSpace = mightNeedSpaceBetweenTokens(prev, parseTokens[tokenIndex]);
		for (; tokenIndex < parseTokens.length && parseTokens[tokenIndex].lineIndex === lineIndex; tokenIndex++) {
...
function processChangedTokens(treeRoot, originalCode) {
	...
	for (let i = 0; i < originalCode.length; i++) {
		...
		if (c === ';') {
			const possibleComment = parseTokens[tokenIndex];
			if (possibleComment !== undefined && possibleComment.type === ParseTreeTokenType.COMMENT)
				tokenIndex++;
			...
			if (index === -1) {
				...
				tokenIndex = dumpLine(parseTokens, tokenIndex, result);
