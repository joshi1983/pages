...
export class CommentDumpingStringBuffer {
	constructor(comments, processCommentToken) {
		if (!(comments instanceof Array))
			throw new Error(`comments must be an Array. Not: ${comments}`);
		if (typeof processCommentToken !== 'function')
			throw new Error(`processCommentToken must be a function but got ${processCommentToken}`);
		this.processCommentToken = processCommentToken;
	...
	insertAtStart(s) {
		this.stringBuffer.insertAtStart(s);
	}
	...
	processAllRemainingComments() {
		...
		for (let i = 0; i < this.comments.length; i++) {
			this.processCommentToken(this.comments[i], this);
		}
