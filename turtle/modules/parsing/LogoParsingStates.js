...
export class LogoParsingStates {
	...
	static canBeUnaryOperator(previousToken, tok, nextToken, nextTokenString) {
		if (tok.type !== ParseTreeTokenType.BINARY_OPERATOR) {
			if (tok.type !== ParseTreeTokenType.UNARY_OPERATOR ||
		...
		if (previousToken) {
			...
			if (previousToken.colIndex === tok.colIndex - 1 && [
				...
				return false;
			if (tok.val === '-' && previousToken.isStringLiteral())
				return true; // We can't subtract from a string value in WebLogo.
				// If the previous token is a string literal, the '-' must be a unary operator.
		}
	...
	static isExpectingProcedureEndOrCommand(tok) {
		...
		if (toToken === null || toToken.children.length !== 3)
			return false;
	...
	static getImpossibleTypes(tok) {
		...
		if (tok.type === ParseTreeTokenType.UNARY_OPERATOR &&
		tok.children.length === 0) {
			result.add(ParseTreeTokenType.BINARY_OPERATOR);
		}
		return result;
	}
