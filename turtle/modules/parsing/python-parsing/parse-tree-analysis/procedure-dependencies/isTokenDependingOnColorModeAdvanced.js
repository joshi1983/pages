...
export function isTokenDependingOnColorModeAdvanced(cachedParseTree, token) {
	...
	if (children.length === 1 && children[0].type === ParseTreeTokenType.ARGUMENT_LIST) {
		// FIXME: clean this up after new python parser replaces DT Python Parser.
		// This if-condition will always be true with the new Python parser so no if-statement should be needed.
		children = children[0].children;
	}
	const childTokens = filterBracketsAndCommas(children);
