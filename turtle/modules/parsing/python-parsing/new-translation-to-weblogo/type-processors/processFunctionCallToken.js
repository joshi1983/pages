...
function handleGroupArgsAsList(token, funcInfo, parameterValueTokens, result, convertProcedureName, cachedParseTree, settings) {
	...
	for (let i = 0; i < parameterValueTokens.length; i++) {
		if (i !== 0)
			result.append(' ');
		processToken(parameterValueTokens[i], result, cachedParseTree, settings);
	}
	result.append(']\n');
}
...
export function processFunctionCallToken(token, result, cachedParseTree, settings) {
	...
	if (useBrackets)
		result.append('(');
	if (processSpecialFunctionCall(token, result, cachedParseTree, settings)) {
		if (useBrackets)
			appendClosingBracket(result);
		return;
	}
	if (processCustomFunctionCall(token, result, cachedParseTree, settings)) {
		if (useBrackets)
			appendClosingBracket(result);
		return;
	}
	...
	if (funcInfo !== undefined && !matchesCustomFunction(functionName, cachedParseTree)) {
		...
		(funcInfo.groupTwoArgsAsList === true && parameterValueTokens.length === 2)) {
			handleGroupArgsAsList(token, funcInfo, parameterValueTokens, result, convertProcedureName, cachedParseTree, settings);
			return;
		}
	...
	for (let i = 0; i < parameterValueTokens.length; i++) {
		...
		if (funcInfo !== undefined && funcInfo.args !== undefined && funcInfo.args.length > i) {
			...
			processArgInfo(argInfo, result, cachedParseTree, settings);
		}
		if (isDefinitelyColorTypeArgument(functionName, i)) {
			processColorValueToken(child, result, cachedParseTree, settings);
		}
