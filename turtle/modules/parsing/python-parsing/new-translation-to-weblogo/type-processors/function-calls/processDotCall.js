...
function processSingleArgument(argToken, result, cachedParseTree, settings) {
	...
	if (types !== undefined) {
		...
		if (exclusiveColorTypes.has(sTypes)) {
			...
			result.append('\npyDot -1 ');
			processToken(argToken, result, cachedParseTree, settings);
			result.append('\n');
			return;
		}
	}
...
export function processDotCall(token, result, cachedParseTree, settings) {
	...
	if (parameterTokens.length === 0) {
		...
		processSingleArgument(parameterTokens[0], result, cachedParseTree, settings);
	}
	else if (parameterTokens.length === 2) {
		result.append('\npyDot ');
		processToken(parameterTokens[0], result, cachedParseTree, settings);
		result.append(' ');
		processToken(getArgumentValueToken(parameterTokens[1]), result, cachedParseTree, settings);
		result.append('\n');
	}
	else
		return false;
