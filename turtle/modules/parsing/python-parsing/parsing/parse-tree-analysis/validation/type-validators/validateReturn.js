...
export function validateReturn(token, parseLogger) {
	...
	if (children.length === 1) {
		...
		if (!mightBeDataValueToken(child)) {
			parseLogger.error(`The child of a RETURN token should be something that can evaluate to a data value.  ` +
	...
	if (closestFuncDef === null)
		parseLogger.error(`A RETURN must be in a FUNCTION_DEFINITION but none were found.`, token);
};