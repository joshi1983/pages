...
export function processAssignmentOperatorToken(token, result, cachedParseTree) {
	if (token.children.length !== 2)
		throw new Error(`Expected 2 children but got ${token.children.length}.  Operator: ${token.val}`);
	result.processCommentsUpToToken(token);
	if (usesSubscript(token.children[0])) {
	...
	if (leftSide.type === ParseTreeTokenType.IDENTIFIER && isSafeWebLogoIdentifier(varName)) {
