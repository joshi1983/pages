...
'../../../parse-tree-analysis/function-definition/getDefaultValuesForParameters.js';
...
export function processCustomFunctionCall(token, result, cachedParseTree) {
	...
	if (functionDefinitions.length === 1) {
		...
		const tokenChildren = filterBracketsAndCommas(token.children);
		...
		for (let i = 0; i < parameters.size; i++) {
			if (i < tokenChildren.length)
				processToken(tokenChildren[i], result, cachedParseTree);
			else {
