...
import { numericValuesToDataType } from './numericValuesToDataType.js';
	...
	*/
	...
	getIntersectionWith(otherType) {
		...
		otherType.name === 'color' || otherType.name === 'alphacolor') {
			...
			const subtypesClone = this.subtypes.deepClone();
			subtypesClone.intersectWith(intTypes);
			if (subtypesClone.isEmpty())
				return null; // indicate no intersection found
			if (subtypesClone.types.size === 1) {
				const type = subtypesClone.types.values().next().value;
				if (typeof type === 'object') {
					if (type.name === 'num' &&
					(type.max < 0 || type.isUnfiniteOnly))
						return null;
					if (type.name === 'num' ||
					type.name === 'int') {
						if (type.min > 255.5)
							...
							if (otherType.name === 'colorlist' ||
							otherType.name === 'alphacolorlist')
								return otherType;

							if (otherType.name === 'color')
								return DataTypes.createFromName('colorlist');

							if (otherType.name === 'alphacolor')
								return DataTypes.createFromName('alphacolorlist');
						}
					}
				}
			}

			return new DataListType(subtypesClone, this.minLen);
		}
		if (otherType.name === 'color')
	...
	tightenForValue(value, extraInfo) {
		const subtypes = new DataTypes(); // empty
		const numbers = value.filter(v => typeof v === 'number');
		if (numbers.length !== 0) {
			subtypes.types.add(numericValuesToDataType(numbers));
		}
		for (const val of value) {
			if (typeof val !== 'number')
				subtypes.addTypes(DataTypes.getTypesCompatibleWithValue(val, extraInfo));
		}
		if (subtypes.isEmpty())
