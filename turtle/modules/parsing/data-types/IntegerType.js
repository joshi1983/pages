...
export class IntegerType extends DataType {
	...
	getIntersectionWith(otherType) {
		if (otherType.name === 'num') {
			...
			const max = Math.floor(Math.min(otherType.max, this.max));
			const min = Math.ceil(Math.max(otherType.min, this.min));
	...
	tightenForValue(value) {
		return IntegerType.tightenForValue(value);
	}
