...
export class ColorStringType extends DataType {
	...
	isProperSubsetOf(otherType) {
		return ['color', 'string'].indexOf(otherType.name) !== -1;
	}
	...
	mayBeCompatibleWith(token) {
		...
		ParseTreeTokenType.LONG_STRING_LITERAL].indexOf(token.type) !== -1)
	...
	getIntersectionWith(otherType) {
		if (['color', 'colorstring', 'string'].indexOf(otherType.name) !== -1)
			return this;
