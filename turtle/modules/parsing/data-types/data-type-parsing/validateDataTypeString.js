...
recognizedNames.push('finite', 'max', 'min', 'minlen', 'returntypes', 'unfinite');
...
export function validateDataTypeString(s) {
	...
	if (s[0] === ':')
		return 'A data type must not start with a colon';
	else if (s[s.length - 1] === ':')
		return 'A data type must end with a colon';
	else if (/\:\d+\:/.test(s))
		return "An integer shouldn't be surrounded by colons";

	const m = /[<>|\(,]\d/.exec(s);
	if (m instanceof Array)
		return `Found numeric digit at an invalid place in ${m[0]}`;
	...
	if (unrecognizedNames.length !== 0) {
		if (originalString !== undefined) {
			try {
				new DataTypes(originalString);
				return; // indicate s is valid.
			}
			catch (e) {
			}
		}
