...
function containsBothFiniteAndUnfiniteNumbers(types) {
	const typesArray = Array.from(types);
	let finiteFound = typesArray.some(t => t.name === 'num' && t.isFiniteOnly);
	let unfiniteFound = typesArray.some(t => t.name === 'num' && t.isUnfiniteOnly);
	return finiteFound && unfiniteFound;
}
...
export function mergeCompositeDataTypes(types) {
	...
	else if (containsBothFiniteAndUnfiniteNumbers(types)) {
		removeTypeNames(['num'], types);
