...
export class DataTypes {
	...
	static {
		DataTypes.typesArray.forEach(function(type) {
	...
	*/
	static contains(dataTypes, otherTypeOrTypes) {
		...
		if (otherTypeOrTypes instanceof DataType && otherTypeOrTypes.name === 'color') {
		...
		if (otherTypeOrTypes instanceof DataType) {
			...
			for (let t of dataTypes) {
				if (otherType.isSubsetOf(t)) {
					return true;
				}
			}
			return false;
		}
		else if (otherTypeOrTypes instanceof Set) {
			...
			throw new Error('otherTypeOrTypes must either be a DataType or a Set containing DataType instances');
		}
	}
	...
	*/
	...
	static getTypesCompatibleWithValue(val, extraInfo) {
		if (extraInfo === undefined)
			extraInfo = {};
		let resultTypes = DataTypes.typesArray.filter(t => t.mayBeCompatibleWithValue(val, extraInfo));
		// remove any supersets.
		resultTypes = resultTypes.filter(t => !resultTypes.some(other => other.isProperSubsetOf(t)));
	...
	getTypesContaining(types) {
		...
		for (const type of this.types) {
			if (DataTypes.contains(type, types))
	...
	optimize() {
		...
		const sortedTypes = Array.from(this.types);

		DataTypes.sortBySetSize(sortedTypes);
		for (let ti in sortedTypes) {
			const t = sortedTypes[ti];
			if (!DataTypes.contains(optimizedTypes, t)) {
				optimizedTypes.add(t);
			}
		}
		if (DataTypes.contains(optimizedTypes, new ColorType())) {
	...
	static sortBySetSize(typesArray) {
		...
		for (let startI = 0; startI < typesArray.length - 1; startI++) {
			for (let i = startI + 1; i < typesArray.length; i++) {
				if (DataTypes.contains(typesArray[i], typesArray[startI])) {
					// swap.
	...
	static stringify(typeSet) {
		...
		if (typeSet instanceof Set)
			...
			throw new Error('DataTypes, Set, or Array expected');
