...
import { DataTypeTokenType } from './data-type-parsing/DataTypeTokenType.js';
...
import { getAttributesFromToken } from './data-type-parsing/getAttributesFromToken.js';
...
import { optimizeIntegersInDataTypeSet } from './optimizeIntegersInDataTypeSet.js';
import { optimizeListsInDataTypeSet } from './optimizeListsInDataTypeSet.js';
import { optimizeCProcsInDataTypeSet } from './optimizeCProcsInDataTypeSet.js';
...
import { removeListsOverlappingColorLists } from './removeListsOverlappingColorLists.js';
...
async function asyncInit() {
	...
	await ColorListType.asyncInit();
	await AlphaColorListType.asyncInit();
	nullType = new NullType();
}
const initPromise = asyncInit();
const keysWithAttributesOrTemplates = new Set([
	'cproc', 'int', 'list', 'num', 'string'
]);
...
export class DataTypes {
	...
	*/
	static createFromName(name) {
		...
		if (name === 'null')
			return nullType;
		else if (!DataTypes.typesMap.has(name)) {
	...
	static getTypesCompatibleWithValue(val, extraInfo) {
		...
		removeListsOverlappingColorLists(resultTypes, val);
	...
	static intersect(typeSet1, typeSet2) {
		...
		let result = new Set();
		for (const t1 of typeSet1) {
			if (typeof t1.getIntersectionWith !== 'function') {
				console.error('t1 = ', t1);
				throw new Error(`t1 must implement getIntersectionWith. t1=${t1}`);
			}
			...
			if (t1Intersection.size !== 0)
				result = DataTypes.union(t1Intersection, result);
		}
		if (result.size > 1) {
			// remove int if there is any num in result.
			if (Array.from(result).some(t => t.name === 'num')) {
				for (const type of Array.from(result)) {
					if (type.name === 'int') {
						result.delete(type);
					}
				}
	...
	mayBeCompatibleWithValue(val) {
		for (let item of this.types)
			if (item.mayBeCompatibleWithValue(val))
				return true;
		return false;
	}
	...
	optimize() {
		...
		if (DataTypes.contains(optimizedTypes, new AlphaColorType())) {
			...
			if (canSimplifyWithColor) {
				colorSubTypeNames.push('int');
				optimizedTypes = new Set(Array.from(optimizedTypes).filter(
					t => colorSubTypeNames.indexOf(t.name) === -1));
				optimizedTypes.add(new ColorType());
			}
		}
		optimizedTypes = removeContainedTypes(optimizedTypes);
		optimizeIntegersInDataTypeSet(optimizedTypes);
		optimizeListsInDataTypeSet(optimizedTypes);
		optimizeCProcsInDataTypeSet(optimizedTypes);
	...
	static parseTokensToDataTypeSet(tokens) {
		...
		return new Set(tokens.map(function(typeToken) {
			...
			else if (keysWithAttributesOrTemplates.has(typeToken.val) ||
			typeToken.val.startsWith('cproc')) {
				const attributes = getAttributesFromToken(typeToken);
				if (typeToken.val.startsWith('cproc')) {
					const returnTypes = attributes.returntypes;
					return CProcType.parseName(typeToken.val, returnTypes);
				}
				else if (typeToken.val === 'int' &&
				(typeof attributes.max === 'number' ||
				typeof attributes.min === 'number')) {
					return new IntegerType(attributes.max, attributes.min);
				}
				const minLen = attributes.minlen;
				if (typeToken.val === 'list') {
					const templates = typeToken.children.filter(t =>
						t.type === DataTypeTokenType.TEMPLATE_EXPRESSION);
					if (templates.length === 1) {
						const subtypes = DataTypes.parseTokensToDataTypeSet(templates[0].children);
						return new DataListType(subtypes, minLen);
					}
					else
						return new DataListType(undefined, minLen);
				}
				else if (typeToken.val === 'num' && (
				attributes.finite === null || attributes.unfinite === null ||
				typeof attributes.min === 'number' ||
				typeof attributes.max === 'number')) {
					return new NumberType(attributes.finite === null,
					attributes.unfinite === null, attributes.min, attributes.max);
				}
				else
					return new StringType(minLen);
			}
			throw new Error(`Unable to get type instance for typeToken.val ${typeToken.val}`);
		}));
	}
