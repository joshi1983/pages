...
function shouldCreateExpressionDot(previousToken) {
	if (previousToken.type === ParseTreeTokenType.CURLY_BRACKET_EXPRESSION &&
	endsWithCurlyRightBracket(previousToken))
		return true;
...
function getGoodPrevious(previousToken) {
	if (previousToken.children.length !== 0) {
		let token = previousToken.children[previousToken.children.length - 1];
