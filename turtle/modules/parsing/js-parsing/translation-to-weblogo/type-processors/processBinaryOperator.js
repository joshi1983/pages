...
function processChildren(processToken, tokens, result, settings) {
...
export function processBinaryOperator(processToken) {
	...
	return function(token, result, settings) {
		...
		if (commandName !== undefined) {
			...
			processChildren(processToken, children, result, settings);
		}
			...
			processChildren(processToken, children.slice(1), result, settings);
			result.append(')');
		}
	};
};