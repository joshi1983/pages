...
import { processSpecialIdentifier } from
'./identifiers/processSpecialIdentifier.js';
...
export function processIdentifier(processToken) {
	...
	return function(token, result, settings) {
		if (processSpecialIdentifier(token, result, settings))
			return;
		...
		if (token.children.length !== 0) {
			if (token.children.length === 1) {
				...
				if (child.type === ParseTreeTokenType.UNARY_OPERATOR) {
					processUnaryOperator(processToken)(child, result, settings);
				}
				else {
					let fullIdentifierPath = token.val;
					for (let tok = child; tok !== undefined; tok = tok.children[0]) {
