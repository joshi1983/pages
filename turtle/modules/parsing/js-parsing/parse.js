...
import { processAsync } from './parsing/processAsync.js';
...
import { processThis } from './parsing/processThis.js';
...
const tokenTypeProcessorsMap = new Map([
	...
	[ParseTreeTokenType.ASYNC, processAsync],
	...
	[ParseTreeTokenType.DELETE, processDelete],
	[ParseTreeTokenType.DOT, processDot],
	[ParseTreeTokenType.ELSE, processElse],
	[ParseTreeTokenType.EXPORT, processExport],
	...
	for (let i = 0; i < nonCommentTokens.length; i++) {
		...
		if (processor !== undefined) {
			...
			if (!addCodeBlockIfNeeded(token, tok)) {
				while (!shouldAppendChild(token, tok))
