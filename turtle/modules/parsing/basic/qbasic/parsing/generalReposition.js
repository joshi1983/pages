...
function repositionAfterDeclaration(prev, next) {
	if (prev.lineIndex !== next.lineIndex) {
		if ((prev.type === ParseTreeTokenType.DIM ||
		prev.type === ParseTreeTokenType.LET ||
		prev.type === ParseTreeTokenType.CONST) && prev.children.length !== 0)
			prev = prev.parentNode;

		while (prev.lineIndex !== next.lineIndex &&
		prev.children.length !== 0 &&
		prev.children[prev.children.length - 1].type !== ParseTreeTokenType.COMMA &&
		(prev.type === ParseTreeTokenType.COMMON ||
		prev.type === ParseTreeTokenType.SHARED)) {
			prev = prev.parentNode;
		}
	}
	return prev;
...
function repositionValueToken(prev, next) {
	if (valueTokenTypes.has(next.type) && prev.children.length !== 0) {
		...
		if (prev.type === ParseTreeTokenType.TREE_ROOT) {
			// look for the highest arg list that is not closed.
			let t = prev;
			while (t !== undefined) {
				const lastChild = t.children[t.children.length - 1];
				if (t.type === ParseTreeTokenType.ARG_LIST) {
					if (lastChild === undefined || lastChild.type !== ParseTreeTokenType.CURVED_RIGHT_BRACKET) {
						prev = t;
					}
					break;
				}
