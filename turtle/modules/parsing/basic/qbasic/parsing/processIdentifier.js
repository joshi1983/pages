...
import { moveNextIfNeeded } from './moveNextIfNeeded.js';
...
'open-output', 'open-random', 'window-screen'
]);
...
function isGoodPrevious(token, next) {
	...
	lastChild.type === ParseTreeTokenType.IDENTIFIER) {
		if (lastChild.val.toLowerCase() === 'seg')
			return false;
	}
	if (token.type === ParseTreeTokenType.DIM && lastChild !== undefined) {
		...
		lastChild.lineIndex !== next.lineIndex)
			return false;
		if (lastChild.type === ParseTreeTokenType.SHARED)
			return false;
	}
	...
	if (token.type === ParseTreeTokenType.SHARED) {
		return lastChild === undefined ||
			lastChild.type === ParseTreeTokenType.AS ||
			lastChild.type === ParseTreeTokenType.COMMA;
	}
...
function shouldAddArgList(prev, next) {
	if (prev.type === ParseTreeTokenType.FUNCTION ||
		prev.type === ParseTreeTokenType.SUB)
		return true;
	if (prev.type === ParseTreeTokenType.DEF)
		return next.val.toLowerCase() !== 'seg';
	return false;
}
