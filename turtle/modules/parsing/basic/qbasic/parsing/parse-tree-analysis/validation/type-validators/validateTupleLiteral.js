...
export function validateTupleLiteral(token, parseLogger) {
	...
	if (children.length < 2) {
		...
		if (last.type !== ParseTreeTokenType.CURVED_RIGHT_BRACKET)
			parseLogger.error(`Expected a TUPLE_LITERAL to have a last child of ) and a type of CURVED_RIGHT_BRACKET but found val of ${last.val} and type of ${ParseTreeTokenType.getNameFor(last.type)}`, token);
	}
