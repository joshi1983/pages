...
export function validateDef(token, parseLogger) {
	...
	if (parent.type === ParseTreeTokenType.END_DEF) {
		...
		if (token.children.length === 0 && token.parentNode.type !== ParseTreeTokenType.EXIT)
			parseLogger.error(`Expected DEF without a parent of END_DEF or EXIT to have at least 1 child but found none`, token);
