...
function isAdjacentTokensOfInterest(scanTokens, i, customFunctionNames) {
	...
	for (const operandToken of [first, second]) {
		const s = operandToken.s.toLowerCase();
		if (!customFunctionNames.has(s)) {
			const info = QBasicInternalFunctions.getFunctionInfo(s);
			if (info !== undefined && info.returnTypes === null) {
				// inserting a * between these would make the code less understandable
				// if one of the operands can't return any values.
				// For example, "x * print" makes no sense because print can't return anything.
				return false;
			}
		}
	}

	const secondS = second.s.toLowerCase();
	if (['dim', 'for', 'next', 'step', 'then', 'to'].some(keyword => secondS.startsWith(keyword)))
		return false;
		// avoid inserting * if the second token has a good chance of splitting into a keyword.
