...
'../../parsing/parse-tree-analysis/isCustomFunctionOrSub.js';
...
function shouldProcessSemicolons(argList, options) {
	if (skipSemicolonProcessing(argList))
		return false;

	if (options.skipStringArgumentChecksFor !== undefined) {
		if (options.skipStringArgumentChecksFor(argList) === true)
			return false;
	}
	...
	if (isCustomFunctionOrSub(getTreeRoot(argList), nameToken.val))
		return false;
...
export function processArgList(token, result, options) {
	...
	if (shouldProcessSemicolons(token, options)) {
		// look for semicolons.
