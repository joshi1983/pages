...
function shouldWrapTokenInBrackets(token) {
	...
	if (ParseTreeTokenType.BINARY_OPERATOR === parent.type) {
		...
		shouldWrapTokenInBrackets(parent))
			return true;
	}
	return false;
}
...
export function processFunctionCall(token, result, options) {
	...
	let wrapAllParametersWithSquareBrackets = false;
	let wrapInBrackets = shouldWrapTokenInBrackets(token);
	if (info !== undefined) {
		if (info.wrapAllParametersWithSquareBrackets === true)
			wrapAllParametersWithSquareBrackets = info.wrapAllParametersWithSquareBrackets;
