...
function translateCondition(selectValueToken, token, result, options) {
	...
	function conditionalProcessToken(tok) {
		if (tok.type === ParseTreeTokenType.IDENTIFIER &&
		tok.val.toLowerCase() === 'is')
			processToken(selectValueToken, result, options);
		else
			processToken(tok, result, options);
	}
	...
	for (let i = 0; i < codeBlockIndex; i++) {
		...
		if (isToToken(child)) {
			...
			if (child.type !== ParseTreeTokenType.BINARY_OPERATOR ||
			!comparisonOperators.has(child.val)) {
				processToken(selectValueToken, result, options);
				result.append(' = ');
				...
				if (operands.length === 2) {
					conditionalProcessToken(left);
					result.append(` ${child.val} `);
					conditionalProcessToken(right);
				}
			}
			result.append(' ');
		}
