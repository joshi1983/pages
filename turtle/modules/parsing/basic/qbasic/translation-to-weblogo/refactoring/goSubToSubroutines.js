	...
	if (gosubs.length !== 0) {
		...
		gosubs.forEach(function(gosubToken) {
			...
			if (labelToken !== undefined) {
				...
				gosubToken.val = null;
				const argList = new ParseTreeToken(null, child.lineIndex, child.colIndex + 1,
					ParseTreeTokenType.ARG_LIST);
				argList.appendChild(new ParseTreeToken('(', argList.lineIndex, argList.colIndex,
					ParseTreeTokenType.CURVED_LEFT_BRACKET));
				argList.appendChild(new ParseTreeToken(')', argList.lineIndex, argList.colIndex,
					ParseTreeTokenType.CURVED_RIGHT_BRACKET));
				gosubToken.appendChild(argList);
			}
		});
	}
};