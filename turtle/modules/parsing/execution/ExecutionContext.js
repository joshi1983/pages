...
export class ExecutionContext {
	...
	// called to help the browser free memory
	disconnect() {
		this.globalVariables = undefined;
		this.procedures = undefined;
		this.instructions = undefined;
		this.procedureStack = undefined;
		this.repcountStack = undefined;
		this.forcountStack = undefined;
		this.valueStack = undefined;
		this.logoProgram = undefined;
	}
	...
	repRatio() {
		const lastRepcount = this.repcountStack[this.repcountStack.length - 1];
		if (lastRepcount.max <= 1)
			return 0;
		return (lastRepcount.current - 1) / (lastRepcount.max - 1);
	}

	swap(varName1, varName2) {
		varName1 = varName1.toLowerCase();
		varName2 = varName2.toLowerCase();
		const value1 = this.readVariable(varName1);
		const value2 = this.readVariable(varName2);
		this.make(varName1, value2);
		this.make(varName2, value1);
	}
};