...
export class LogoProgramExecuter extends EventDispatcher {
	...
	disconnect() {
		...
		this.instructionExecutionCount = undefined;
		this.executionContext.disconnect();
	...
	executeInstruction() {
		...
		try {
			if (instruction === undefined) {
				this.pauseContinuousExecution(true);
				this.halt();
	...
	*/
	executeInstructionAsync() {
		...
		try {
			if (instruction === undefined) {
				this.pauseContinuousExecution(false);
				this.halt();
				// false because we don't want to dispatch execution-stopped twice.
				super._dispatchEvent('execution-stopped', {'cause': 'halted-normal'});
			}
				...
				result.catch(function(e) {
					outer.pauseContinuousExecution(true);
					outer.halt();
	...
	*/
	...
	// In other words, halt.
	pauseContinuousExecution(dispatchStopped) {
		this._isPaused = true;
