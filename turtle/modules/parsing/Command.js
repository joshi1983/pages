...
export class Command {
	...
	static getLowerCaseCommandNameSet(commandInfo) {
		...
		return new Set([commandInfo.primaryName.toLowerCase(), ...commandInfo.names]);
	}
	...
	static getParameterSanitization(commandInfo, parameterIndex) {
		if (typeof commandInfo === 'string')
			commandInfo = Command.getCommandInfo(commandInfo);
		if (parameterIndex >= 0 && parameterIndex < commandInfo.args.length)
			return commandInfo.args[parameterIndex].sanitization;
	}
	...
	static getParameterTypes(commandInfo, parameterIndex) {
		...
		if (parameterIndex >= 0 && parameterIndex < commandInfo.args.length)
			...
			return 'bool|gradient|list|num|plist|string|transparent'; // return all types
	}
};