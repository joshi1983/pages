...
export class WriteOptimizedCachedParseTree {
	...
	_refreshProceduresMap() {
		if (this.proceduresMap !== undefined) {
			this.proceduresMap = getProceduresMap(this.root);
		}
	}
	...
	_removedTokenFromTokensByType(token, type) {
		...
		if (index === -1) {
			throw new Error(`token not found. token.val=${token.val}. type=${ParseTreeTokenType.getNameFor(token.type)}`);
		}else
	...
	tokenRemoved(removedToken) {
		...
		if (index === -1)
			...
			if (removedToken.type === ParseTreeTokenType.PROCEDURE_START_KEYWORD) {
				this._refreshProceduresMap();
			}
		}
	}
	...
	tokensRemoved(removedTokens) {
		...
		if (removedTokens.some(t => t.type === ParseTreeTokenType.PROCEDURE_START_KEYWORD)) {
			this._refreshProceduresMap();
		}
