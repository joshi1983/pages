...
function validateUnaryTokenBasics(token) {
...
function evaluateParameterizedGroup(pgToken, proceduresMap) {
	...
	if (!info.isStaticEvaluationSafe)
		return undefined;
	let childVals = pgToken.children.map(t => evaluateToken(t, proceduresMap));
	if (childVals.some(v => v === undefined || v === null))
		return undefined;
	childVals = convertArgsUsingArgsInfo(childVals, info.args);
