...
import { getDataTypesForListTokenBasic } from './token-evaluation/getDataTypesForListTokenBasic.js';
...
export function getTokenTypesBasic(token, isStrict, extraInfo) {
	...
	if (token.type === ParseTreeTokenType.BINARY_OPERATOR) {
		if (isStrict && token.children.length === 2 &&
		(OperatorDataTypes.isReturnTypesAffectedByBinaryInputTypes(token.val) ||
		token.val === '/'))
		...
		if (isStrict && token.children.length === 1 &&
		OperatorDataTypes.isReturnTypesAffectedByUnaryInputTypes(token.val))
		...
		info !== undefined && info.primaryName === 'repcount'))
		...
		return getDataTypesForListTokenBasic(token, isStrict, extraInfo);
	}
