...
'./variable-assignment-scopes/analyzeTokenTypesForProcedureCPROCs.js';
import { canTypesBeEvaluatedFromPartialInputs } from
'../command-data-types/canTypesBeEvaluatedFromPartialInputs.js';
...
export function analyzeTokenDataTypes(cachedParseTree, tokenValueMap, variables) {
	...
	tokensRemaining.forEach(function(token) {
		...
		if (token.type === ParseTreeTokenType.PARAMETERIZED_GROUP) {
			...
			if (info !== undefined) {
				const childTypes = token.children.map(c => result.get(c));
				const knownTypes = childTypes.map(c => c !== undefined);
				if (canTypesBeEvaluatedFromPartialInputs(info.primaryName, knownTypes)) {
					const typesStr = CommandDataTypes.getReturnDataTypesFromInputs(info.primaryName, childTypes);
					if (typesStr === null)
						types = null;
					else
						types = new DataTypes(typesStr);
				}
			}
		}
		if (types === undefined)
			types = getTokenTypesBasic(token, false, extraInfo);
		if (types !== undefined)
			result.set(token, types);
	});
