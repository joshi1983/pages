...
function addAllDescendingTokens(tokenSet, token) {
	tokenSet.add(token);
	for (let i = 0; i < token.children.length; i++) {
		const child = token.children[i];
		addAllDescendingTokens(tokenSet, child);
	}
}

export function analyzeVariableScopeApplicableTokens(variables) {
	...
	applicableVariables.forEach(function(variable) {
		...
		scopes.forEach(function(scope) {
			let token = scope.fromToken;
			if (isAfterOrSame(scope.assignToken, scope.fromToken))
				token = scope.assignToken.nextSibling;
			for (;
			token !== null && !isTerminating(token, variable.name);
			token = token.nextSibling) {
				addAllDescendingTokens(scope.applicableTokens, token);
			}
		});
	});
};