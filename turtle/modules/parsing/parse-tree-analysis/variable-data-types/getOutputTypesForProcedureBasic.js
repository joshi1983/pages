...
export function getOutputTypesForProcedureBasic(procedure, tokenToTypes) {
	...
	if (startToken === undefined) // might happen with invalid WebLogo code like 'to print\nend'.
		return new DataTypes();

	const outputCalls = CommandCalls.filterCommandCalls(getAllDescendentsAsArray(startToken), 'output');
