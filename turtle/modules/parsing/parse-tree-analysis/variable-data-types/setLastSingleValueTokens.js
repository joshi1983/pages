...
function isSafeForCommandCallToMutationPoint(callToken) {
	...
	const callChild = callToken.children[index];
	if (callChild === undefined)
		throw new Error(`Unable to get child at index ${index}.  length=${callToken.children.length}, info.primaryName=${info.primaryName}`);

	const tokenVal = callChild.val;
...
function commandCallToMutationPoint(callToken) {
	...
	const token = getLastDescendentTokenOf(callToken);
