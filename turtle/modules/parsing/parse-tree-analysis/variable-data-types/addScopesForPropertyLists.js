...
function isSetPropertyCallInRange(fromToken, toToken, plistVariableName, cachedParseTree) {
	...
	const fromIndex = cachedParseTree.getSortedTokenIndex(fromToken);
	const toIndex = cachedParseTree.getSortedTokenIndex(toToken);
...
export function addScopesForPropertyLists(result, cachedParseTree, tokenValueMap) {
	const plistCreateCalls = cachedParseTree.getCommandCallsByName('plistCreate').
	...
	plistCreateCalls.forEach(function(plistCreateCall) {
		const makeToken = plistCreateCall.parentNode;
		const variableName = makeToken.children[0].val.toLowerCase();
		const variable = result.getVariableByName(variableName);
		const procedure = getProcedureFromAnyTokenInProcedure(plistCreateCall);
		let scopes = variable.getScopesAt(makeToken.nextSibling, procedure);
		if (scopes.length > 0) {
			let n = plistCreateCall.parentNode.nextSibling;
				...
				if (scopes.length === 1) {
					...
					if (!isSetPropertyCallInRange(n, scope.toToken, variableName, cachedParseTree)) {
						if (n === scope.fromToken) // identical interval so no need to make new scope.
							...
							const newScope = new VariableScope(n.previousSibling, n, scope.toToken, plistTypes, scope.requiredTypes,
