...
function getTypesFromParameterizedGroup(conditionToken, variableName) {
	...
	if (info !== undefined) {
		if (typePredicateMap.has(info.primaryName)) {
			const firstChild = conditionToken.children[0];
			if (conditionToken.children.length === 1 &&
		firstChild.type === ParseTreeTokenType.VARIABLE_READ &&
		firstChild.val.toLowerCase() === variableName)
				return new DataTypes(typePredicateMap.get(info.primaryName));
		}
