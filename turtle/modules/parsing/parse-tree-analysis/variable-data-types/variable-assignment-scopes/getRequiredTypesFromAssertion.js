...
function handleDataType(typeName) {
	return function(token, cachedParseTree, tokenToDataTypes, tokenValuesMap, result) {
		handleSingleTypeCheck(token, typeName, result);
	};
}
...
for (const [key, val] of typePredicateMap) {
	mapBase.set(key, handleDataType(val));
}
const map = new Map();
...
*/
export function getRequiredTypesFromAssertion(assertToken, cachedParseTree, tokenToDataTypes,
tokenValuesMap) {
