...
function isSetPropertyCallInRange(fromToken, toToken, plistVariableName, cachedParseTree) {
	...
	for (let i = fromIndex; i < toIndex; i++) {
		...
		CommandCalls.tokenMatchesPrimaryName(token, 'setProperty2')))
			return true;
	}
	return false;
}
...
export function addScopesForPropertyLists(result, cachedParseTree, tokenValueMap) {
	...
	createPListCalls.forEach(function(createPListCall) {
		...
		if (scopes.length > 0) {
			...
			let giveUp = false;
			for (; CommandCalls.tokenMatchesPrimaryName(n, 'setProperty') ||
			CommandCalls.tokenMatchesPrimaryName(n, 'setProperty2'); n = n.nextSibling) {
