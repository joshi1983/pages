...
export class VariableAssignmentScope {
	...
	*/
	...
	/*
	from should be a Token or ParseTreeToken.
	to should be a Token or ParseTreeToken.
	procedure should be a Procedure.
	*/
	constainsInRange(location, from, to, procedure) {
		if (this.procedure === undefined) {
			if (procedure === undefined)
				return compareTokenLocations(from, location) <= 0 &&
					compareTokenLocations(to, location) >= 0;
			else {
				if (procedure.parameters.indexOf(this.variable.name) !== -1)
					return false;
				return true;
			}
		}
