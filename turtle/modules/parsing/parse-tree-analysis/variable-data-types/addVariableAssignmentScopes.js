...
export function addVariableAssignmentScopes(cachedParseTree, variables) {
	...
	const nonMakeTokens = tokens.filter(tok => !CommandCalls.tokenMatchesPrimaryName(tok, 'make'));
	const makeTokens = tokens.filter(tok => CommandCalls.tokenMatchesPrimaryName(tok, 'make'));
