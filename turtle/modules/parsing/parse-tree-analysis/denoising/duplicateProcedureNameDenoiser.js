...
export function duplicateProcedureNameDenoiser(cachedParseTree, parseMessages) {
	...
	if (duplicateMessages.length !== 0) {
		const procs = cachedParseTree.getTokensByType(ParseTreeTokenType.PROCEDURE_START_KEYWORD).map(tokenToProcedure);
		...
		const newMessages = parseMessages.filter(m => !messagesToRemove.has(m));
