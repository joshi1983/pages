...
const supportedProtocolsArray = ['data', 'http', 'https', 'local', 'weblogo'];
...
export function absoluteUrl(s, extendedChecks) {
	...
	if (!s.toLowerCase().startsWith('data') && (s.indexOf('://') === -1 || s.indexOf('://') > 7))
	...
	if (colonIndex !== -1)
		return `Too many slashes after protocol in URL: ${s}`;
}