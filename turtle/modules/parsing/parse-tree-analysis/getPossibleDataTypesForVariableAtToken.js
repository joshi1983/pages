...
function removeSubtypes(dataTypes) {
	for (const type of dataTypes.types) {
		if (type.name === 'list')
			type.subtypes = undefined;
	}
}
...
export function getPossibleDataTypesForVariableAtToken(token, varName, proceduresMap, varNamesNotToCheck) {
	...
	/*
	Remove subtypes from list because we can't rule out a mutation of the originally assigned value. 
	getAnalyzedVariables and a CachedParseTree is recommended whenever possible so the types are as tight as possible.
	*/
	removeSubtypes(result);
	return result;
};