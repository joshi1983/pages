...
import { isUnconditionallyExecutedBefore } from './poly-command-usage/isUnconditionallyExecutedBefore.js';
import { pathCommandNamesStr } from '../pathCommandNames.js';
import { ParseTreeTokenType } from
'../../ParseTreeTokenType.js';
import { validatePolyStartCalledTwice } from './poly-command-usage/validatePolyStartCalledTwice.js';
import { validatePolyEndCalledTwiceInvolvingProcedure } from
'./poly-command-usage/validatePolyEndCalledTwiceInvolvingProcedure.js';
		...
		polyEndCalls.forEach(function(polyEndCall) {
			...
			for (let i = endIndex - 1; i >= 0; i--) {
				...
				if (token.type !== ParseTreeTokenType.PARAMETERIZED_GROUP)
					continue;
				if (CommandCalls.tokenMatchesPrimaryNames(token, ['stop', 'output']))
					break;
				// stop and output can terminate a procedure so stop looking for ways polyEnd could be called without corresponding polyStart.

				if (CommandCalls.tokenMatchesPrimaryName(token, 'polyEnd') && isUnconditionallyExecutedBefore(token, polyEndCall)) {
					...
					if (!procedureFound && !loopFound) {
						const isSingleEnough = tokensBetween.length === 1 && isSingleCallEnoughForFillablePath(tokensBetween[0], polyEndCall);
