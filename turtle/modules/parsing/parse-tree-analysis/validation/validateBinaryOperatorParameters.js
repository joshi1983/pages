...
// Assumes the returned value is not used as HTML
// In other words, the returned value can be used as innerText but not escaped properly for HTML.
	...
	if (info === undefined)
		...
		for (let i = 0; i < 2; i++) {
			...
			if (!requiredTypes.hasIntersectionWith(actualTokenTypes)) {
				...
				return prefix + ' operand for operator ' + token.val + ' must be of type ' + typeString + '.  The specified types were determined to be '
				+ dataTypesToEnglish(actualTokenTypes, false);
				// passing false to dataTypesToEnglish because the message is never shown as HTML
			}
		}
	}
}
