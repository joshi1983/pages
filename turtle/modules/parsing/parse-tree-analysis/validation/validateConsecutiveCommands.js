import { areChildrenIndependentlyUseful } from
'../isIndependentlyUseful.js';
import { Command } from
'../../Command.js';
import { CommandCalls } from '../CommandCalls.js';
import { getDescendentsOfType } from
'../../generic-parsing-utilities/getDescendentsOfType.js';
import { isFirstLevelInstruction } from
'../isFirstLevelInstruction.js';
...
const namesMaybeHidingPen = new Set([
	'setPenBlendMode', 'setPenColor', 'setPenGradient', 'setPenSize'
]);

export { namesMaybeHidingPen, positionRelatedCalculatingCommandsArray, positionRelatedCommands };
...
function hidesPen(call) {
	const info = Command.getCommandInfo(call.val);
	if (info === undefined)
		return MaybeDecided.Maybe; // procedure calls might generally cause changes to pen visibility.
		// We're not looking more closely to be sure.
	
	if (info.primaryName === 'penUp')
		return MaybeDecided.Yes;

	if (namesMaybeHidingPen.has(info.primaryName))
		return MaybeDecided.Maybe;

	return MaybeDecided.No;
}

function penMightBeVisibleAt(token) {
	token = token.previousSibling;
	while (token !== null) {
		const results = getDescendentsOfType(token, ParseTreeTokenType.PARAMETERIZED_GROUP).map(hidesPen);
		if (results.some(r => r === MaybeDecided.Yes))
			return false;
		if (results.some(r => r === MaybeDecided.Maybe))
			break;

		const prev = token.previousSibling;
		if (prev === null && token.parentNode.type === ParseTreeTokenType.TREE_ROOT)
			return true;
...
export function validateConsecutiveCommands(cachedParseTree, parseLogger) {
	...
	const filteredCommandCalls = commandCalls.
		filter(function(token) {
			...
			info.primaryName === 'jumpTo') {
				if (polyStarts.length !== 0 && !isAfterPolyEnd(token))
					return false;

				if (info.primaryName === 'setPos' && penMightBeVisibleAt(token))
					return false;
			}
			return true;
		});
