		...
		if (shapeInfo !== undefined) {
			if (shapeInfo.length !== 3)
				...
				if (!(arcLinesInfo instanceof Array))
					parseLogger.error(`Arclines data must be a list.  drawArcLineShape will fail with an error if any of its shapes don't contain a path.  Learn more by clicking <span class="command">drawArcLineShape</span>.`, token, true);
