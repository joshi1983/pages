...
export function validateIfElseExpressionDataTypes(cachedParseTree, parseLogger) {
	...
	ifelses.forEach(function(ifelseExpressionToken) {
		...
		if (child1Types !== undefined) {
			if (!requiredDataTypes.hasIntersectionWith(child1Types))
				parseLogger.error(`The corresponding ifelse has acceptable types ${dataTypesToEnglish(DataTypes.stringify(requiredDataTypes))} but the first ifelse-case's types were found to be ${dataTypesToEnglish(child1Types)}`, child1, false);
		}
		if (child2Types !== undefined) {
			if (!requiredDataTypes.hasIntersectionWith(child2Types))
				parseLogger.error(`The corresponding ifelse has acceptable types ${dataTypesToEnglish(DataTypes.stringify(requiredDataTypes))} but the second ifelse-case's types were found to be ${dataTypesToEnglish(child2Types)}`, child2, false);
		}
	});
};