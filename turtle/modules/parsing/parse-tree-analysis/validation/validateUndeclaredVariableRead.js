...
'./undefined-variables/getVariableReferencesNotInitiallyDefined.js';
...
export function validateUndeclaredVariableRead(cachedParseTree, parseLogger) {
	const initUndefinedTokens = getVariableReferencesNotInitiallyDefined(cachedParseTree);
	const initUndefinedTokenSet = new Set(initUndefinedTokens);
	let variableRefTokens = getTokensByType(cachedParseTree, ParseTreeTokenType.VARIABLE_READ).filter(function(token) {
		if (initUndefinedTokenSet.has(token))
			return false;
		return !Procedure.isParameterToken(token);
	});
	ArrayUtils.pushAll(variableRefTokens, getVariableReferenceTokens(cachedParseTree));
	initUndefinedTokens.forEach(function(token) {
		parseLogger.error(`Variable "${token.val}" not assigned a value before being read`, token, false);
	});
