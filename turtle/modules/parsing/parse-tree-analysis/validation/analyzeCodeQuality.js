...
import { validateAlwaysReturnValue } from './validateAlwaysReturnValue.js';
...
import { validateUnaryOperators } from './validateUnaryOperators.js';
...
export function analyzeCodeQuality(tree, parseLogger, proceduresMap, initialVariablesMap, options) {
	if (proceduresMap !== undefined && !(proceduresMap instanceof Map))
		throw new Error(`proceduresMap must be a Map or undefined but found ${proceduresMap}.`);
	if (initialVariablesMap !== undefined && !(initialVariablesMap instanceof Map))
		throw new Error(`initialVariablesMap must be a Map or undefined but found ${initialVariablesMap}.`);
		
	let runExtraValidators;
	let isCompleteProgram;
	if (options !== undefined) {
		if (typeof options !== 'object')
			throw new Error(`options must either not be specified or be an object but found ${options}`);

		runExtraValidators = options.runExtraValidators;
		isCompleteProgram = options.isCompleteProgram;
	}
	...
	for (const validator of validators2)
		validator(cachedParseTree, parseLogger, options);
	if (isCompleteProgram === true)
		validatePenUp(cachedParseTree, parseLogger, options);
