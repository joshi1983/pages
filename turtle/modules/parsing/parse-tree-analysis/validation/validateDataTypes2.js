	...
	forEach(function(scope) {
		...
		if (scopes.length === 1 && scope.procedure === procedure) {
			...
			if (intersectedTypes.isEmpty())
				parseLogger.error(`The value assigned to ${scope.variable.name} doesn't match any acceptable data type.  The types that won't lead to an error were narrowed down to ${dataTypesToEnglish(scope.requiredTypes)}.`, scope.assignToken);
		}
	});
};