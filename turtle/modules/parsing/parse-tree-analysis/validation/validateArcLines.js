...
export function validateArcLines(cachedParseTree, parseLogger) {
	const callsOfInterest = cachedParseTree.getCommandCallsByName('arcLines').
	...
	callsOfInterest.forEach(function(token) {
		...
		if (infoVal.length === 0)
			...
			parseLogger.warn( `Nothing will be drawn based on the specified arc lines data.  Consider removing the call to <span class="command">arcLines</span> or changing its data to draw something.`, childOfInterest, true);
		else
