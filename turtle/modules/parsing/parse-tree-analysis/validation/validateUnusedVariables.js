...
import { getLastPossibleToToken } from '../variable-data-types/variable-assignment-scopes/getLastPossibleToToken.js';
...
function isScopeOfInterest(varReadTokens, lastToken, proceduresStrictlyFromTree) {
	return function(scope) {
		...
		let toIndex = Math.min(varReadTokens.length - 1,
			binarySearch(varReadTokens, compareParseTokens, toToken, true));
		const from = scope.fromToken;
		for (let i = fromIndex; i <= toIndex; i++) {
			...
			if (varName === scope.variable.name) {
				...
				if (scope.isParameter === false &&
				isStrictlyVariableReadToken(tokenPair.token) === false)
					continue;
				if (scope.contains(tokenPair.token, tokenPair.procedure) ||
				scope.constainsInRange(tokenPair.token, from, toToken, tokenPair.procedure))
					return false;
			}
		}
		return true;
