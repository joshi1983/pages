			...
			if (token.children.length > index) {
				...
				if (typeof v === 'number' && !isNaN(v)) {
					...
					if (arg.min !== undefined && arg.min > v)
						parseLogger.error('The minimum is ' + arg.min + ' but you are inputting ' + v + extra, token.children[index]);
					if (arg.max !== undefined && arg.max < v)
						parseLogger.error('The maximum is ' + arg.max + ' but you are inputting ' + v + extra, token.children[index]);
				}
			}
		});
	});
