				...
				shapesInfo.forEach(function(shapeInfo, index) {
					if (shapeInfo.length !== 3)
						...
						if (arcLinesInfo.length === 0)
							parseLogger.error(`Empty arclines data found at item index ${index + 1}.  drawArcLines will fail with an error if any of its shapes don't contain a path.  Learn more by clicking <span class="command">drawArcLineShapes</span>.`, token, true);
