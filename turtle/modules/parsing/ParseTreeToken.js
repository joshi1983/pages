...
export class ParseTreeToken {
	...
	static createFromScannedToken(token, impossibleTypes) {
		...
		if (ParseTreeTokenType.NUMBER_LITERAL !== type)
	...
	toString() {
		...
		else if (this.type === ParseTreeTokenType.BOOLEAN_LITERAL)
			return "" + this.val;
		else if (this.type === ParseTreeTokenType.NUMBER_LITERAL) {
			if (this.originalString !== undefined && parseFloat(this.originalString) === this.val)
