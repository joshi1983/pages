...
export class ParseTreeToken {
	...
	constructor(val, parentNode, lineIndex, colIndex, type, originalString) {
		...
		if (typeof type !== 'number')
			throw new Error('type must be a number. type specified as ' + type + ' when val specified as ' + val);
	...
	appendChild(nextChild) {
		if (!(nextChild instanceof ParseTreeToken))
			throw new Error('nextChild must be a ParseTreeToken');
		if (nextChild === this)
	...
	getAllDescendentsAsArray() {
		const result = [];
		this.children.forEach(function(child) {
			const childDescendents = child.getAllDescendentsAsArray();
			result.push(child);
			for (let i = 0; i < childDescendents.length; i++) {
				result.push(childDescendents[i]);
			}
		});
		return result;
	}
	...
	static getDescendentsOfType(token, type) {
		const result = [];
		if (token.type === type)
			result.push(token);
		for (let i = 0; i < token.children.length; i++) {
			const results = ParseTreeToken.getDescendentsOfType(token.children[i], type);
	...
	static getFirstDescendentTokenOf(n) {
	...
	getLastToken() {
		return ParseTreeToken.getLastDescendentTokenOf(this.getRoot());
	}

	static getLastDescendentTokenOf(n) {
		while (n.children.length !== 0) {
