...
export class ParseTreeToken {
	...
	insertAsFirstChild(newFirstChild) {
		const currentFirst = this.children[0];
		if (currentFirst === undefined)
			this.appendChild(newFirstChild);
		else {
			currentFirst.appendPreviousSibling(newFirstChild);
		}
	}
	...
	// removes this token while adding all of 
	// its children to the parent.
	// Another name for this could be "removeTokenOnly".
	removeSingleToken() {
		while (this.children.length !== 0) {
			const child = this.children[this.children.length - 1];
			child.remove();
			this.appendSibling(child);
		}
		this.remove();
	}
