...
export class SearchQuery {
	constructor(queryString) {
		...
		const parts = queryString.split(' ').filter(s => s !== '');
	...
	isMatchingStrict(example) {
		...
		if (example.hiddenKeywords !== undefined && example.hiddenKeywords.some(function(keyword) {
			...
			return true;
		if (this.commandConstraints.length !== 0)
			return true;
		return false;
	}
	...
	isMatchingLoose(example, code) {
		if (this.isMatchingMedium(example))
			return true;
		if (typeof code === 'string') {
			if (code.indexOf(this.queryString) !== -1)
				return true;
		}
		return false;
	}
