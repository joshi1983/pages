export class ArrayUtils {
	static combine() {
		...
		for (let i = 0; i < arguments.length; i++) {
			ArrayUtils.pushAll(result, arguments[i]);
		}
		return result;
	}
	...
	static insertArray(a, index, fromArray) {
		const minIndex = index + fromArray.length;
		for (let i = a.length + fromArray.length - 1; i >= minIndex; i--) {
			a[i] = a[i - fromArray.length];
		}
		const endIndex = index + fromArray.length;
		for (let i = index; i < endIndex; i++) {
			a[i] = fromArray[i - index];
		}
	}
