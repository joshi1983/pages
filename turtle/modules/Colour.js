...
export class Colour extends EventDispatcher {
	...
	static getSanitizationTips(s) {
		...
		if (Colour.isValidColourString('#' + s))
			return `Add # before your color code. For example, "#${s}`;
		if (Colour.isValidColourString(s.replace(/"/g, '')))
			return `Remove extra quote at beginning of your color code. For example, "${s.replace(/"/g, '')}`;
		if (s.length > 1 && Colour.isValidColourString(s.substring(1)))
			return `Remove the first ${s.charAt(0)} before your color code. For example, "${s.substring(1)}`;
		if (s.replace(/#/g, '').length < s.length - 1)
			return `There should be at most 1 # and it can only be at the beginning so remove it elsewhere. For example, "#${s.replace(/#/g, '')}`;
	}
