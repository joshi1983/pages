...
export function validateTokenChildrenLength(tokens, parseLogger) {
	tokens.forEach(function(token) {
		...
		if (numChildrenExpected !== undefined && numChildrenExpected !== token.children.length)
			parseLogger.error(`Expected ${numChildrenExpected} children expected for a token of type ${ParseTreeTokenType.getNameFor(token.type)} but found ${token.children.length}`, token);
	});
};