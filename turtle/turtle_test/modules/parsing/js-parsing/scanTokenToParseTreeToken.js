import { canBeBinaryOperator } from './scanning/canBeBinaryOperator.js';
import { canBeUnaryOperator } from './scanning/canBeUnaryOperator.js';
import { isAssignmentOperator } from './scanning/isAssignmentOperator.js';
...
export function scanTokenToParseTreeToken(token) {
	...
	if (type === undefined) {
		...
		else if (isAssignmentOperator(token.s))
			type = ParseTreeTokenType.ASSIGNMENT_OPERATOR;
		else if (canBeBinaryOperator(token.s))
			type = ParseTreeTokenType.BINARY_OPERATOR;
		else if (canBeUnaryOperator(token.s))
