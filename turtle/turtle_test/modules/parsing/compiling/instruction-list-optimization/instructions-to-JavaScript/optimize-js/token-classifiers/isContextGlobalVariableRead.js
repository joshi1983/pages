...
export function isContextGlobalVariableRead(token) {
	...
	if (p === null || p.children.length === 0)
		return false;
	p = p.children[0];
	if (p === null || p.type !== ParseTreeTokenType.IDENTIFIER || p.val !== 'context' || p.children.length === 0)
		return false;
	p = p.children[0];
	if (p === null || p.type !== ParseTreeTokenType.DOT || p.children.length === 0)
		return false;
	p = p.children[0];
