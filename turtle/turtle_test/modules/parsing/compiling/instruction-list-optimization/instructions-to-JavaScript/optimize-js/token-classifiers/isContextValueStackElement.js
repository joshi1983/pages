...
export function isContextValueStackElement(token) {
	if (token.type !== ParseTreeTokenType.EXPRESSION_INDEX_EXPRESSION ||
	token.children.length !== 2)
		return false;
	let tok = token.children[0];
	if (tok.type !== ParseTreeTokenType.IDENTIFIER || tok.val !== 'context' || tok.children.length !== 1)
		return false;
	tok = tok.children[0];
	if (tok.type !== ParseTreeTokenType.DOT || tok.children.length !== 1)
		return false;
	tok = tok.children[0];
