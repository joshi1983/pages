...
function isDotLocalVariablesSet(token) {
	...
	if (token.type !== ParseTreeTokenType.DOT || token.children.length !== 1)
		return false;
	token = token.children[0];
	if (token.type !== ParseTreeTokenType.IDENTIFIER || token.val !== 'set' || token.children.length !== 0)
		return false;
	return true;
}
...
context.getCurrentExecutingProcedure().localVariables.set("colorindex", 4)
*/
export function isContextLocalVariablesSet(token) {
	...
	if (expressionDot.type !== ParseTreeTokenType.EXPRESSION_DOT || expressionDot.children.length !== 2)
		return false;
	if (!isDotLocalVariablesSet(expressionDot.children[1]))
		return false;
