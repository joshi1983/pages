...
function processTokenWithPossibleBrackets(token, result, cachedParseTree) {
	const useBrackets = (token.type === ParseTreeTokenType.FUNCTION_CALL);
	if (useBrackets)
		result.append('(');
...
function areBracketsNeededFor(token, cachedParseTree) {
...
export function processBinaryOperatorToken(token, result, cachedParseTree) {
	...
	if (shouldValueBeSubstituted(opSymbol, token, cachedParseTree)) {
		...
		processTokenWithPossibleBrackets(leftSide, result, cachedParseTree);
