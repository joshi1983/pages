...
export class ListCommands {
	...
	duplicate(value, duplicateCount) {
		const result = [];
	...
	sublist(list1, fromIndex, toIndex) {
		...
		if (fromIndex > list1.length)
			throw new Error(`fromIndex must not be greater than list1's length of ${list1.length}.  fromIndex specified to be ${fromIndex}`);
		fromIndex--;
		if (toIndex < 1)
...
[combine, fput, indexOf, lput, mixItems, mixItems2].forEach(function(func) {
