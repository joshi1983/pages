...
async function startDownscaling() {
	...
	try {
		...
		// Check that this isn't a kind of error that is thrown when the graphics screen is resized to 0 height or width.
		if (typeof e.message !== 'string' ||
		(!e.message.startsWith('Unable to downscale because originalSize.w') &&
		!e.message.startsWith('Neither dimension can be 0 but got width='))) {
			// Don't fail silently if we're not expecting this specific kind of error/exception.
			console.error(e);
		}
	}
...
setTimeout(function() {
	...
	delayDownscaling();
}, 10000); // give a little time for modules to initialize.
