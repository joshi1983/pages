...
export function drawSimpleRect(svgDrawer, path) {
	...
	if (yFormatted.indexOf('"0"') !== -1)
		yFormatted = '';
	svgDrawer.pushTag(`<rect${xFormatted}${yFormatted} width="${rounder.formatNumber(width)}" height="${rounder.formatNumber(height)}"${svgDrawer.getStyleAttributes(path.style)}${rotation} />`);
};